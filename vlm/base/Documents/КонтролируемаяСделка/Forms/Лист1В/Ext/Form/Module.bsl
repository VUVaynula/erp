...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ИдентификаторСледующегоЛиста1В.СписокВыбора.Очистить();
	Для Каждого ЗначениеСписка Из Параметры.Листы1В Цикл
		Элементы.ИдентификаторСледующегоЛиста1В.СписокВыбора.Добавить(Строка(ЗначениеСписка.Значение), ЗначениеСписка.Представление);
	КонецЦикла;
	
	НастроитьСвязьСЛистами1Б(ЭтотОбъект);
	
	НастроитьСвязьСЛистами1В(ЭтотОбъект);
...
Функция ВернутьСтруктуруЗакрытия()
	...
	Если ЗначениеЗаполнено(ИдентификаторСледующегоЛиста1В) Тогда
		Структура.Вставить("ИдентификаторСледующегоЛиста1В", Новый УникальныйИдентификатор(ИдентификаторСледующегоЛиста1В));
	Иначе
		Структура.Вставить("ИдентификаторСледующегоЛиста1В", ОбщегоНазначенияКлиентСервер.ПустойУникальныйИдентификатор());
	КонецЕсли;
...
Функция НезаполняемыеПоля()
	...
	ИначеЕсли ЭтоРозничнаяПродажа(ПризнакУчастникаСделки) Тогда
		НезаполняемыеПоля.Добавить("ДатаСовершенияСделки");
		НезаполняемыеПоля.Добавить("Контрагент");
		НезаполняемыеПоля.Добавить("НомерДоговора");
		НезаполняемыеПоля.Добавить("ДатаДоговора");
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьСвязьСЛистами1В(Форма)
	
	Элементы = Форма.Элементы;
	
	ПризнакУчастникаСделки = Форма.ПризнакУчастникаСделки;
	
	Элементы.ИдентификаторСледующегоЛиста1В.Видимость = 
		ЗначениеЗаполнено(ПризнакУчастникаСделки)
		И ПризнакУчастникаСделки = ПредопределенноеЗначение("Перечисление.ТипыУчастниковЦепочкиКонтролируемыхСделок.ТорговыйПосредник")
		И Элементы.ИдентификаторСледующегоЛиста1В.СписокВыбора.Количество() > 0;
		
	ЭтоРозничнаяПродажа = ЭтоРозничнаяПродажа(ПризнакУчастникаСделки);
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоРозничнаяПродажа(ПризнакУчастникаСделки)
