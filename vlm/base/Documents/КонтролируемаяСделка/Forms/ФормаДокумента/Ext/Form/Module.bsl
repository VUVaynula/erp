...
Процедура ИзменениеСтрокиЛиста1В(ЭтоНовый = Ложь, Копирование = Ложь)
	...
	СтруктураПараметров.Вставить("Листы1В", СписокВыбораЛистов1В(Копирование));
...
Функция ПолучитьСтруктуруТабличнойЧастиЛисты1В()
	...
	СтруктураТабличнойЧастиСделки = Новый Структура(
		...
		|КомментарийКЛисту1Б,
		|ИдентификаторСледующегоЛиста1В,
	...
	СтруктураТабличнойЧастиСделки.Вставить("СвязанныеЛисты1Б", СвязанныеЛисты1Б);
...
&НаКлиенте
Функция СписокВыбораЛистов1В(Копирование)
	
	// Добавляем все листы, кроме текущего.
	СтрокаТаблицы = Элементы.Листы1В.ТекущиеДанные;
	ИдентификаторТекущейСтроки = Неопределено;
	Если СтрокаТаблицы <> Неопределено И Не Копирование Тогда
	...
	Для Каждого СтрокаЛиста1В Из Объект.Листы1В Цикл
		...
		Если СтрокаТаблицы <> Неопределено
			...
			// Можно выбирать в качестве следующей сделки только сделки с таким же типом листа.
