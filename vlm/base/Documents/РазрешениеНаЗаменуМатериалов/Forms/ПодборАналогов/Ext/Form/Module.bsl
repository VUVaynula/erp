...
Процедура ЗаполнитьДанные(ПараметрыТоваров, ПараметрыАналогов, ПовторноеЗаполнение = Ложь)
	...
	ДанныеРазрешение = Новый Структура("ТаблицаМатериаловРазрешений, ТаблицаАналоговРазрешений, Разрешение, ВзаимозаменяемыйАналог",
...
Процедура ЗаполнитьКоличествоПоРазрешению()
	...
	СтруктураОтбора = Новый Структура("Разрешение, ЭтоАналог, КлючСтроки", ТекущаяСтрокаАналоги.Разрешение);
	
	Если ВзаимозаменяемыйАналог Тогда
		
		КоэффициентРасчета = 0;
		СтруктураОтбора.ЭтоАналог = Истина;
		СтруктураОтбора.КлючСтроки = 0;
		МассивСтрок = Аналоги.НайтиСтроки(СтруктураОтбора);
	...
	СтруктураОтбора.КлючСтроки = КлючСтроки;
	...
	Для каждого СтрокаАналоги Из МассивСтрок Цикл
		...
		Если БазаРаспределения <> 0 Тогда
			...
			Для каждого КлючСвязиИД Из МассивКлючейСвязи Цикл
				...
				СтрокиМатериалов = Материалы.НайтиСтроки(Новый Структура("КлючСвязи, КлючСтроки", КлючСвязи, КлючСтроки));
	...
	Для каждого СтрокаТаблицы Из ТаблицаАналоги Цикл
		...
		Если Подтвердить Тогда
			...
			Для Каждого СтрокаМассива Из МассивОстатковАналогов Цикл
				...
				КоличествоКСписанию = Мин(КоличествоАналогаЗаменить, СтрокаМассива.Свободно);
...
Процедура УстановитьНачальноеОформлениеСтраницы(ИмяСтраницы, ПризнакПоказатьОстатки, УчитыватьЗапасы)
	...
	Если ИмяСтраницы = "Аналоги" Тогда
		...
		КлючСтроки = СтрокаМатериал.КлючСтроки;
	   ...
	   И Форма.РежимОтображенияОстатков <> РежимНеОбособленныеОстатки Тогда
		...
		Для Каждого СтрокаОстатков Из ОстаткиПоНазначению Цикл
			...
			Для Каждого СтрокаАналог Из МассивОстатковАналогов Цикл
				...
				ЕстьДругоеНазначениеВНаборе = РазличныеНазначенияНабораМатериалов(
					Форма, СтрокаАналог.КлючСвязиРазрешение, Назначение, СтрокаАналог.ВзаимозаменяемыйАналог);
					
				Если ЕстьДругоеНазначениеВНаборе Тогда
					ОбособленныйОстаток = 0;
				Иначе
					ОбособленныйОстаток = СтрокаОстатков.Свободно;
				КонецЕсли;
				
				Если ЕстьДругоеНазначениеВНаборе И ДоступностьРежимаОстатков Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция РазличныеНазначенияНабораМатериалов(
	Форма, КлючСвязиРазрешение, ПроверяемоеНазначение, ВзаимозаменяемыйАналог)

	ЕстьДругоеНазначениеВНаборе = Ложь;
	КлючСтроки = Форма.КлючСтроки;
	
	Если Не ВзаимозаменяемыйАналог Тогда
	
		Для Каждого СтрокаМатериал Из Форма.Материалы Цикл
	
			Если СтрНайти(КлючСвязиРазрешение, СтрокаМатериал.КлючСвязи) > 0
			   И КлючСтроки = СтрокаМатериал.КлючСтроки Тогда
			
				Если СтрокаМатериал.Назначение <> ПроверяемоеНазначение Тогда
					ЕстьДругоеНазначениеВНаборе = Истина;
					Прервать;
				КонецЕсли;
	...
	Возврат ЕстьДругоеНазначениеВНаборе;
