...
Процедура ДобавитьИзображениеСПроверкойЗаписи()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстВопроса = НСтр("ru = 'Для добавления изображения необходимо записать документ. Записать?';
							|en = 'Для добавления изображения необходимо записать документ. Записать?'", 
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура ПросмотретьИзображениеОбработчик()
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Отсутствует изображение для просмотра';
								|en = 'Отсутствует изображение для просмотра'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Процедура УстановитьТекстНевыбраннойКартинки(ТекущиеДанные)
	...
	Если ЭтоТоварИзСервиса(ТекущиеДанные)
		...
		ТекстНевыбраннойКартинки = НСтр("ru = 'Для этой карточки 1С:Номенклатура изображения отсутствуют';
										|en = 'Для этой карточки 1С:Номенклатура изображения отсутствуют'", 
			ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	Иначе
		ТекстНевыбраннойКартинки = НСтр("ru = 'Выберите изображение';
										|en = 'Выберите изображение'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	КонецЕсли;
