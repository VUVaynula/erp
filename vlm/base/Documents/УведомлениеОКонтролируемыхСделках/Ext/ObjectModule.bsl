	...
	Для Каждого ЭлементВыгрузки Из СодержаниеВыгрузки Цикл
		...
		Исключение
			ИмяСобытия = НСтр("ru = 'Уведомление о контролируемых сделках. Выгрузка документа.';
								|en = 'Уведомление о контролируемых сделках. Выгрузка документа.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ЭлектронноеПредставление505(ОсновныеСведенияВыгрузки, УникальныйИдентификатор = Неопределено)
	...
	Возврат СформироватьОкончательныйПакетФайлов(Контекст, ВременныйПакетФайлов, УникальныйИдентификатор);
...
Процедура ЗаполнитьСведенияОПоследующейРеализации505(Контекст, ТекстСделки, Листы1В, Сделка)
	...
	Для Каждого Лист1В Из СписокЛистов1В Цикл
		...
		УстановитьНеобязательныйАтрибут(Шаблон_СвСдПослРеал, "НомПорЛистаВ.1", Лист1В.НомерЛиста1В);
		УстановитьНеобязательныйАтрибут(Шаблон_СвСдПослРеал, "НомПорЛистаВ.2", Лист1В.НомерЛиста1В_Следующий1);
		УстановитьНеобязательныйАтрибут(Шаблон_СвСдПослРеал, "НомПорЛистаВ.3", Лист1В.НомерЛиста1В_Следующий2);
		УстановитьНеобязательныйАтрибут(Шаблон_СвСдПослРеал, "НомПорЛистаВ.4", Лист1В.НомерЛиста1В_Следующий3);
