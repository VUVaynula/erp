...
Функция ПолучитьТекстЗапросаПоКонтролируемымСделкам_2012() Экспорт
	...
	ТекстЗапроса =
	"// Ограничим список организаций и контрагентов теми, что попадут в уведомление.
	...
	|
	|ВЫБРАТЬ
	|	КонтролируемаяСделка.Контрагент  КАК Ссылка
	|ИЗ
	|	Документ.КонтролируемаяСделка КАК КонтролируемаяСделка
	|ГДЕ
	|	КонтролируемаяСделка.УведомлениеОКонтролируемойСделке = &УведомлениеОКонтролируемойСделке
	|	И НЕ КонтролируемаяСделка.Контрагент В (ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка),
	...
	|
	|ВЫБРАТЬ
	|	КонтролируемаяСделка.Комиссионер КАК Ссылка
	|ИЗ
	|	Документ.КонтролируемаяСделка КАК КонтролируемаяСделка
	|ГДЕ
	|	КонтролируемаяСделка.УведомлениеОКонтролируемойСделке = &УведомлениеОКонтролируемойСделке
	|	И НЕ КонтролируемаяСделка.Комиссионер В (ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка),
	|											ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	|	И НЕ КонтролируемаяСделка.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ
	...
	|	И НЕ Листы1В.Контрагент В (ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка),
	|											ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	...
	|	И НЕ Листы1Г.Контрагент В (ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка),
	|											ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Получим периодические реквизиты по организациям и контрагентам на дату актуальности сведений.
	|
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|
	|	ВЫБОР
	|		КОГДА Контрагенты.ГоловнойКонтрагент = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА
	|			Контрагенты.Ссылка
	|		ИНАЧЕ
	...
	| Организации.СтранаРегистрации.Код КАК КодСтраныРегистрации,
	...
	|	КонтрагентыКонтактнаяИнформация.Значение  КАК ЮридическийАдресЗначение
	|ИЗ
	|	ДатыАктуальностиАдресов КАК ДатыАктуальностиАдресов
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|// Соберем актуальные значения периодических реквизитов организаций и контрагентов в одной таблице.
	|
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Контрагенты.Ссылка КАК Ссылка,
	|	Контрагенты.ОбособленноеПодразделение КАК ОбособленноеПодразделение,
	...
	|	КонтролируемаяСделкаЛисты1В.ИдентификаторСледующегоЛиста1В КАК ИдентификаторСледующегоЛиста1В,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КонтролируемаяСделка.СвязьЛистов1Би1В КАК КонтролируемаяСделкаСвязьЛистов1Би1В
	|		ПО (КонтролируемаяСделкаЛисты1В.ИдентификаторЛиста1В = КонтролируемаяСделкаСвязьЛистов1Би1В.ИдентификаторЛиста1В)
	|ГДЕ
...
Функция ПолучитьТекстЗапросаПоКонтролируемымСделкам_2018() Экспорт
	...
	|ГДЕ
	|	КонтролируемаяСделка.УведомлениеОКонтролируемойСделке = &УведомлениеОКонтролируемойСделке
	|	И НЕ КонтролируемаяСделка.Контрагент В (ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	...
	|ГДЕ
	|	КонтролируемаяСделка.УведомлениеОКонтролируемойСделке = &УведомлениеОКонтролируемойСделке
	|	И НЕ КонтролируемаяСделка.Комиссионер В (ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|											ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	|	И НЕ КонтролируемаяСделка.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ
	...
	|ГДЕ
	|	Листы1В.Ссылка.УведомлениеОКонтролируемойСделке = &УведомлениеОКонтролируемойСделке
	|	И НЕ Листы1В.Контрагент В (ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|											ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	|	И НЕ Листы1В.Ссылка.ПометкаУдаления
	|
	|ОБЪЕДИНИТЬ
	...
	|ГДЕ
	|	Листы1Г.Ссылка.УведомлениеОКонтролируемойСделке = &УведомлениеОКонтролируемойСделке
	|	И НЕ Листы1Г.Контрагент В (ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|											ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	|	И НЕ Листы1Г.Ссылка.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура СформироватьКонтролируемыеСделки(Уведомление, СообщатьОПрогрессе)
	...
	КешАдресныхДанных = Новый Соответствие;
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСделка.ИдентификаторЛиста1Б = Новый УникальныйИдентификатор();
		ЗаполнитьАдресаСделки(НоваяСделка, Выборка.Грузоотправитель, Выборка.Грузополучатель, ВерсияУведомления, КешАдресныхДанных);
	КонецЦикла;
...
Функция ПолучитьВременныеТаблицыДляЗаполненияСделок(Уведомление)
	...
	ВалютаРегламентированногоУчета = ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(ПараметрыУведомления.Организация);
...
Процедура ЗаполнитьАдресаСделки(СведенияОСделке, Грузоотправитель, Грузополучатель, ВерсияУведомления, КешАдресныхДанных)
	
	Если ВерсияУведомления < КонтролируемыеСделкиКлиентСервер.ВерсияУведомления_2024() Тогда
	...
	Иначе
		...
		ПараметрыАдресаГрузополучателя = ПараметрыАдресаСделки2024(Грузополучатель, КешАдресныхДанных, СведенияОСделке.ДатаСовершенияСделки);
		Если ПараметрыАдресаГрузополучателя <> Неопределено Тогда
...
// 
// Возвращаемое значение:
//  Структура - Получить сведения об уведомлении:
// * Ошибка - Булево - 
...
Функция ПолучитьОсновныеСведенияУведомленияДляВыгрузки(Уведомление) Экспорт
	...
	Если РеквизитыУведомления.КодМестаПредставления = КодМестаПредставленияКрупнейший() И ПараметрыОрганизации.КрупнейшийНалогоплательщик Тогда
		РеквизитыКН = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация,
			ДатаСведений, "КодНОКрупнейший, КППЮЛКрупнейший, ОКТМОКрупнейший");
		ОсновныеСведения.Вставить("КодНО", РеквизитыКН.КодНОКрупнейший);
		ОсновныеСведения.Вставить("КППЮЛ", РеквизитыКН.КППЮЛКрупнейший);
		ОсновныеСведения.Вставить("ОКТМО", РеквизитыКН.ОКТМОКрупнейший);
	КонецЕсли;
