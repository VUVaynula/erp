...
Функция ПолучитьЛистыУведомления(Уведомление, Лист1АДляПечати = Неопределено) Экспорт
	...
	|	Листы1В КАК Листы1В
	|УПОРЯДОЧИТЬ ПО
	|	Листы1В.НомерЛиста1А, Листы1В.НомерСтроки
	|;
	|
	|//////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Листы1Г КАК Листы1Г
	|УПОРЯДОЧИТЬ ПО
	|	Листы1Г.НомерЛиста1А, Листы1Г.ИдентификаторЛиста1Б, Листы1Г.НомерСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТипНомераЛиста1В = ОбщегоНазначения.ОписаниеТипаСтрока(5);
	
	Листы1В.Колонки.Добавить("НомерЛиста1Б", ТипНомераСтраницы);
	Листы1В.Колонки.Добавить("НомерЛиста1В", ТипНомераЛиста1В);
	Листы1В.Колонки.Добавить("НомерЛиста1В_Следующий1", ТипНомераЛиста1В);
	Листы1В.Колонки.Добавить("НомерЛиста1В_Следующий2", ТипНомераЛиста1В);
	Листы1В.Колонки.Добавить("НомерЛиста1В_Следующий3", ТипНомераЛиста1В);
	...
	Для каждого Лист1А Из Листы1А Цикл
		...
		Если Лист1А.УказыватьСведенияЦепочкиСозданияСтоимости Тогда
			...
			// Номера листов 1В делаем уникальным в пределах листа 1А.
			// Явных указаний на это нет, возмжно, уникальность должна быть и в пределах листа 1Б.
			// Но из текста заполнения точно про это не сказано, поэтому мы для простоты делаем уникальность в пределах 1А.
			НомерЛиста1В = 0;
			НомерЛиста1ВБез1Б = 0;
			
			ТаблицаСледущихЛистов1В = Новый ТаблицаЗначений;
			ТаблицаСледущихЛистов1В.Колонки.Добавить("НомерЛиста1Б", ТипНомераСтраницы);
			ТаблицаСледущихЛистов1В.Колонки.Добавить("ИдентификаторЛиста1В", Новый ОписаниеТипов("УникальныйИдентификатор"));
			ТаблицаСледущихЛистов1В.Колонки.Добавить("НомерЛиста1В", ТипНомераЛиста1В);
			ТаблицаСледущихЛистов1В.Колонки.Добавить("НомерЛиста1В_Следующий1", ТипНомераЛиста1В);
			ТаблицаСледущихЛистов1В.Колонки.Добавить("НомерЛиста1В_Следующий2", ТипНомераЛиста1В);
			ТаблицаСледущихЛистов1В.Колонки.Добавить("НомерЛиста1В_Следующий3", ТипНомераЛиста1В);
			ТаблицаСледущихЛистов1В.Колонки.Добавить("ИдентификаторСледующегоЛиста1В_0", Новый ОписаниеТипов("УникальныйИдентификатор"));
			ТаблицаСледущихЛистов1В.Колонки.Добавить("ИдентификаторСледующегоЛиста1В_1", Новый ОписаниеТипов("УникальныйИдентификатор"));
			ТаблицаСледущихЛистов1В.Колонки.Добавить("ИдентификаторСледующегоЛиста1В_2", Новый ОписаниеТипов("УникальныйИдентификатор"));
			ТаблицаСледущихЛистов1В.Колонки.Добавить("ИдентификаторСледующегоЛиста1В_3", Новый ОписаниеТипов("УникальныйИдентификатор"));
			...
			Для Каждого Лист1В Из СписокЛистов1В Цикл
				...
				Если Лист1В.ИдентификаторЛиста1Б <> NULL Тогда
					НомерЛиста1В = НомерЛиста1В + 1;
					Лист1В.НомерЛиста1В = Формат(НомерЛиста1В, "ЧЦ=5; ЧДЦ=0; ЧГ=0");
				...
				Иначе
					НомерЛиста1ВБез1Б = НомерЛиста1ВБез1Б + 1;
					Лист1В.НомерЛиста1В = "0." + Формат(НомерЛиста1ВБез1Б, "ЧЦ=3; ЧДЦ=0; ЧГ=0");
				КонецЕсли;
				
				СтрокаСледующегоЛиста1В = ТаблицаСледущихЛистов1В.Добавить();
				СтрокаСледующегоЛиста1В.НомерЛиста1Б = Лист1В.НомерЛиста1Б;
				СтрокаСледующегоЛиста1В.ИдентификаторЛиста1В = Лист1В.ИдентификаторЛиста1В;
				СтрокаСледующегоЛиста1В.ИдентификаторСледующегоЛиста1В_0 = Лист1В.ИдентификаторСледующегоЛиста1В;
				СтрокаСледующегоЛиста1В.НомерЛиста1В = Лист1В.НомерЛиста1В;
				
			КонецЦикла;
			
			ЗаполнитьСвязиЛистов1В(СписокЛистов1В, ТаблицаСледущихЛистов1В);
...
Процедура ЗаполнитьСвязиЛистов1В(СписокЛистов1В, ТаблицаСледущихЛистов1В)
	
	ДлинаИерархии = 3;
	Для НомерИерархии = 1 По ДлинаИерархии Цикл
		
		Для Каждого Лист1В Из ТаблицаСледущихЛистов1В Цикл
			
			СтрокиСледующегоЛиста1В = ТаблицаСледущихЛистов1В.НайтиСтроки(Новый Структура("НомерЛиста1Б", Лист1В.НомерЛиста1Б));
			Для Каждого СтрокаСледующегоЛиста1В Из СтрокиСледующегоЛиста1В Цикл
				Если СтрокаСледующегоЛиста1В["ИдентификаторСледующегоЛиста1В_" + Формат(НомерИерархии - 1, "ЧДЦ=0; ЧН=0; ЧГ=0")] = Лист1В.ИдентификаторЛиста1В Тогда
					СтрокаСледующегоЛиста1В["НомерЛиста1В_Следующий" + Формат(НомерИерархии, "ЧДЦ=0; ЧН=0; ЧГ=0")] = Лист1В.НомерЛиста1В;
					СтрокаСледующегоЛиста1В["ИдентификаторСледующегоЛиста1В_" + Формат(НомерИерархии, "ЧДЦ=0; ЧН=0; ЧГ=0")] = Лист1В.ИдентификаторСледующегоЛиста1В_0;
				КонецЕсли;
			КонецЦикла;
	...
	ИндексСтрокиСписка = 0;
	Для Каждого СтрокаСписка Из СписокЛистов1В Цикл
		СтрокаСледующихЛистов = ТаблицаСледущихЛистов1В[ИндексСтрокиСписка];
		Для НомерИерархии = 1 По ДлинаИерархии Цикл
			СтрокаСписка["НомерЛиста1В_Следующий" + Формат(НомерИерархии, "ЧДЦ=0; ЧН=0; ЧГ=0")] =
				СтрокаСледующихЛистов["НомерЛиста1В_Следующий" + Формат(НомерИерархии, "ЧДЦ=0; ЧН=0; ЧГ=0")]
		КонецЦикла;
		ИндексСтрокиСписка = ИндексСтрокиСписка + 1;
	КонецЦикла;
...
Процедура ЗаполнитьКолонкиАдресаИностранногоКонтрагента(СтрокаКонтрагента)
	...
	Если ЗначениеЗаполнено(СведенияОбАдресе.Регион) Тогда
		СтрокаКонтрагента.АдресИностранный_Регион = СведенияОбАдресе.Регион;
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(СведенияОбАдресе.НаселенныйПункт) Тогда
		СтрокаКонтрагента.АдресИностранный_НаселенныйПункт = СведенияОбАдресе.НаселенныйПункт;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СведенияОбАдресе.Район) Тогда
		СтрокаКонтрагента.АдресИностранный_Район = СведенияОбАдресе.Район;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СведенияОбАдресе.Улица) Тогда
		Если СведенияОбАдресе.ТипАдреса ="ЕАЭС" Тогда
			АдресЕАЭС = Новый Массив;
			
			АдресЕАЭС.Добавить(СокрЛП(СведенияОбАдресе.Улица));
			
			ШаблонЧастиОкончанияАдреса = "%1 %2";
			
			Если ЗначениеЗаполнено(СведенияОбАдресе.Здание) Тогда
				АдресЕАЭС.Добавить(СтрШаблон(ШаблонЧастиОкончанияАдреса,
			...
			ПолныйАдресЕАЭС = СтрСоединить(АдресЕАЭС,", ");
			
			СтрокаКонтрагента.АдресИностранный_УлицаДом = ПолныйАдресЕАЭС;
			
		Иначе
			СтрокаКонтрагента.АдресИностранный_УлицаДом = СведенияОбАдресе.Улица;
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеЗаполненияЛиста1В(СведенияОбУведомлении, Лист1В)
	...
	ДобавитьСтрокуВСтруктуруПараметров(ДанныеЗаполнения, "НомерЛиста1В_", Лист1В.НомерЛиста1В);
	...
	ДобавитьСтрокуВСтруктуруПараметров(ДанныеЗаполнения, "НомерЛиста1В_2_", Лист1В.НомерЛиста1В_Следующий1);
	ДобавитьСтрокуВСтруктуруПараметров(ДанныеЗаполнения, "НомерЛиста1В_3_", Лист1В.НомерЛиста1В_Следующий2);
	ДобавитьСтрокуВСтруктуруПараметров(ДанныеЗаполнения, "НомерЛиста1В_4_", Лист1В.НомерЛиста1В_Следующий3);
	...
	Если Перечисления.ТипыИспользованияПроисхожденияТовараКонтролируемыхСделок.НужноВыгружатьСведенияОСделке(Лист1В.ИспользованиеПроисхождениеТовара) Тогда
		...
		ДанныеЗаполнения.Вставить("ПризнакУчастникаСделки",
			Перечисления.ТипыУчастниковЦепочкиКонтролируемыхСделок.КодДляВыгрузки(Лист1В.ПризнакУчастникаСделки));
...
Функция ДанныеЗаполненияРаздела2(СведенияОбУведомлении, Лист2, ДанныеРаздела2)
	...
	Если СведенияОКонтрагенте.Строка020ТипОрганизации <> 1 Тогда
		...
		ГородИндекс.Добавить(СведенияОКонтрагенте.АдресИностранный_Регион);
		...
		ДобавитьСтрокуВСтруктуруПараметров(ДанныеЗаполнения, "АдресИностранный_Регион_", "");
