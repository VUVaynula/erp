...
#Область Отложенно
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение даты начала действия локальной проверки разрешительного режима при розничной продаже';
									|en = 'Заполнение даты начала действия локальной проверки разрешительного режима при розничной продаже'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение раздельных дат контроля маркировки по товарным группам';
									|en = 'Заполнение раздельных дат контроля маркировки по товарным группам'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
