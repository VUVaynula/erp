...
Процедура ЧислоВОбластиМакета(Число, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати)
	...
	Пока Истина Цикл 
		Индекс = Индекс + 1;
		Обл = ПечатнаяФорма.Области.Найти(ПрефиксПараметра + "_" + Формат(Индекс, "ЧГ="));
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		ПустаяСтрока = ПустаяСтрока + КонтекстПечати["СимволПоУмолчанию"];
		
		Если Не ТочкаПройдена И Индекс > 1 Тогда 
			ТочкаПройдена = ТочкаПройдена Или (ПраваяЧастьОбласти + 1 <> Обл.Лево);
		КонецЕсли;
		ПраваяЧастьОбласти = Обл.Право;
		
		ДоЗапятой = ДоЗапятой + ?(ТочкаПройдена, 0, 1);
		ПослеЗапятой = ПослеЗапятой + ?(ТочкаПройдена, 1, 0);
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(ДоЗапятой) Или Не ЗначениеЗаполнено(Число) Тогда 
		Строка = "";
	Иначе
		Остаток = (Число - Цел(Число));
		...
		Пока ЗначениеЗаполнено(Остаток - Цел(Остаток)) И СтрДлина(ДЧ) < ПослеЗапятой Цикл 
			...
			Остаток = ?(Остаток < 0, -1*Остаток, Остаток);
		КонецЦикла;
		Строка = Лев(Формат(Цел(Число), "ЧН=0; ЧГ=") + ПустаяСтрока, ДоЗапятой) + Лев(ДЧ + ПустаяСтрока, ПослеЗапятой);
	КонецЕсли;
	
	СтрокаВОбластиМакетаСлужебная(Строка, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати);
КонецПроцедуры
...
Процедура ДатаВОбластиМакета(Дата, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати)
	СтрокаВОбластиМакетаСлужебная(Формат(Дата, КонтекстПечати["ФорматДатыПоУмолчанию"]), ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати);
КонецПроцедуры

Процедура СтрокаВОбластиМакетаСлужебная(Строка, ПрефиксПараметра, ПечатнаяФорма, КонтекстПечати)
	Индекс = 0;
	...
	Пока Истина Цикл 
		...
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Текст = ?(Индекс > ДлинаСтроки, КонтекстПечати["СимволПоУмолчанию"], Сред(Строка, Индекс, 1));
	КонецЦикла;
КонецПроцедуры
...
Процедура ПоложитьПФВСписокЛистов(Листы, ПечатнаяФорма, Наименование, КонтекстПечати) Экспорт 
	Если КонтекстПечати.Автонумерация Тогда 
		СтрокаВОбластиМакетаСлужебная(Прав("000" + Формат(Листы.Количество() + 1, "ЧГ="), 3), "НомСтр", ПечатнаяФорма, КонтекстПечати);
	КонецЕсли;
