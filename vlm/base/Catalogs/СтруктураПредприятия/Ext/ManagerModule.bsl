...
Процедура ЗаполнитьИспользуемыеМетодикиУправления() Экспорт
	...
	Запрос = Новый Запрос(
		...
		|			И Производство2_2.Значение
		|			И НЕ ДинамическаяСтруктура.Значение) КАК ИспользоватьПооперационноеПланирование
		|ИЗ
		|	Справочник.СтруктураПредприятия КАК СтруктураПредприятия
		...
		|		ПО (ИСТИНА)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.ДинамическаяСтруктураЗаказовНаПроизводство КАК ДинамическаяСтруктура
		|		ПО (ИСТИНА)
		|");
