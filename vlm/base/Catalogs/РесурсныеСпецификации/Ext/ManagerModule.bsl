...
Процедура СоздатьПромежуточнуюНоменклатуруХарактеристику(Строка, КэшСсылокПромежуточногоВыпуска)
	...
	Попытка
		...
		Если СоздатьХарактеристику Тогда
			...
			Таблица.Колонки.Добавить("НоменклатураИсходногоКачества", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
			...
			НоваяСтрока.Номенклатура = Строка.НоменклатураПромежуточногоВыпуска;
			НоваяСтрока.НоменклатураИсходногоКачества = Строка.Номенклатура;
			...
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Таблица, СтруктураДействий);
			
			Строка.ХарактеристикаПромежуточногоВыпуска = НоваяСтрока.Характеристика;
			КэшСсылокПромежуточногоВыпуска.Вставить(Строка.Характеристика, НоваяСтрока.Характеристика);
		КонецЕсли;
