...
Функция РассчитатьГрафикЗаказаВнутриЗамераВремени(ЗаказНаПроизводство, Граница)
	...
	Запрос = Новый Запрос(
		...
		|	ВЫБОР
		|		КОГДА ДанныеЗаказа.НачатьНеРанее >= &ТекущаяДата ТОГДА
		|			ДанныеЗаказа.НачатьНеРанее
		|		ИНАЧЕ
		|			&ТекущаяДата
		...
		// в текущей конфигурации поддерживается только размещение "К окончанию"
		|	ЛОЖЬ КАК РазмещениеКНачалу,
		|	ГраницыРазмещения.ПраваяГраница КАК НачалоРазмещения
		|ИЗ
