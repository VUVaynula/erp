...
Функция ПараметрыВыгрузкиОтчета2025(ИмяФормы, СтруктураРеквизитовФормы, ДанныеОтчета, РеглОтчетВыгружаетсяВБанк)
	...
	Если ДанныеОтчета.ЭтоКрупнейшийНалогоплательщик Тогда
		ПараметрыВыгрузки.Вставить("КППОрг",
	...
	Если ЗначениеЗаполнено(ЗначениеПоказателяРаздела(ДеревоДанныхРазделов, "Баланс", "АудитДа")) Тогда
	...
	Если ЗначениеЗаполнено(ЗначениеПоказателяРаздела(ДеревоДанныхРазделов, "Баланс", "УтверждениеДа")) Тогда
...
Процедура ПроверитьВозможностьВыгрузки_ФормаОтчета2025Кв1(СтатусПроверки, ПараметрыПроверки)
	...
	Если НЕ ПараметрыВыгрузки.ОтчетВыгружаетсяВБанк И ПараметрыВыгрузки.ЭтоИП Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
	...
	Если НЕ ПараметрыВыгрузки.ЕстьПояснительнаяЗаписка Тогда
		СообщитьОбОшибкеЗаполнения(РезультатВыгрузки,
