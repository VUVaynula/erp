...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.р.НДС прилож.07 25к2") Тогда
			...
			Для каждого Строка1 Из Строки1 Цикл
				...
				Если ЗначениеЗаполнено(ОблОбъедСтрока1_Верх) И ЗначениеЗаполнено(ОблОбъедСтрока1_Низ)
					...
					Для НомерКолонки = 2 По 10 Цикл
						...
						ОблОбъед.Объединить();
					КонецЦикла;
					ОблОбъед = ПечФорма1.Область(ОблОбъедСтрока1_Верх, 16, ОблОбъедСтрока1_Низ, 15);
					ОблОбъед.Объединить();
