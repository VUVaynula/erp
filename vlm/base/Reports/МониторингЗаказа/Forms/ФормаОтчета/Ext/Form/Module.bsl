...
Процедура ЗагрузитьПервыйУровень()
	...
	|
	|ГДЕ
	|	Изделия.Требуется <> 0 И &УсловиеПолучения
	|
	|СГРУППИРОВАТЬ ПО
	|	План.Номенклатура,
	|	План.Характеристика,
...
Процедура НачатьОжиданиеДлительнойОперации()
	...
	ФормаДлительнойОперации = ДлительныеОперацииКлиент.ОткрытьФормуДлительнойОперации(ЭтаФорма, ДлительнаяОперация.ИдентификаторЗадания);
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	Попытка
		...
		Если ПроверитьВыполнениеЗадания(ДлительнаяОперация) Тогда
...
&НаСервереБезКонтекста
Функция ПроверитьВыполнениеЗадания(ДлительнаяОперация)
	
	Если Не ЗначениеЗаполнено(ДлительнаяОперация) Или Не ЗначениеЗаполнено(ДлительнаяОперация.ИдентификаторЗадания) Тогда
