...
Функция ТекстЗапросаГрафикПоВидамРЦ(СтатусГрафика)
	...
	Если СтатусГрафика = РегистрыСведений.ГрафикЭтаповПроизводства2_2.СтатусРабочийГрафик() Тогда
		...
		|	ВЫБОР
		|		КОГДА ДоступностьВидовРабочихЦентров.ВидРабочегоЦентра.Подразделение.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
		|			ТОГДА НАЧАЛОПЕРИОДА(ДоступностьВидовРабочихЦентров.ДатаИнтервала, ДЕНЬ)
		|		ИНАЧЕ ДоступностьВидовРабочихЦентров.ДатаИнтервала
		|	КОНЕЦ КАК Начало,
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА ПланированиеЗагрузки.ВидРабочегоЦентра.Подразделение.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
		|			ТОГДА НАЧАЛОПЕРИОДА(ПланированиеЗагрузки.ДатаИнтервала, ДЕНЬ)
		|		ИНАЧЕ ПланированиеЗагрузки.ДатаИнтервала
		|	КОНЕЦ,
