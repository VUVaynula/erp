...
Функция ЕстьИзмененияИтоговНабора(Набор, ДвиженияДоИзменения) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Не удалось определить ресурсы для сравнения итогов набора записей.';
							|en = 'Cannot identify the resources for comparing record set totals.'");
...
Функция ТекстРекомендацииДляРегистратораРасчетов() Экспорт
	...
	|en = 'Для аналитики учета по партнерам и объекта расчетов документа необходимо проверить правильность остатков и оборотов
	|в отчете ""Ведомость расчетов с клиентами"" или ""Ведомость расчетов с поставщиками"".'");
...
Функция ТекстНепредвиденнаяСитуацияПриОбновленииДокумента() Экспорт
	...
	Возврат НСтр("ru = 'При обновлении движений документа произошла исключительная ситуация (см. журнал регистрации). Обновление документа отменено.';
				|en = 'При обновлении движений документа произошла исключительная ситуация (см. журнал регистрации). Обновление документа отменено.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|';
	|en = 'Очищает движения ресурса ""Оплачивается"" по Заявкам на расходование средств с пустыми объектами расчетов.
	|Исправляет порядок операции движений по ресурсу ""%1""
	|Перезаполняет поле ""Порядок зачета по дате платежа"" для платежных документов.
	|Сбрасывает реквизит ""По данным объекта расчетов источника"" в случае если в движениях документа не заполнены реквизиты приемника.
	|Исправляет остаток ресурса ""%2"" при расчетах с детализациями ""Аванс по заказам, долг по накладным"", ""Аванс по договорам, долг по накладным"".
