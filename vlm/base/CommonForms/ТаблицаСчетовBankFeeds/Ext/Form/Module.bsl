...
Процедура ДобавитьБанковскийСчет(Банк, Счет, КодВалютыБуквенный, ИдентификаторСчета)
	...
	Иначе
		...
		Исключение
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось изменить расчетный счет: %1';
											|en = 'Cannot edit the current account: %1'"), РасчетныйСчет);
		...
		Если СтрокаТаблицы.Пометка Тогда
			Если СтрокаТаблицы.УжеДобавленВ1С И НЕ СтрокаТаблицы.ПодключенКСервису
				...
				НСтр("ru = 'Банковский счет ""%1"" существует в справочнике, но не подключен к сервису.
					|Подключить?';
					|en = 'The ""%1"" bank account exists in the catalog, but is not connected to the service.
					|Connect the account?'"),
					СтрокаТаблицы.Счет);
