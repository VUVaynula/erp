...
//++ НЕ УТКА
	...
	Если РеквизитПутьКДанным = ""
		...
		Элементы.ПодсказкаСтруктураЗаказаНаПроизводство0.Видимость = Не НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство;
		Элементы.ПодсказкаСтруктураЗаказаНаПроизводство1.Видимость = НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство;
...
//++ Устарело_Производство21
		...
		Иначе
//-- Устарело_Производство21
			СписокВыборка = Элементы.МетодикаУправленияПроизводством.СписокВыбора;
			СписокВыборка.Очистить();
			СписокВыборка.Добавить(Перечисления.МетодикаУправленияПроизводством.БезПланирования,
			...
			Если НЕ НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство Тогда
				СписокВыборка.Добавить(Перечисления.МетодикаУправленияПроизводством.ПланированиеПотребностиВМатериалах);
			КонецЕсли;
			СписокВыборка.Добавить(Перечисления.МетодикаУправленияПроизводством.ПланированиеПроизводственныхРесурсов,
				?(НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство,
	...
	Иначе
		...
		Иначе
			...
			Текст = НСтр("ru = 'График производства не рассчитывается. Сроки выполнения этапов не назначаются, этапы выполняются согласно очереди.';
						|en = 'Production schedule is not calculated. Stage execution dates are not assigned, stages are executed according to the order.'");
