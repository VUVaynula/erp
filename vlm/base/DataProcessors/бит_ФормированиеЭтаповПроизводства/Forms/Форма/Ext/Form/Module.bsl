...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// {{ПервыйБИТ ЛистТребований [внедрение]  [VUVaynula@1cbit.ru] [02.06.2026 21:25:33] +        
	ОписаниеКоличества = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3));  
	МассивРеквизитов = Новый Массив;   
	НовыйРеквизит = Новый РеквизитФормы("КоличествоВыделенных", ОписаниеКоличества);
	МассивРеквизитов.Добавить(НовыйРеквизит);
	ИзменитьРеквизиты(МассивРеквизитов);  
	
	НовыйЭлемент = Этаформа.Элементы.Вставить("КоличествоВыделенных",Тип("ПолеФормы"), Элементы.ГруппаДействияНоменклатурныйПлан, Элементы.Группа9);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным = "КоличествоВыделенных";
	НовыйЭлемент.Заголовок = "Общее количество";
...
Процедура ПланИтогиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	КонецЕсли;
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 11:47:14] +        
	Если СтрНайти("бит_ЗаказКлиента,бит_ЗаказНаПроизводство,бит_Тара,бит_КодОперации,бит_Назначение", ИмяПоля) > 0 Тогда
...
Процедура ПланИтогиКЗапускуОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.бит_ФормированиеЭтаповПроизводства.Форма.ВыборИзделийКЗапуску",
		ПараметрыФормы,
		ЭтаФорма,,,,
...
Процедура ПараметрыСозданияДокументовГиперссылка(Элемент)
	...
	ОткрытьФорму(
		"Обработка.бит_ФормированиеЭтаповПроизводства.Форма.ПараметрыСозданияДокументов",
...
Процедура ПоказатьНастройки(Команда)
	...
	ОткрытьФорму(
		"Обработка.бит_ФормированиеЭтаповПроизводства.Форма.НастройкиФормы",
...
Процедура ЗаполнитьНоменклатурныйПланНаСервере()
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.бит_ФормированиеЭтаповПроизводства.ПолучитьНоменклатурныйПланОтложенно",
		ПараметрыПроцедуры,
		ПараметрыВыполнения);
...
&НаКлиенте
Процедура бит_ЗаполнитьДоступныеВРЦ(мСпецификаций)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 18:45:48] +  
	бит_ДоступноеОборудование.Очистить();
	Если мСпецификаций.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	сДоступноеОборудование = бит_ВидыРабочихЦентров.ОпределитьДоступноеОборудованиеНаСервере(мСпецификаций);
	Для каждого стр из сДоступноеОборудование Цикл 
		нов = бит_ДоступноеОборудование.Добавить();
		нов.ВРЦ = стр.Ключ;
		нов.Дата = стр.Значение.Дата;
	КонецЦикла;
	СортироватьДоступноеОборудование();
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 18:45:48] -
КонецПроцедуры

&НаСервере
Процедура СортироватьДоступноеОборудование()
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [05.05.2026 12:05:55] +        
	бит_ДоступноеОборудование.Сортировать("ВРЦ");
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [05.05.2026 12:05:55] -
КонецПроцедуры
...
Процедура ОбновитьПланПриАктивизацииСтроки()
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:33:33] +        
	мСпецификаций = Новый Массив;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:33:33] -  
	
	// {{ПервыйБИТ ЛистТребований [внедрение]  [VUVaynula@1cbit.ru] [03.06.2026 1:27:37] +        
		ЭтаФорма.КоличествоВыделенных = 0;
	// }}ПервыйБИТ ЛистТребований [внедрение]  [VUVaynula@1cbit.ru] [03.06.2026 1:27:37] -
	Если Элементы.ПланИтоги.ТекущиеДанные <> Неопределено Тогда
		
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:28:53] +        
		Если Элементы.ПланИтоги.ВыделенныеСтроки.Количество() = 1 Тогда
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:28:53] -
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:29:21] +        
			мСпецификаций.Добавить(Элементы.ПланИтоги.ТекущиеДанные.Спецификация);
			
			// {{ПервыйБИТ ЛистТребований [внедрение]  [VUVaynula@1cbit.ru] [03.06.2026 1:26:07] +        
			ЭтаФорма.КоличествоВыделенных = ДанныеСтроки.КЗапуску; 
			// }}ПервыйБИТ ЛистТребований [внедрение]  [VUVaynula@1cbit.ru] [03.06.2026 1:26:07] -
		Иначе 
			Для каждого стр из Элементы.ПланИтоги.ВыделенныеСтроки  Цикл
				ТД = Объект.ПланИтоги.НайтиПоИдентификатору(стр);
				
				мСпецификаций.Добавить(ТД.Спецификация);
				//---------------------------------------------
				ДанныеСтроки = СтрокаПланИтогиВСтруктуру(ТД);
				...
				ДанныеАктивнойСтроки.МинимальнаяПартияВыпуска = ДанныеАктивнойСтроки.МинимальнаяПартияВыпуска + Сведения.МинимальнаяПартияВыпуска;
				ДанныеАктивнойСтроки.ОптимальнаяПартияВыпуска = ДанныеАктивнойСтроки.ОптимальнаяПартияВыпуска + Сведения.ОптимальнаяПартияВыпуска;
				ДанныеАктивнойСтроки.ЕстьПобочныйВыпуск = ДанныеАктивнойСтроки.ЕстьПобочныйВыпуск + Сведения.ЕстьПобочныйВыпуск;
				ДанныеАктивнойСтроки.ПобочныйВыпускКоличествоИзделий = ДанныеАктивнойСтроки.ПобочныйВыпускКоличествоИзделий + Сведения.ПобочныйВыпускКоличествоИзделий;
				ДанныеАктивнойСтроки.ПобочныйВыпускНоменклатураПредставление = ДанныеАктивнойСтроки.ПобочныйВыпускНоменклатураПредставление + Сведения.ПобочныйВыпускНоменклатураПредставление;
				
			
				ЕстьЗаданияКРасчету = Макс(ЕстьЗаданияКРасчету, Сведения.ЕстьЗаданияКРасчету);
				
				ЭтаФорма.КоличествоВыделенных = ЭтаФорма.КоличествоВыделенных + ДанныеСтроки.КЗапуску;
				//---------------------------------------------	
			КонецЦикла;
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 18:45:00] +        
	бит_ЗаполнитьДоступныеВРЦ(мСпецификаций);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 18:45:00] -
КонецПроцедуры
...
Процедура ПоместитьПланВХранилище(ДанныеСтроки, Таблица, АдресПланХранилище)
	...
	Если ДанныеСтроки.ПланВХранилище Тогда
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:57:54] +        
		//СтруктураПоиска = Новый Структура("НомерГруппы, Номенклатура, Характеристика");
		СтруктураПоиска = Новый Структура("НомерГруппы, Номенклатура, Характеристика, бит_КодОперации");
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 15:57:54] -
...
Функция СтрокаПланИтогиВСтруктуру(Строка)
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 18:56:16] +        
	Результат.Вставить("бит_ЗаказНаПроизводство", Строка.бит_ЗаказНаПроизводство);
	Результат.Вставить("бит_ЗаказКлиента", Строка.бит_ЗаказКлиента);
	Результат.Вставить("бит_Тара", Строка.бит_Тара);
	Результат.Вставить("бит_СрокЗаказаКлиента", Строка.бит_СрокЗаказаКлиента);
	Результат.Вставить("бит_КодОперации", Строка.бит_КодОперации);  
	Результат.Вставить("бит_Назначение", Строка.бит_Назначение);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 18:56:16] -	
	Возврат Результат;
...
Функция ДетальныеЗаписи(ДанныеСтроки, АдресПланХранилище, АдресПользовательскиеОтборы, ОбособитьВыпускПолуфабрикатов, ДетальныеЗаписиПакет = Неопределено)
	...
	Если ДанныеСтроки.ПланВХранилище Тогда
		...
		СтруктураПоиска = Новый Структура("НомерГруппы, Номенклатура, Характеристика");
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, ДанныеСтроки);
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:20:14] +        
		СтруктураПоиска = Новый Структура("НомерГруппы, Номенклатура, Характеристика, Назначение, ЗаказНаПроизводство, бит_КодОперации");
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, ДанныеСтроки);
		СтруктураПоиска.Назначение = ДанныеСтроки.бит_Назначение;
		СтруктураПоиска.ЗаказНаПроизводство = ДанныеСтроки.бит_ЗаказНаПроизводство;
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:20:14] -
	...
	Иначе
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:10:53] +        
		ТаблицаОтбор[0].ЗаказНаПроизводство = ДанныеСтроки.бит_ЗаказНаПроизводство;  
		ТаблицаОтбор[0].Назначение	= ДанныеСтроки.бит_Назначение;
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:10:53] -
...
Функция ДетальныеЗаписиПакетно(ТаблицаОтбор, АдресПользовательскиеОтборы, ОбособитьВыпускПолуфабрикатов)
	...
	ТекстЗапроса =
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:11:50] +        
		|	ТаблицаОтбор.ЗаказНаПроизводство     КАК ЗаказНаПроизводство,
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:09:18] +        
		|	МАКСИМУМ(ЕСТЬNULL(График.Начало, ДАТАВРЕМЯ(1,1,1)))          КАК ДатаЗапуска,
		|	МАКСИМУМ(ЕСТЬNULL(График.Окончание, ДАТАВРЕМЯ(1,1,1)))       КАК ДатаВыпуска,
		|	МАКСИМУМ(ЕСТЬNULL(ГрафикПотребления.Начало, ДАТАВРЕМЯ(1,1,1))) КАК ЖелаемаяДатаВыпуска, 
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:09:18] -
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:12:50] +        
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:13:20] +        
		|			ТаблицаОтбор.ЗаказНаПроизводство,
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:10:12] +        
		//|,
		...
		|	План.Характеристика      КАК Характеристика,
		|	План.ЗаказНаПроизводство КАК ЗаказНаПроизводство, 
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:11:00] +        
		|	План.Назначение			КАК Назначение,
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:11:00] -
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:11:44] +        
		|, План.Назначение
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:11:44] -
		|
		|ИМЕЮЩИЕ
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:12:03] +        
		|, Назначение
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:12:03] -
		|;
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	ЕСТЬNULL(РеквизитыПродукции.НомерСтроки, 10000000)                  КАК НомерСтрокиПродукция,
		...
		|
		|	ЕСТЬNULL(РеквизитыПродукции.ДатаПотребности, План.ЖелаемаяДатаВыпуска) КАК ЖелаемаяДатаВыпуска,
		|
		|	План.КлючПартия                                 КАК КлючПартия,
		|	План.КлючНоменклатура                           КАК КлючНоменклатура,
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:14:24] +        
		|			И План.Назначение = ВТПродукцияЗаказа.Назначение
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:14:24] -
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК РеквизитыПродукции
		|		ПО План.ЗаказНаПроизводство = РеквизитыПродукции.Ссылка
		|			И План.КлючНоменклатура = РеквизитыПродукции.КлючНоменклатура
		|			И ЕСТЬNULL(ВТПродукцияЗаказа.ПродукцияЗаказа, ЛОЖЬ)
		|
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:04:55] +        
		|			И План.ЗаказНаПроизводство = ТаблицаОтбор.ЗаказНаПроизводство
		|			И План.Назначение = ТаблицаОтбор.Назначение
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:04:55] -
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 21:22:48] +
	Таблица.Колонки.Добавить("бит_ВРЦ", Новый ОписаниеТипов("СправочникСсылка.ВидыРабочихЦентров"));
	//Таблица.Колонки.Добавить("бит_УИДСтроки", Новый ОписаниеТипов("уникальныйидентификатор"));	
	Таблица.Колонки.Добавить("бит_ОтделеннаяСтрока", Новый ОписаниеТипов("Булево"));
	Таблица.Колонки.Добавить("бит_МинДатаНачалаПланирования", Новый ОписаниеТипов("Дата", , , , , Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	Таблица.Колонки.Добавить("бит_МинДатаНачалаЗавершения", Новый ОписаниеТипов("Дата", , , , , Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	Таблица.Колонки.Добавить("бит_ВсегоЧасов", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(10, 2)));
	Таблица.Колонки.Добавить("бит_КодОперации", Новый ОписаниеТипов("СправочникСсылка.бит_КодыОпераций"));
	Для каждого стр из Таблица Цикл
		Если ЗначениеЗаполнено(стр.Спецификация) Тогда
			стр.бит_ВРЦ = бит_ВидыРабочихЦентров.ПолучитьОсновноеОборудованиеПоСпецификации(стр.Спецификация); 
			стр.бит_КодОперации = бит_ВидыРабочихЦентров.ПолучитьКодОперацииПоСпецификации(стр.Спецификация);
			//стр.бит_УИДСтроки = новый УникальныйИдентификатор;
			РассчитатьВремяСтроки(стр);	
		КонецЕсли;
	КонецЦикла;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 21:22:48] -
	Возврат Таблица;
...
&НаСервереБезКонтекста
Процедура РассчитатьВремяСтроки(стр) 
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 16:44:50] +  
	Если ЗначениеЗаполнено(стр.бит_ВРЦ) Тогда 
		МассивВРЦ = Новый Массив;
		МассивВРЦ.Добавить(стр.бит_ВРЦ);
		соответствие = бит_ВидыРабочихЦентров.ПолучитьМинимальныеДатыДоступностиВРЦ(МассивВРЦ);  
		
		МаксимальнаяДоступностьСтанкаВДеньВМинутах = ПеревестиВремяВМинуты(стр.бит_ВРЦ.МаксимальнаяДоступностьРЦ, стр.бит_ВРЦ.ЕдиницаИзмеренияДоступностиРЦ); 
		
		ДлительностьВыполненияРаботыВМинутахНаЕдиницуВеса = ОпределитьДлительностьПоСпецификацииВМинутах(стр.Спецификация, стр.бит_КодОперации);
		ДлительностьВыполненияРаботыВМинутах = ДлительностьВыполненияРаботыВМинутахНаЕдиницуВеса * стр.КЗапуску;
		стр.бит_ВсегоЧасов = ДлительностьВыполненияРаботыВМинутах/60;
		
		СтруктураДанных = соответствие.Получить(стр.бит_ВРЦ);
		Если СтруктураДанных <> Неопределено Тогда
			
			СвободноМинутВМинимальнуюДату = ?(СтруктураДанных.ДоступноМинут = Неопределено, 0, СтруктураДанных.ДоступноМинут); 
			стр.бит_МинДатаНачалаПланирования = СтруктураДанных.Дата;  
			
			Если ДлительностьВыполненияРаботыВМинутах = 0 
				Или (СвободноМинутВМинимальнуюДату > 0 И ДлительностьВыполненияРаботыВМинутах <= СвободноМинутВМинимальнуюДату) Тогда
				стр.бит_МинДатаНачалаЗавершения = стр.бит_МинДатаНачалаПланирования + ДлительностьВыполненияРаботыВМинутах * 60;
			ИначеЕсли МаксимальнаяДоступностьСтанкаВДеньВМинутах = 0 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Для " + стр.бит_ВРЦ + " не указано время доступности в день", стр.бит_ВРЦ);
				стр.бит_МинДатаНачалаЗавершения = Дата("00010101");
			Иначе
				Остаток = ДлительностьВыполненияРаботыВМинутах - СвободноМинутВМинимальнуюДату;

				ПолныхДней = Цел(Остаток / МаксимальнаяДоступностьСтанкаВДеньВМинутах);
				ОстатокМинут = Остаток % МаксимальнаяДоступностьСтанкаВДеньВМинутах;

				НачалоСледующегоДня = НачалоДня(стр.бит_МинДатаНачалаПланирования) + 86400;

				Если ОстатокМинут = 0 Тогда
					стр.бит_МинДатаНачалаЗавершения = НачалоСледующегоДня + (ПолныхДней - 1) * 86400 + МаксимальнаяДоступностьСтанкаВДеньВМинутах * 60;
				Иначе
					стр.бит_МинДатаНачалаЗавершения = НачалоСледующегоДня + ПолныхДней * 86400 + ОстатокМинут * 60;
				КонецЕсли; 
				
				Если СвободноМинутВМинимальнуюДату = 0 Тогда
					стр.бит_МинДатаНачалаЗавершения = стр.бит_МинДатаНачалаЗавершения - 86400;
				КонецЕсли;
				
			КонецЕсли;
			
		Иначе
			стр.бит_МинДатаНачалаПланирования = Дата("00010101");
			стр.бит_МинДатаНачалаЗавершения = Дата("00010101");
		КонецЕсли;
...
Функция НовыйОтборДетальныеЗаписи()
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:10:03] +        
	ТаблицаОтбор.Колонки.Добавить("ЗаказНаПроизводство", Новый ОписаниеТипов("ДокументСсылка.ЗаказНаПроизводство2_2"));
	ТаблицаОтбор.Колонки.Добавить("Назначение", Новый ОписаниеТипов("СправочникСсылка.Назначения"));
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 17:10:03] -
...
&НаСервереБезКонтекста
Функция ОпределитьДлительностьПоСпецификацииВМинутах(знач Спецификация, знач бит_КодОперации)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:54:23] +        
	Запрос = новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	ТехнологическиеОперации.бит_КодОперации = &бит_КодОперации
	                      |	И ЭтапыПроизводства.Владелец = &РесурснаяСпецификация");
	Запрос.УстановитьПараметр("РесурснаяСпецификация", Спецификация);
	Запрос.УстановитьПараметр("бит_КодОперации", бит_КодОперации);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда 
		Результат = ПеревестиВремяВМинуты(Выборка.ВремяШтучное, Выборка.ВремяШтучноеЕдИзм);
	Иначе           
		Результат = 0;
	КонецЕсли;
	ВремяНаКилограм = Результат / Спецификация.ОсновноеИзделиеКоличествоУпаковок;
	
	Возврат ВремяНаКилограм;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 0:54:23] -
КонецФункции

&НаСервереБезКонтекста
Функция ПеревестиВремяВМинуты(Знач ВремяШтучное, Знач ВремяШтучноеЕдИзм)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [06.05.2026 0:16:29] +        
	Если ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.Секунда Тогда
		Коэффициент = 1/60;
	ИначеЕсли ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.Минута Тогда
		Коэффициент = 1;
	ИначеЕсли ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.Час Тогда
		Коэффициент = 60;
	ИначеЕсли ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.День 
		Или ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.Сутки Тогда
		Коэффициент = 60 * 24;
	ИначеЕсли ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.Месяц Тогда
		Коэффициент = 60 * 24 * 30;
	ИначеЕсли ВремяШтучноеЕдИзм = Перечисления.ЕдиницыИзмеренияВремени.Год Тогда
		Коэффициент = 60 * 24 * 365;
	КонецЕсли;

	Возврат Коэффициент * ВремяШтучное;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [06.05.2026 0:16:29] -	
КонецФункции
	...
	Для Индекс = 0 По Данные.Изделия.Количество() - 1 Цикл
		...
		Если НомерГруппы <> СтрокаИзделия.НомерГруппы
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрокаИтог, СтрокаГруппы,
				"НачатьНеРанее, ДатаПотребности, Спецификация, Организация, Партнер, Договор, НаправлениеДеятельности, ПодразделениеДиспетчер, ЗаказПодДеятельность, ХозяйственнаяОперация");
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:12:13] +        
		НоваяСтрокаИтог.бит_ВРЦ = СтрокаГруппы.бит_ВРЦ;
		НоваяСтрока.бит_ВРЦ = СтрокаИзделия.бит_ВРЦ;
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:12:13] -
...
Процедура СоздатьДокументыНаСервере(Отказ)
	...
	ПараметрыПроцедуры.Вставить("ДатыОбеспеченияПоГрафику", Данные.ДатыОбеспеченияПоГрафику);
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.бит_ФормированиеЭтаповПроизводства.СоздатьДокументыОтложенно",
		ПараметрыПроцедуры,
		ПараметрыВыполнения);
...
Процедура УдалитьСозданныеОбъекты(Отказ)
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.бит_ФормированиеЭтаповПроизводства.УдалитьОбъектыОтложенно",
		ПараметрыПроцедуры,
		ПараметрыВыполнения);
...
//&НаСервере
//Функция ПроверитьСоответствиеСпецификацияВРЦ(ТекСтрока) 
//	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:52:09] +        
//	Запрос = Новый Запрос("ВЫБРАТЬ
...
//	                      |	И РесурсныеСпецификации.Родитель = &Родитель
//	                      |;
//	                      |
//	                      |////////////////////////////////////////////////////////////////////////////////
//	                      |ВЫБРАТЬ ПЕРВЫЕ 1
//	                      |	втРесСпец.РесурснаяСпецификация КАК РесурснаяСпецификация,
//	                      |	втРесСпец.НачалоДействия КАК НачалоДействия
//	                      |ИЗ
//	                      |	Справочник.ЭтапыПроизводства.ВидыРабочихЦентров КАК ЭтапыПроизводстваВидыРабочихЦентров
//	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втРесСпец КАК втРесСпец
//	                      |		ПО (втРесСпец.РесурснаяСпецификация = ЭтапыПроизводстваВидыРабочихЦентров.Ссылка.Владелец)
//	                      |ГДЕ
//	                      |	ЭтапыПроизводстваВидыРабочихЦентров.Ссылка.Владелец.Родитель = &Родитель
//	                      |	И ЭтапыПроизводстваВидыРабочихЦентров.ВидРабочегоЦентра = &ВидРабочегоЦентра
//	                      |	И ЭтапыПроизводстваВидыРабочихЦентров.Ссылка.Владелец.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСпецификаций.Действует)
//	                      |
//	                      |УПОРЯДОЧИТЬ ПО
//	                      |	НачалоДействия УБЫВ");
//	Запрос.УстановитьПараметр("Родитель"			, ТекСтрока.Спецификация.Родитель);
//	Запрос.УстановитьПараметр("ВидРабочегоЦентра"	, ТекСтрока.бит_ВРЦ); 
//	Запрос.УстановитьПараметр("Номенклатура"		, ТекСтрока.Номенклатура);
//	
//	Выборка = Запрос.Выполнить().Выбрать();
//	ТекСпец = Неопределено;
//	Если Выборка.Следующий() Тогда 
//		ТекСпец = Выборка.РесурснаяСпецификация;
//	КонецЕсли;
//	Если ЗначениеЗаполнено(ТекСпец) И ТекСтрока.Спецификация <> ТекСпец Тогда
//		ТекСтрока.Спецификация = ТекСпец;
//	КонецЕсли;
//	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:52:09] -
//КонецФункции
...
Функция ДанныеКФормированию()
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:49:36] +        
	// проверим таблицу. 
	//Для каждого стр из Изделия Цикл
	//	ПроверитьСоответствиеСпецификацияВРЦ(стр);
	//КонецЦикла;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:49:36] -
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:40:32] +        
	Если Группы.Колонки.Найти("бит_ВРЦ") = Неопределено Тогда
    	Группы.Колонки.Добавить("бит_ВРЦ",Новый ОписаниеТипов("СправочникСсылка.ВидыРабочихЦентров"));
	КонецЕсли;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:40:32] -	
	НомерГруппыСледующий = 100000000;
	
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36] + 
	// До типовых перегруппировок физически разделяем старые группы по ВРЦ.
	НомерГруппыСледующий = бит_РазбитьГруппыПоВРЦ(
	Группы,
	Изделия,
	НомерГруппыСледующий
	);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36] -
	...
	Если СтрокиКОбработке.ВГраница() <> -1 Тогда
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:39:33] +        
		//ВременныйИндексИзделия = Изделия.Индексы.Добавить("НомерГруппы, Номенклатура, Характеристика");
		ВременныйИндексИзделия = Изделия.Индексы.Добавить("НомерГруппы, Номенклатура, Характеристика, бит_ВРЦ");
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:39:33] -
		
		СтруктураПоискаИзделия = Новый Структура("НомерГруппы, Номенклатура, Характеристика, бит_ВРЦ");
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:36:39] +        
	//СтруктураПоискаНоменклатура = Новый Структура("НомерГруппы, Номенклатура, Характеристика");  
	СтруктураПоискаНоменклатура = Новый Структура("НомерГруппы, Номенклатура, Характеристика, бит_ВРЦ");  
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:36:39] -
	СтруктураПоискаНовыйНомер = Новый Структура("НомерГруппы, Заказ, Назначение, НазначениеПродукция");
	
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:37:54] +        
	//ВременныйИндексГруппы = Группы.Индексы.Добавить("НомерГруппы, Номенклатура, Характеристика");
	ВременныйИндексГруппы = Группы.Индексы.Добавить("НомерГруппы, Номенклатура, Характеристика, бит_ВРЦ");
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:37:54] -
	...
	Для каждого Изделие Из Изделия Цикл
		...
		ИначеЕсли СтрокаЗаказы.Заказ <> Заказ
			...
			Если НайденныеСтроки.ВГраница() = -1 Тогда
				...
				// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:41:30] +        
				НоваяГруппа.бит_ВРЦ = Изделие.бит_ВРЦ;
				// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:41:30] -
			Иначе
			...
			НоваяГруппа.Требуется = НоваяГруппа.Требуется + Изделие.Требуется;
			
			Группа.КЗапуску = Группа.КЗапуску - Изделие.КЗапуску;     
			Группа.Требуется = Группа.Требуется - Изделие.Требуется;     
	...
	Если СтрокиКОбработке.ВГраница() <> -1 Тогда
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 23:41:24] +        
		СтруктураПоискаГруппы.Вставить("бит_ВРЦ");
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 23:41:24] -
		...
		ВременныйИндексГруппы = Группы.Индексы.Добавить(
			"Перегруппировать, ОсновноеИзделие, Спецификация, ЗаказНаПроизводствоГруппировка, бит_ВРЦ");
		...
		Для каждого Строка Из СтрокиКОбработке Цикл
			...
			// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 23:43:04] +        
			//ЗаполнитьЗначенияСвойств(СтруктураПоискаГруппы, Строка, "Спецификация, ЗаказНаПроизводствоГруппировка");
			ЗаполнитьЗначенияСвойств(СтруктураПоискаГруппы, Строка, "Спецификация, ЗаказНаПроизводствоГруппировка, бит_ВРЦ");
			// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 23:43:04] -
	...
	Запрос.Текст = 
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:42:46] +        
		|	, Группы.бит_ВРЦ КАК бит_ВРЦ
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:42:46] -
		|ПОМЕСТИТЬ Группы
		|ИЗ
		|	&Группы КАК Группы
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:43:21] +        
		|	Изделия.бит_ВРЦ             	 КАК бит_ВРЦ,  
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:43:21] -
		|	ВЫБОР
	...
	ТекстЗапроса =
		...
		|			ТОГДА МИНИМУМ(ISNULL(НормативныеДатыЗапускаВыпускаОтТекущейДаты.ДатаЗапуска, Изделия.ДатаЗапуска))
		|		КОГДА МИНИМУМ(Изделия.ДатаЗапуска) <= &ТекущаяДатаОкончаниеДня
		|				ИЛИ &РассчитатьГрафикОтТекущейДаты
		|			ТОГДА &ТекущаяДата
		|		ИНАЧЕ МИНИМУМ(Изделия.ДатаЗапуска)
		|	КОНЕЦ                                                  КАК НачатьНеРанее,
		|	МИНИМУМ(
		|		ISNULL(НормативныеДатыЗапускаВыпускаОтТекущейДаты.ДатаВыпуска,
		|		Изделия.ЖелаемаяДатаВыпуска))                      КАК ДатаПотребности,
		...
		|	Группы.ЗаказПодДеятельность    КАК ЗаказПодДеятельность,
		|	Группы.ХозяйственнаяОперация   КАК ХозяйственнаяОперация,   
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:10:17] +        
		|	Группы.бит_ВРЦ                 КАК бит_ВРЦ,
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:10:17] -
		...
		|	Изделия.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
		|	Изделия.КоличествоНаПартию       КАК КоличествоНаПартию,  
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:10:48] +        
		|	Изделия.бит_ВРЦ                  КАК бит_ВРЦ,
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:10:48] -
		|	
		|	Изделия.Получатель               КАК Получатель,
		...
		|	ЕСТЬNULL(ПараметрыЗапуска.НачатьНеРанее, &ТекущаяДата) КАК ДатаЗапуска,
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:11:11] +        
		|	Изделия.бит_ВРЦ,
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [30.04.2026 12:11:11] -
		...
		|	ЕСТЬNULL(ПараметрыЗапуска.НачатьНеРанее, &ТекущаяДата),
		|	ЕСТЬNULL(ОчередьГрупп.Порядок, 1000000)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЕСТЬNULL(ОчередьГрупп.Порядок, 1000000),
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:44:32] +        
		|	Изделия.бит_ВРЦ КАК бит_ВРЦ,
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:44:32] -
		|	МАКСИМУМ(Обеспечение.ДатаОбеспечения) КАК Дата
		|ИЗ
		|	Изделия КАК Изделия
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОграниченияГрафикаПроизводстваПоМатериалам КАК Обеспечение
		|	ПО Изделия.КлючПартия = Обеспечение.КлючПартия
		|
		|СГРУППИРОВАТЬ ПО
		|	Изделия.НомерГруппы,
		|	Обеспечение.Этап
		// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:45:06] +        
		| 	, Изделия.бит_ВРЦ
		// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 21:45:06] -
		|";
	...
	ДатыОбеспеченияПоГрафику = РезультатЗапроса[КоличествоПакетов-2].Выгрузить();
	...
	Результат.Вставить("ДатыОбеспеченияПоГрафику", ДатыОбеспеченияПоГрафику);
...
&НаСервере
Функция бит_РазбитьГруппыПоВРЦ(Группы, Изделия, НомерГруппыСледующий)
// {{ПервыйБИТ ЛистТребований [ЛТ5.36] + 
// Разделяет старые группы по бит_ВРЦ, если в одной группе несколько разных ВРЦ.
	//
	//	Если Группы.Колонки.Найти("бит_ВРЦ") = Неопределено Тогда
	//	Группы.Колонки.Добавить(
	//		"бит_ВРЦ",
	//		Новый ОписаниеТипов("СправочникСсылка.ВидыРабочихЦентров")
	//	);
	//КонецЕсли;
	//
	//Если Изделия.Колонки.Найти("бит_ВРЦ") = Неопределено Тогда
	//	Возврат НомерГруппыСледующий;
	//КонецЕсли;
	
	ВременныйИндексИзделия = Изделия.Индексы.Добавить("НомерГруппы");
	СтруктураПоискаИзделия = Новый Структура("НомерГруппы");
	
	// Обходим копию массива строк, потому что в Группы будем добавлять новые строки.
	СтарыеГруппы = Новый Массив;
	Для каждого СтрокаГруппы Из Группы Цикл
		СтарыеГруппы.Добавить(СтрокаГруппы);
	КонецЦикла;
	
	Для каждого СтрокаГруппы Из СтарыеГруппы Цикл
		
		СтарыйНомерГруппы = СтрокаГруппы.НомерГруппы;
		
		СтруктураПоискаИзделия.НомерГруппы = СтарыйНомерГруппы;
		СтрокиИзделия = Изделия.НайтиСтроки(СтруктураПоискаИзделия);
		...
		ВРЦПоГруппе = Новый ТаблицаЗначений;
		ВРЦПоГруппе.Колонки.Добавить("бит_ВРЦ", Новый ОписаниеТипов("СправочникСсылка.ВидыРабочихЦентров"));
		ВРЦПоГруппе.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		ВРЦПоГруппе.Колонки.Добавить("Спецификация", Новый ОписаниеТипов("СправочникСсылка.РесурсныеСпецификации"));
		ВРЦПоГруппе.Колонки.Добавить("КЗапуску", Новый ОписаниеТипов("Число"));
		ВРЦПоГруппе.Колонки.Добавить("Требуется", Новый ОписаниеТипов("Число"));
		ВРЦПоГруппе.Колонки.Добавить("НомерГруппыНовый", Новый ОписаниеТипов("Число"));
		ВРЦПоГруппе.Индексы.Добавить("бит_ВРЦ");
		
		// Собираем все разные ВРЦ внутри одной старой группы.
		Для каждого СтрокаИзделия Из СтрокиИзделия Цикл
			
			СтрокаВРЦ = ВРЦПоГруппе.НайтиСтроки(Новый Структура("бит_ВРЦ, Номенклатура, Спецификация", СтрокаИзделия.бит_ВРЦ, СтрокаИзделия.Номенклатура, СтрокаИзделия.Спецификация));
			
			Если СтрокаВРЦ.Количество() = 0 Тогда
				СтрокаВРЦ = ВРЦПоГруппе.Добавить();
				СтрокаВРЦ.бит_ВРЦ = СтрокаИзделия.бит_ВРЦ;
				СтрокаВРЦ.Номенклатура = СтрокаИзделия.Номенклатура;
				СтрокаВРЦ.Спецификация = СтрокаИзделия.Спецификация;
				
				СтрокаВРЦ.КЗапуску = 0;
				СтрокаВРЦ.Требуется = 0;
				СтрокаВРЦ.НомерГруппыНовый = 0;
			КонецЕсли;
			
			Если ТипЗнч(СтрокаВРЦ) = Тип("Массив") Тогда
				СтрокаВРЦ = СтрокаВРЦ[0];
			КонецЕсли;
			
			СтрокаВРЦ.КЗапуску = СтрокаВРЦ.КЗапуску + СтрокаИзделия.КЗапуску;
			СтрокаВРЦ.Требуется = СтрокаВРЦ.Требуется + СтрокаИзделия.Требуется;
			
		КонецЦикла;
		
		// Если ВРЦ один — просто записываем его в группу.
		Если ВРЦПоГруппе.Количество() = 1 Тогда
			СтрокаГруппы.бит_ВРЦ = ВРЦПоГруппе[0].бит_ВРЦ;
			Продолжить;
		КонецЕсли;
		
		// Первый ВРЦ оставляем в старой группе.
		Для ИндексВРЦ = 0 По ВРЦПоГруппе.Количество() - 1 Цикл
			
			СтрокаВРЦ = ВРЦПоГруппе[ИндексВРЦ];
			
			Если ИндексВРЦ = 0 Тогда
				
				СтрокаВРЦ.НомерГруппыНовый = СтарыйНомерГруппы;
				
				СтрокаГруппы.бит_ВРЦ = СтрокаВРЦ.бит_ВРЦ;
				СтрокаГруппы.КЗапуску = СтрокаВРЦ.КЗапуску;
				СтрокаГруппы.Требуется = СтрокаВРЦ.Требуется;
				
			Иначе
				
				СтрокаВРЦ.НомерГруппыНовый = НомерГруппыСледующий;
				НомерГруппыСледующий = НомерГруппыСледующий + 1;
				
				НоваяГруппа = Группы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяГруппа, СтрокаГруппы);
				
				НоваяГруппа.НомерГруппы = СтрокаВРЦ.НомерГруппыНовый;
				НоваяГруппа.Спецификация = СтрокаВРЦ.Спецификация;
				НоваяГруппа.бит_ВРЦ = СтрокаВРЦ.бит_ВРЦ;
				НоваяГруппа.КЗапуску = СтрокаВРЦ.КЗапуску;
				НоваяГруппа.Требуется = СтрокаВРЦ.Требуется;
				
			КонецЕсли;
			
		КонецЦикла;
		
		// Переназначаем изделия на новые номера групп по их ВРЦ.
		Для каждого СтрокаИзделия Из СтрокиИзделия Цикл
			
			СтрокаВРЦ = ВРЦПоГруппе.Найти(СтрокаИзделия.бит_ВРЦ, "бит_ВРЦ");
			
			Если СтрокаВРЦ <> Неопределено Тогда
				СтрокаИзделия.НомерГруппы = СтрокаВРЦ.НомерГруппыНовый;
			КонецЕсли;
...
Процедура ВосстановитьНастройки()
	...
	Если ПараметрыСозданияДокументов = Неопределено Тогда
		ПараметрыСозданияДокументов = Обработки.бит_ФормированиеЭтаповПроизводства.ПараметрыСозданияДокументовПоУмолчанию();
	Иначе
		
		ПараметрыПоУмолчанию = Обработки.бит_ФормированиеЭтаповПроизводства.ПараметрыСозданияДокументовПоУмолчанию();
...
Процедура бит_ЗаменитьОборудование(Команда) 
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [27.04.2026 16:21:58] +        
	Режим = "ЗаменаОборудования";
	ОткрытьДопФорму(Режим);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [27.04.2026 16:21:58] -
КонецПроцедуры   

&НаКлиенте
Функция СтрокаПланВСтруктуру(ТД, Режим = Неопределено)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [26.04.2026 19:47:33] +        
	Структура = Новый Структура;
	Структура.Вставить("Спецификация", ТД.Спецификация);
	Структура.Вставить("Номенклатура", ТД.Номенклатура);
	Структура.Вставить("Характеристика", ТД.Характеристика);
	Структура.Вставить("Количество", ТД.КЗапуску);
	Структура.Вставить("бит_ВРЦ", ТД.бит_ВРЦ);       
	Структура.Вставить("КодОперации", ТД.бит_КодОперации);
	Структура.Вставить("Назначение", ТД.Назначение); 
	Структура.Вставить("ЗаказНаПроизводство", ТД.ЗаказНаПроизводство); 
	
	мВРЦ = Новый Массив;
	Для каждого стр из бит_ДоступноеОборудование Цикл
		мВРЦ.Добавить(стр.ВРЦ);
	КонецЦикла;
	Структура.Вставить("мВРЦ", мВРЦ);                
...
&НаКлиенте
Процедура бит_РазделитьПотребность(Команда)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [27.04.2026 19:12:28] +        
	Режим = "РазделениеПотребности";
	ОткрытьДопФорму(Режим);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [27.04.2026 19:12:28] -
КонецПроцедуры             

&НаКлиенте
Процедура ОткрытьДопФорму(Режим) 
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 2:46:16] +        
	ТД = Элементы.План.ТекущиеДанные;
	Если ТД <> Неопределено Тогда
		Если ТД.Отметка = Ложь Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Сначала необходимо отметить указать количество к запуску");
		Иначе
				
			ДанныеСтрокиПлан = СтрокаПланВСтруктуру(ТД, Режим);
			ОткрытьФорму("Обработка.бит_ФормированиеЭтаповПроизводства.Форма.бит_ФормаЗаменыОборудования", 
							Новый Структура("ДанныеСтроки", ДанныеСтрокиПлан), 
							ЭтаФорма, 
							УникальныйИдентификатор, 
							, 
							, 
							Новый ОписаниеОповещения("ПослеЗаменыОборудования", ЭтотОбъект, Новый Структура("Режим, ТД", Режим, ТД)), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;	  
	КонецЕсли;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 2:46:16] -
КонецПроцедуры                                                           
...
&НаКлиенте
Процедура ПослеЗаменыОборудования(ДанныеСтроки, ДопПараметры) Экспорт
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 2:55:02] +        
	Если ДанныеСтроки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТД = ДопПараметры.ТД;
	Если ДопПараметры.Режим = "РазделениеПотребности" Тогда
		нов = Объект.План.Добавить();
		ЗаполнитьЗначенияСвойств(нов, ТД);
		нов.бит_ОтделеннаяСтрока = Истина;
		нов.бит_ВРЦ = ДанныеСтроки.ВРЦ;
		нов.КЗапуску = ДанныеСтроки.ОтделяемоеКоличество;
		нов.Требуется = ДанныеСтроки.ОтделяемоеКоличество;
		
		нов.Спецификация = ДанныеСтроки.Спецификация;
		
		ТД.КЗапуску = ТД.КЗапуску - ДанныеСтроки.ОтделяемоеКоличество; 
		ТД.Требуется = ТД.КЗапуску;

	ИначеЕсли ДопПараметры.Режим = "ЗаменаОборудования" Тогда 
		ТД.бит_ВРЦ = ДанныеСтроки.ВРЦ;               
		ТД.Спецификация = ДанныеСтроки.Спецификация;
	КонецЕсли;  
	
	ПересчитатьДатыСроки();
	Для каждого стр из Элементы.ПланИтоги.ВыделенныеСтроки Цикл
		ТД = Объект.ПланИтоги.НайтиПоИдентификатору(стр);
		ТД.ПланВХранилище = Истина;
	КонецЦикла;	         
	
	ПослемМанипуляцийНадПланамиСохранитьВХранилище();
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 2:55:02] -
КонецПроцедуры            

&НаСервере
Процедура ПоместитьПланыВХранилище(СтруктураПоиска)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 18:35:11] +   
	Таблица = ПолучитьИзВременногоХранилища(АдресПланХранилище);
	
	СтрокиПоКлючу = Таблица.НайтиСтроки(СтруктураПоиска);
	...
	ПоместитьВоВременноеХранилище(Таблица, АдресПланХранилище);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 18:35:11] -
КонецПроцедуры

&НаСервере
Функция ПересчитатьДатыСроки()
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 16:01:28] +        
	Для каждого стр из Объект.План Цикл
		РассчитатьВремяСтроки(стр);		
	КонецЦикла;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [29.04.2026 16:01:28] -
...
&НаКлиенте
Процедура бит_ОтменитьОтделение(Команда)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [28.04.2026 2:55:38] +        
	ТД = Элементы.План.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат;
	КонецЕсли;             
	Если ТД.бит_ОтделеннаяСтрока = Ложь Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Можно отменить разделение только для отмененной строки");	
		Возврат;
	КонецЕсли;              

	СтруктураОтбора = Новый Структура;
	//СтруктураОтбора.Вставить("Спецификация", ТД.Спецификация);
	СтруктураОтбора.Вставить("Номенклатура", ТД.Номенклатура);
	СтруктураОтбора.Вставить("Характеристика", ТД.Характеристика);
	СтруктураОтбора.Вставить("Назначение", ТД.Назначение); 
	СтруктураОтбора.Вставить("ЗаказНаПроизводство", ТД.ЗаказНаПроизводство); 
	СтруктураОтбора.Вставить("бит_ОтделеннаяСтрока", Ложь);
	СтруктураОтбора.Вставить("бит_КодОперации", ТД.бит_КодОперации);

    СтрокаПоКлючу = Объект.План.НайтиСтроки(СтруктураОтбора);
	
	Если СтрокаПоКлючу.Количество() > 0 Тогда
		СтрокаПоКлючу[0].КЗапуску = СтрокаПоКлючу[0].КЗапуску + ТД.КЗапуску;
	КонецЕсли;
	Объект.План.Удалить(ТД); 
...
&НаКлиенте
Процедура ПослемМанипуляцийНадПланамиСохранитьВХранилище()
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 15:35:57] +        
	ТД = Элементы.ПланИтоги.ТекущиеДанные;
	ДанныеСтроки = СтрокаПланИтогиВСтруктуру(ТД);
	СтруктураПоиска = Новый Структура("НомерГруппы, Номенклатура, Характеристика, Назначение, ЗаказНаПроизводство, бит_КодОперации");
	ЗаполнитьЗначенияСвойств(СтруктураПоиска, ДанныеСтроки);
	СтруктураПоиска.Назначение = ДанныеСтроки.бит_Назначение;
	...
	ПоместитьПланыВХранилище(СтруктураПоиска);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 15:35:57] -
КонецПроцедуры
	
//&НаСервере
//Функция ОтобратьСтроки(СтруктураОтбора)
//	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [06.05.2026 15:28:56] +        
//	
//    // }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [06.05.2026 15:28:56] -
//КонецФункции

&НаКлиенте
Процедура бит_ЗапуститьРасчетГрафиков(Команда)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:40:54] +        
	//// 1) запуск с клиента фонового задания на сервере
	//Результат = ЗапуститьРасчетГрафиковНаСервере();
	//
	//// 2) на клиенте подключение обработчика завершения фонового задания
	//бит_ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект); 
	//бит_ПараметрыОжидания.ВыводитьОкноОжидания = Истина;
	//бит_ОповещениеРасчет = Новый ОписаниеОповещения("бит_ПриЗавершенииРасчета", ЭтотОбъект);
	//ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, бит_ОповещениеРасчет, бит_ПараметрыОжидания);
	...
	ДлительныеОперацииКлиент.ОжидатьЗавершение(СтруктураФоновогоЗадания, Новый ОписаниеОповещения("ПослеРасчетаГрафиковПроизводства", ЭтотОбъект), ПараметрыОжидания);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:40:54] -
КонецПроцедуры

//&НаСервере
//Функция ЗапуститьРасчетГрафиковНаСервере()
//    // {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:28:24] +
//	бит_ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(ЭтаФорма.УникальныйИдентификатор);              
//	
//	Возврат ДлительныеОперации.ВыполнитьФункцию(бит_ПараметрыВыполнения, "ПланированиеПроизводства.РасчетГрафикаПроизводства"); 
//    // }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:28:24] -
//КонецФункции 
//			 
//&НаКлиенте
//Процедура бит_ПриЗавершенииРасчета(РезультатИзВремХран, расш_ДопПараметры) Экспорт
//    // {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:39:32] +        
...
//    
//    ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Расчет завершен");      
//    // }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:39:32] -
//КонецПроцедуры

&НаКлиенте
Процедура ПослеРасчетаГрафиковПроизводства(Результат, ДополнительныеПараметры) Экспорт
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:44:26] +        
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Расчет завершен");
	// }}ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:44:26] -
КонецПроцедуры

&НаСервере
Функция ВыполнитьФоновоеЗаданиеНаСервере(УникальныйИдентификатор)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.36]  [VUVaynula@1cbit.ru] [07.05.2026 14:44:38] +        
	НаименованиеЗадания = "Расчет графика производства";
	ВыполняемыйМетод = "ПланированиеПроизводства.РасчетГрафикаПроизводства";
