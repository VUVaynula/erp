...
Процедура РезервироватьНаСкладе(Команда)
	...
	// {{ПервыйБИТ ЛистТребований [5.36]  [VUVaynula@1cbit.ru] [10.08.2025 17:59:23] +        
	Элементы.группаКПРезервировать.Скрыть();
	// }}ПервыйБИТ ЛистТребований [5.36]  [VUVaynula@1cbit.ru] [10.08.2025 17:59:23] -
КонецПроцедуры
...
Процедура СохранитьНастройки()
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:29:59] +        
	Настройки.Вставить("бит_Исполнение"	, бит_Исполнение);
	Настройки.Вставить("бит_Марка"		, бит_Марка);
	Настройки.Вставить("бит_Сечение"	, бит_Сечение);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:29:59] -
	ЗначениеНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Обработка.бит_СостояниеОбеспеченияЗаказов.Форма", "Основная");
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.бит_СостояниеОбеспеченияЗаказов.Форма", "Основная", Настройки);
...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(ЗначениеНастроек) = Тип("Структура") Тогда
		...
		КонецЕсли;
		// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:32:08] +        
		Если ЗначениеНастроек.Свойство("бит_Исполнение") Тогда
			бит_Исполнение = ЗначениеНастроек.бит_Исполнение;
		КонецЕсли; 
		Если ЗначениеНастроек.Свойство("бит_Марка") Тогда
			бит_Марка = ЗначениеНастроек.бит_Марка;
		КонецЕсли;
		Если ЗначениеНастроек.Свойство("бит_Сечение") Тогда
			бит_Сечение = ЗначениеНастроек.бит_Сечение;
		КонецЕсли;
		// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:32:08] -
...
Процедура УправлениеПараметрамиСписков(ИмяЭлемента, ДополнительныеПараметры = Неопределено)
	...
	Если ИмяЭлемента = Неопределено Тогда
		...
		Если КонтекстИспользования = "РАЗДЕЛ" Тогда
			...
			// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 15:40:24] +        
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокЗаказы, "ПризнакИсполнение", ПолучитьЗначениеПоУмолчанию("ПризнакВидИсполненияГотовойПродукции"), Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокЗаказы, "ПризнакСечение", ПолучитьЗначениеПоУмолчанию("ПризнакРазмерГотовойПродукции"), Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокЗаказы, "ПризнакМарка", ПолучитьЗначениеПоУмолчанию("ПризнакMARKA"), Истина);	
			// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 15:40:24] -
		КонецЕсли;
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:41:52] +      

	Если ИмяЭлемента = Неопределено 
		Или ИмяЭлемента = Элементы.бит_Исполнение.Имя 
		Или ИмяЭлемента = Элементы.бит_Сечение.Имя 
		Или ИмяЭлемента = Элементы.бит_Марка.Имя Тогда
		
		Если КонтекстИспользования = "РАЗДЕЛ" Тогда   
			
			НоменклатураОтбора = Новый Массив;
	        ЕстьОтбор = Ложь;
			ПолучитьНоменклатуруОтбора(НоменклатураОтбора, ЕстьОтбор);
			
			ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокЗаказы, "Номенклатура", НоменклатураОтбора, ЕстьОтбор);
...
Функция ТекстЗапроса(ИмяЭлемента, ПодменятьТекущийЗаказ = Ложь)
	...
	ТекстЗапроса = 
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 16:49:28] +        
	"ВЫБРАТЬ
	|ЗапасыИПотребностиОбороты.Номенклатура КАК Номенклатура,
	|СУММА(ЗапасыИПотребностиОбороты.ЗаказаноПриход) КАК ПроизводствоОбщий  
	|ПОМЕСТИТЬ втПроизводствоОбщий
	|ИЗ
	|РегистрНакопления.ЗапасыИПотребности.Обороты(, , Регистратор,{Номенклатура В(&Номенклатура)}  Заказ ССЫЛКА Документ.ЗаказНаПроизводство2_2) КАК ЗапасыИПотребностиОбороты
	|ГДЕ
	|ЗапасыИПотребностиОбороты.Регистратор ССЫЛКА Документ.ЗаказНаПроизводство2_2
	|СГРУППИРОВАТЬ ПО
	|	ЗапасыИПотребностиОбороты.Номенклатура
	|ИНДЕКСИРОВАТЬ ПО Номенклатура;
	|/////////////////////////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗапасыИПотребностиОбороты.Номенклатура КАК Номенклатура,
	|	СУММА(ЗапасыИПотребностиОбороты.ЗаказаноПриход) КАК ПроизводствоСклад
	|ПОМЕСТИТЬ втПроизводствоСклад
	|ИЗ
	...
	|				И (Заказ.ДокументОснование = НЕОПРЕДЕЛЕНО
	|					ИЛИ Заказ.ДокументОснование.Ссылка ЕСТЬ NULL)
	|			{Номенклатура В(&Номенклатура)}) КАК ЗапасыИПотребностиОбороты
	|ГДЕ
	|	ЗапасыИПотребностиОбороты.Регистратор ССЫЛКА Документ.ЗаказНаПроизводство2_2
	|	И ЗапасыИПотребностиОбороты.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗапасыИПотребностиОбороты.Номенклатура
	|ИНДЕКСИРОВАТЬ ПО Номенклатура;
	| 
	//Доступность из отчета остатки и доступность товаров. берем только "доступно" на "сейчас"
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|				И Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) {Номенклатура В(&Номенклатура)}) КАК ИнформацияОДоступности
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Склад,
		...
		|ИЗ
		|	РегистрНакопления.ТоварыКОтгрузке.Остатки(, Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка){Номенклатура В(&Номенклатура)}) КАК ТоварыКОтгрузке
		|
		|ИНДЕКСИРОВАТЬ ПО Номенклатура, Склад, Назначение;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	""Сейчас"" КАК ТипЗаписи,
		|	ТоварыКОтгрузке.Номенклатура КАК Номенклатура,
		|	ТоварыКОтгрузке.Склад КАК Склад,
		|	ТоварыКОтгрузке.Назначение КАК Назначение,
		...
		|{ГДЕ ИнформацияОДоступности.Номенклатура В(&Номенклатура)}
		|ГДЕ
		...
		|	{ГДЕ План.Номенклатура В(&Номенклатура)}
		|	ГДЕ
		|		План.Склад ССЫЛКА Справочник.Склады
		|		И План.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		...
		|	ГДЕ
		|		План.Склад ССЫЛКА Справочник.Склады
		|		И План.НазначениеСтруктуры = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		И НЕ План.ВидСтроки В (ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Продукция), ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.ПроизводствоПродукции))
		...
		|			ИНАЧЕ ЕСТЬNULL(ДатыГрафика.Окончание, ДАТАВРЕМЯ(1, 1, 1))
		|		КОНЕЦ,
		|		0,
		|		0,
		|		ВЫБОР
		|			КОГДА План.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.ПроизводствоПродукции)
		|				ТОГДА 0
		|			ИНАЧЕ План.Запланировано
		|		КОНЕЦ,
		|		0,
		|		ВЫБОР
		|			КОГДА План.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.ПроизводствоПродукции)
		...
		|			ПО План.ЗаказНаПроизводство = ДатыГрафика.ЗаказНаПроизводство
		|				И (ДатыГрафика.СтатусГрафика = 0)
		|				И План.КлючПартия = ДатыГрафика.КлючПартия
		|				И (ДатыГрафика.Этап = ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка))
		|	 {ГДЕ План.Номенклатура В(&Номенклатура)}
		|	ГДЕ
		|		План.Склад ССЫЛКА Справочник.Склады
		|		И План.Запланировано <> 0
		|		И План.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК Набор
		|       
		|
		|СГРУППИРОВАТЬ ПО
		|	Набор.ТипЗаписи,
		...
		|			И (ТаблицаДанных.ТипЗаписи = ""Сейчас"")
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втБаза.Номенклатура КАК Номенклатура,
		|	втБаза.Склад КАК Склад,
		|	СУММА(втБаза.Доступно) КАК Доступно
		|ПОМЕСТИТЬ втДоступноПоСкладу
		|ИЗ
		|	втБаза КАК втБаза
		|ГДЕ
		|	втБаза.ТипЗаписи = ""Сейчас""
		|	И втБаза.Доступно > 0
		|
		|СГРУППИРОВАТЬ ПО
		|	втБаза.Склад,
	...
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 16:49:28] -
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 15:31:53] +        
		|ЕСТЬNULL(бит_ЗначенияПризнаковНоменклатурыМарки.бит_Значение, ЗНАЧЕНИЕ(Справочник.Марки.ПустаяСсылка)) КАК Марка,
		|ЕСТЬNULL(бит_ЗначенияПризнаковНоменклатурыСечение.бит_Значение, ЗНАЧЕНИЕ(Справочник.бит_ДополнительныеПризнакиНоменклатуры.ПустаяСсылка)) КАК Сечение,
		|ЕСТЬNULL(бит_ЗначенияПризнаковНоменклатурыИсполнение.бит_Значение, ЗНАЧЕНИЕ(Справочник.бит_ДополнительныеПризнакиНоменклатуры.ПустаяСсылка)) КАК Исполнение,
		|ЕСТЬNULL(втПроизводствоОбщий.ПроизводствоОбщий, 0) КАК ПроизводствоОбщий,  
		|ЕСТЬNULL(втПроизводствоСклад.ПроизводствоСклад, 0) КАК ПроизводствоСклад,   
		|ЕСТЬNULL(втДоступноПоСкладу.Доступно, 0) КАК НаСкладе,
		|	&ЗаказНаОтгрузкуПереопределяемый                            КАК ЗаказКлиента,
		// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 15:31:53] -
		...
		|		
		// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 15:34:54] +        
		|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_ЗначенияПризнаковНоменклатуры КАК бит_ЗначенияПризнаковНоменклатурыМарки
		|ПО СостояниеЗаказа.Номенклатура = бит_ЗначенияПризнаковНоменклатурыМарки.бит_Номенклатура
		|	И (бит_ЗначенияПризнаковНоменклатурыМарки.бит_Признак = &ПризнакМарка)
		|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_ЗначенияПризнаковНоменклатуры КАК бит_ЗначенияПризнаковНоменклатурыСечение
		|ПО СостояниеЗаказа.Номенклатура = бит_ЗначенияПризнаковНоменклатурыСечение.бит_Номенклатура
		|	И (бит_ЗначенияПризнаковНоменклатурыСечение.бит_Признак = &ПризнакСечение)
		|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_ЗначенияПризнаковНоменклатуры КАК бит_ЗначенияПризнаковНоменклатурыИсполнение
		|ПО СостояниеЗаказа.Номенклатура = бит_ЗначенияПризнаковНоменклатурыИсполнение.бит_Номенклатура
		|	И (бит_ЗначенияПризнаковНоменклатурыИсполнение.бит_Признак = &ПризнакИсполнение) 
		|ЛЕВОЕ СОЕДИНЕНИЕ втПроизводствоОбщий КАК втПроизводствоОбщий ПО СостояниеЗаказа.Номенклатура = втПроизводствоОбщий.Номенклатура 
		|ЛЕВОЕ СОЕДИНЕНИЕ втПроизводствоСклад КАК втПроизводствоСклад ПО СостояниеЗаказа.Номенклатура = втПроизводствоСклад.Номенклатура
		|ЛЕВОЕ СОЕДИНЕНИЕ втДоступноПоСкладу КАК втДоступноПоСкладу ПО СостояниеЗаказа.Номенклатура = втДоступноПоСкладу.Номенклатура И СостояниеЗаказа.Склад = втДоступноПоСкладу.Склад
		// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 15:34:54] -
		|ГДЕ
		|	ВЫБОР
		|		КОГДА СостояниеЗаказа.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ЗарезервированПриПоступлении)
	...
	ИначеЕсли ИмяЭлемента = Элементы.СписокЗаказы.Имя Тогда
		...
		ТекстЗапроса = ТекстЗапроса + "
		// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 14:36:39] +        
			|{ГДЕ
			|	СостояниеЗаказа.Номенклатура     В(&Номенклатура)}
		// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 14:36:39] -
			|{ГДЕ
...
&НаКлиенте
Процедура бит_СечениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:41:05] +  
	СтандартнаяОбработка = Ложь;                                                            
	ЗначениеПризнака = ПолучитьЗначениеПоУмолчанию("ПризнакРазмерГотовойПродукции");
	
	ОткрытьФормуПодбора(Элемент, ЗначениеПризнака);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:41:05] -
КонецПроцедуры

&НаСервере
Функция ПолучитьЗначениеПоУмолчанию(ЭлементСправочника)  
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:47:48] +        
	Признак = Справочники.бит_ЗначенияПоУмолчанию[ЭлементСправочника];
	Возврат Справочники.бит_ЗначенияПоУмолчанию.ПолучитьМассивЗначений(Признак, ЭтотОбъект, Истина);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:47:48] -
КонецФункции

&НаКлиенте
Процедура бит_ИсполнениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:41:05] +  
	СтандартнаяОбработка = Ложь;                                                            
	ЗначениеПризнака = ПолучитьЗначениеПоУмолчанию("ПризнакВидИсполненияГотовойПродукции");
...
&НаКлиенте
Процедура ОткрытьФормуПодбора(Элемент, ЗначениеПризнака) 
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:51:47] +        
    ...
    Отбор.ПравоеЗначение = ЗначениеПризнака;
    Отбор.Использование = Истина;
    Отбор.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
    ПФ = Новый Структура;
    ПФ.Вставить("ФиксированныеНастройки", фиксНастройки);
    ОткрытьФорму("Справочник.бит_ДополнительныеПризнакиНоменклатуры.ФормаВыбора", ПФ, Элемент);
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 1:51:47] -
КонецПроцедуры

&НаКлиенте
Процедура бит_МаркаПриИзменении(Элемент)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:33:38] +        
	УправлениеПараметрамиСписков(Элементы.бит_Марка.Имя);
	СохранитьНастройки();
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:33:38] -
КонецПроцедуры

&НаКлиенте
Процедура бит_СечениеПриИзменении(Элемент)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:33:38] +        
	УправлениеПараметрамиСписков(Элементы.бит_Сечение.Имя);
	СохранитьНастройки();
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:33:38] -
КонецПроцедуры

&НаКлиенте
Процедура бит_ИсполнениеПриИзменении(Элемент)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:33:38] +        
	УправлениеПараметрамиСписков(Элементы.бит_Исполнение.Имя);
	СохранитьНастройки();
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:33:38] -
КонецПроцедуры 

&НаСервере
Процедура ПолучитьНоменклатуруОтбора(НомОтбор, ЕстьОтбор)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:51:00] +        
	Если Не ЗначениеЗаполнено(бит_Марка) И Не ЗначениеЗаполнено(бит_Сечение) И Не ЗначениеЗаполнено(бит_Исполнение) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|
		|    ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_ЗначенияПризнаковНоменклатуры КАК СвойстваСечения
		|        ПО СвойстваСечения.бит_Номенклатура = спрНоменклатура.Ссылка
		|        И СвойстваСечения.бит_Признак = &СвойствоСечение
		|
		|    ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_ЗначенияПризнаковНоменклатуры КАК СвойстваМарка
		|        ПО СвойстваМарка.бит_Номенклатура = спрНоменклатура.Ссылка
		|        И СвойстваМарка.бит_Признак = &СвойствоМарка
		|
		|    ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бит_ЗначенияПризнаковНоменклатуры КАК СвойстваИсполнение
		|        ПО СвойстваИсполнение.бит_Номенклатура = спрНоменклатура.Ссылка
		|        И СвойстваИсполнение.бит_Признак = &СвойствоИсполнение
		|
		|ГДЕ
		|       (&ЗначениеСечение = неопределено ИЛИ СвойстваСечения.бит_Значение = &ЗначениеСечение)
		|    и (&ЗначениеМарка  = неопределено  ИЛИ СвойстваМарка.бит_Значение = &ЗначениеМарка)
		|    И (&ЗначениеИсполнение  = неопределено ИЛИ СвойстваИсполнение.бит_Значение = &ЗначениеИсполнение)
		|");
	Запрос.УстановитьПараметр("ЗначениеСечение"	, ?(ЗначениеЗаполнено(бит_Сечение), бит_Сечение, Неопределено));
	Запрос.УстановитьПараметр("ЗначениеМарка"	, ?(ЗначениеЗаполнено(бит_Марка), бит_Марка, Неопределено));
	Запрос.УстановитьПараметр("ЗначениеИсполнение", ?(ЗначениеЗаполнено(бит_Исполнение), бит_Исполнение, Неопределено));
	Запрос.УстановитьПараметр("СвойствоСечение"	, ПолучитьЗначениеПоУмолчанию("ПризнакРазмерГотовойПродукции"));
	Запрос.УстановитьПараметр("СвойствоМарка"	, ПолучитьЗначениеПоУмолчанию("ПризнакMARKA"));
	Запрос.УстановитьПараметр("СвойствоИсполнение", ПолучитьЗначениеПоУмолчанию("ПризнакВидИсполненияГотовойПродукции"));
	
	ТЗ = Запрос.Выполнить().Выгрузить();
	Если ТЗ.Количество() Тогда
		НомОтбор = ТЗ.ВыгрузитьКолонку("Номенклатура");
	КонецЕсли;
	ЕстьОтбор = Истина;
	Возврат;
	// }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [08.08.2025 2:51:00] -
...
&НаКлиенте
Процедура бит_ЗабронироватьВСкладскомЗаказе(Команда)
	// {{ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [09.08.2025 1:10:48] +        
	ТД = Элементы.СписокЗаказы.ТекущиеДанные;
	...
	КонецЕсли;
	ПФ = Новый Структура;
	ПФ.Вставить("ЗаказКлиента"	, ТД.ЗаказКлиента);  
	ПФ.Вставить("Исполнение"	, ТД.Исполнение);
	ПФ.Вставить("Марка"			, ТД.Марка);
	ПФ.Вставить("Номенклатура"	, ТД.Номенклатура);
	ПФ.Вставить("Обособленно"	, ТД.Обособленно);
	ПФ.Вставить("Количество"	, ТД.Количество);
    ПФ.Вставить("Сечение"		, ТД.Сечение);

	ОткрытьФорму("Обработка.бит_СостояниеОбеспеченияЗаказов.Форма.ФормаБронированияВЗапущенныхЗаказах", ПФ, ЭтаФорма, УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
    // }}ПервыйБИТ ЛистТребований [ЛТ5.35]  [VUVaynula@1cbit.ru] [09.08.2025 1:10:48] -
КонецПроцедуры

&НаКлиенте
Процедура бит_РезервироватьПоТаре(Команда) 
	// {{ПервыйБИТ ЛистТребований [5.36]  [VUVaynula@1cbit.ru] [10.08.2025 17:57:59] +        
	ТД = Элементы.СписокЗаказы.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		ТД = Элементы.Список.ТекущиеДанные;
		Если ТД = Неопределено Тогда
	...
	ОбъектыОснований.Добавить(ТД.ЗаказКлиента);
	ОписаниеКоманды = Новый Структура();
	ОписаниеКоманды.Вставить("ОбъектыОснований", ОбъектыОснований);
	
	Если ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ТД.ЗаказКлиента, "Статус") = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовКлиентов.НеСогласован") Тогда
	...
	//ИначеЕсли Не ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ТД.ЗаказКлиента, "Проведен") Тогда
