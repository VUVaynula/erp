...
Функция ЕстьНастройкаТранспортаОбменаОбновления(ВключитьРегламентноеЗадание = Ложь)
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	РегистрСведений.УдалитьНастройкиТранспортаОбменаДанными.Корреспондент
	|ГДЕ
	|	РегистрСведений.УдалитьНастройкиТранспортаОбменаДанными.Корреспондент = &Корреспондент";
...
Процедура НастроитьТранспортОбменаВыгрузки()
	...
	ОбменДаннымиКлиент.ОткрытьФормуЗаписиРегистраСведенийПоОтбору(Отбор, ЗначенияЗаполнения,
		"УдалитьНастройкиТранспортаОбменаДанными", ЭтаФорма, , , ОписаниеОповещения);
