...
Функция ПолучитьТаблицуЗаменыДляФормированияЭтапов(
			...
			УчестьРазрешенияМногиеКоМногим = Истина) Экспорт
	
	Если Товары.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПараметрыВыбораАналогов = ПараметрыВыбораАналогов(Новый ОписаниеТипов("ДокументСсылка.ЭтапПроизводства2_2"));
...
Функция ПолучитьТаблицуЗамены(
	...
	ТаблицаРазрешенийПоПриоритетам.Индексы.Добавить("Разрешение, Ссылка, КлючСтроки");
	...
	ОтборПоРазрешению = Новый Структура("Разрешение, Ссылка, КлючСтроки");
	...
	Пока ТаблицаРазрешенийПоПриоритетам.Количество() > 0 Цикл
		...
		НормативВзаимозаменяемогоМатериала = 1;
		НормативАналога = 1;
		КоэффициентВзаимозаменяемогоМатериала = 1;
		ОбщееКоличествоКРаспределению = 0;
		НазначениеНабора = СтрокаРазрешенийПоПриоритетам.Назначение;
		...
		Для каждого СтрокаТаблицы Из МассивСтрокМатериалы Цикл
			...
			НормативВзаимозаменяемогоМатериала = СтрокаТаблицы.Норматив;
		...
		Если КоэффициентРасчета > 0 Тогда
			...
			Пока УдалитьРазрешенияПоАналогам Цикл
				...
				МассивСтрок = ТаблицаАналоговРазрешений.НайтиСтроки(Новый Структура("ОстатокНаСкладе, ВзаимозаменяемыйАналог", 0, Ложь));
				...
				МассивУдаляемыхСтрок = ТаблицаАналоговРазрешений.НайтиСтроки(Новый Структура("ОстатокНаСкладе, ВзаимозаменяемыйАналог", 0, Истина));
				ИндексСтроки = МассивУдаляемыхСтрок.ВГраница();
				Пока ИндексСтроки > -1 Цикл
					ТаблицаАналоговРазрешений.Удалить(МассивУдаляемыхСтрок[ИндексСтроки]);
					ИндексСтроки = ИндексСтроки - 1;
...
Функция ТекстЗапросаТаблицаМатериалов(РассчитатьПриоритетПрименения = Ложь) Экспорт
	...
	|	Товары.Ссылка КАК Ссылка,
	|	ВЫБОР
	|	КОГДА Товары.РольНоменклатуры = ЗНАЧЕНИЕ(Перечисление.РолиАналогов.ВзаимозаменяемыйАналог)
	...
	|			И НЕ (Товары.Номенклатура = АналогиМатериалов.Номенклатура
	|					И Товары.Характеристика = АналогиМатериалов.Характеристика)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	АналогиМатериалов.Номенклатура КАК Номенклатура,
	|	АналогиМатериалов.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ТаблицаНаборовРазрешений
	|ИЗ
	|	АналогиМатериалов КАК АналогиМатериалов
	|ГДЕ
	...
	|ПОМЕСТИТЬ РазрешенияПоНазначениям
	|ИЗ
	|	АналогиМатериалов КАК АналогиМатериалов
	|ГДЕ
	|	АналогиМатериалов.РольНоменклатуры = ЗНАЧЕНИЕ(Перечисление.РолиАналогов.Материал)
	|	И АналогиМатериалов.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка, Разрешение, Назначение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаНаборовРазрешений.Ссылка КАК Ссылка,
	|	ТаблицаНаборовРазрешений.Разрешение КАК Разрешение,
	|	ТаблицаНаборовРазрешений.Номенклатура КАК Номенклатура,
	|	ТаблицаНаборовРазрешений.Характеристика КАК Характеристика,
	|	РазрешенияПоНазначениям.Назначение КАК Назначение
	|ПОМЕСТИТЬ НаборыРазрешенийПоНазначениям
	|ИЗ
	|	ТаблицаНаборовРазрешений КАК ТаблицаНаборовРазрешений
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РазрешенияПоНазначениям КАК РазрешенияПоНазначениям
	|		ПО ТаблицаНаборовРазрешений.Разрешение = РазрешенияПоНазначениям.Разрешение
	|			И ТаблицаНаборовРазрешений.Ссылка = РазрешенияПоНазначениям.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка, Разрешение, Номенклатура, Характеристика, Назначение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Подзапрос.Ссылка КАК Ссылка,
	|	Подзапрос.Разрешение КАК Разрешение
	|ПОМЕСТИТЬ ФильтрНаборовМногиеКоМногим
	|ИЗ
	|	(ВЫБРАТЬ
	|		НаборыРазрешенийПоНазначениям.Ссылка КАК Ссылка,
	|		НаборыРазрешенийПоНазначениям.Разрешение КАК Разрешение
	|	ИЗ
	|		НаборыРазрешенийПоНазначениям КАК НаборыРазрешенийПоНазначениям
	|	ГДЕ
	|		НЕ ИСТИНА В
	|					(ВЫБРАТЬ ПЕРВЫЕ 1
	|						ИСТИНА
	...
	|						АналогиМатериалов КАК АналогиМатериалов
	|					ГДЕ
	|						АналогиМатериалов.Разрешение = НаборыРазрешенийПоНазначениям.Разрешение
	|						И АналогиМатериалов.Номенклатура = НаборыРазрешенийПоНазначениям.Номенклатура
	|						И АналогиМатериалов.Характеристика = НаборыРазрешенийПоНазначениям.Характеристика
	|						И АналогиМатериалов.Назначение = НаборыРазрешенийПоНазначениям.Назначение
	|						И АналогиМатериалов.Ссылка = НаборыРазрешенийПоНазначениям.Ссылка
	|						И НЕ АналогиМатериалов.Обособленно)
	|		И ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	...
	|						И АналогиМатериалов.Назначение = НаборыРазрешенийПоНазначениям.Назначение
	|						И АналогиМатериалов.Ссылка = НаборыРазрешенийПоНазначениям.Ссылка
	|						И АналогиМатериалов.Обособленно)
	|		И ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	...
	|					И АналогиМатериалов.Назначение = НаборыРазрешенийПоНазначениям.Назначение
	|					И АналогиМатериалов.Ссылка = НаборыРазрешенийПоНазначениям.Ссылка
	|					И АналогиМатериалов.Обособленно)) КАК Подзапрос
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Разрешение, Ссылка
	|;
	...
	|	МАКСИМУМ(АналогиМатериалов.НомерСтрокиТовары) КАК Ключ
	|ПОМЕСТИТЬ НаборыМногиеКоМногимГруппируемыеПоНазначению
	|ИЗ
	|	АналогиМатериалов КАК АналогиМатериалов
	|ГДЕ
	|	ИСТИНА В
	...
	|				ИСТИНА
	|			ИЗ
	|				ФильтрНаборовМногиеКоМногим КАК ФильтрНаборовМногиеКоМногим
	|			ГДЕ
	|				ФильтрНаборовМногиеКоМногим.Разрешение = АналогиМатериалов.Разрешение
	|				И ФильтрНаборовМногиеКоМногим.Ссылка = АналогиМатериалов.Ссылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	АналогиМатериалов.Ссылка,
	|	АналогиМатериалов.Назначение,
	...
	|	АналогиМатериалов.Назначение,
	|	ИСТИНА,
	|	МАКСИМУМ(АналогиМатериалов.НомерСтрокиТовары)
	|ИЗ
	|	АналогиМатериалов КАК АналогиМатериалов
	|ГДЕ
	|	ИСТИНА В
	...
	|	НаборыРазрешенийПоНазначениям.Ссылка КАК Ссылка,
	|	НаборыРазрешенийПоНазначениям.Разрешение КАК Разрешение,
	|	НаборыРазрешенийПоНазначениям.Назначение КАК Назначение
	|ПОМЕСТИТЬ НазначенияКОбнулениюКлюча
	|ИЗ
	|	НаборыРазрешенийПоНазначениям КАК НаборыРазрешенийПоНазначениям
	|ГДЕ
	|	НЕ ИСТИНА В
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НазначенияКОбнулениюКлюча.Ссылка КАК Ссылка,
	|	НазначенияКОбнулениюКлюча.Разрешение КАК Разрешение,
	|	НазначенияКОбнулениюКлюча.Назначение КАК Назначение,
	|	ЛОЖЬ КАК Обособленно,
	|	0 КАК Ключ
	|ПОМЕСТИТЬ КлючиПоРазрешениюСНазначением
	|ИЗ
	|	НазначенияКОбнулениюКлюча КАК НазначенияКОбнулениюКлюча
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	АналогиМатериалов.Разрешение,
	|	АналогиМатериалов.Назначение,
	|	АналогиМатериалов.Обособленно,
	|	МАКСИМУМ(АналогиМатериалов.НомерСтрокиТовары)
	|ИЗ
	|	АналогиМатериалов КАК АналогиМатериалов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РазрешенияПоНазначениям КАК РазрешенияПоНазначениям
	|		ПО (РазрешенияПоНазначениям.Разрешение = АналогиМатериалов.Разрешение)
	|			И (РазрешенияПоНазначениям.Назначение = АналогиМатериалов.Назначение)
	|			И (РазрешенияПоНазначениям.Ссылка = АналогиМатериалов.Ссылка)
	|ГДЕ
	|	НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	...
	|					НазначенияКОбнулениюКлюча КАК НазначенияКОбнулениюКлюча
	|				ГДЕ
	|					НазначенияКОбнулениюКлюча.Разрешение = АналогиМатериалов.Разрешение
	|					И НазначенияКОбнулениюКлюча.Ссылка = АналогиМатериалов.Ссылка
	|					И НазначенияКОбнулениюКлюча.Назначение = АналогиМатериалов.Назначение)
	|	И НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					НаборыМногиеКоМногимГруппируемыеПоНазначению КАК НаборыМногиеКоМногимГруппируемыеПоНазначению
	|				ГДЕ
	|					НаборыМногиеКоМногимГруппируемыеПоНазначению.Разрешение = АналогиМатериалов.Разрешение
	|					И НаборыМногиеКоМногимГруппируемыеПоНазначению.Ссылка = АналогиМатериалов.Ссылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	АналогиМатериалов.Ссылка,
	|	АналогиМатериалов.Разрешение,
	...
	|
	|ВЫБРАТЬ
	|	НаборыМногиеКоМногимГруппируемыеПоНазначению.Ссылка,
	|	НаборыМногиеКоМногимГруппируемыеПоНазначению.Разрешение,
	|	НаборыМногиеКоМногимГруппируемыеПоНазначению.Назначение,
	|	НаборыМногиеКоМногимГруппируемыеПоНазначению.Обособленно,
	|	НаборыМногиеКоМногимГруппируемыеПоНазначению.Ключ
	|ИЗ
	|	НаборыМногиеКоМногимГруппируемыеПоНазначению КАК НаборыМногиеКоМногимГруппируемыеПоНазначению
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	АналогиМатериалов.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(КлючиПоРазрешениюСНазначением.Ключ, АналогиМатериалов.КлючСтроки) КАК КлючСтроки,
	|	АналогиМатериалов.Период КАК Период,
	|	АналогиМатериалов.Разрешение КАК Разрешение,
	|	АналогиМатериалов.Номенклатура КАК Номенклатура,
	|	АналогиМатериалов.Характеристика КАК Характеристика,
	...
	|	АналогиМатериалов.РольНоменклатуры КАК РольНоменклатуры,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(АналогиМатериалов.Разрешение) = ТИП(Документ.РазрешениеНаЗаменуМатериалов)
	|			ТОГДА ВЫРАЗИТЬ(АналогиМатериалов.Разрешение КАК Документ.РазрешениеНаЗаменуМатериалов).МоментВремени
	|		КОГДА ТИПЗНАЧЕНИЯ(АналогиМатериалов.Разрешение) = ТИП(Документ.КорректировкаРегистров)
	|			ТОГДА ВЫРАЗИТЬ(АналогиМатериалов.Разрешение КАК Документ.КорректировкаРегистров).МоментВремени
	|	КОНЕЦ КАК РазрешениеМоментВремени
	|ПОМЕСТИТЬ ТаблицаМатериаловПоНазначениям
	|ИЗ
	|	АналогиМатериалов КАК АналогиМатериалов
	|		ЛЕВОЕ СОЕДИНЕНИЕ КлючиПоРазрешениюСНазначением КАК КлючиПоРазрешениюСНазначением
	|		ПО (АналогиМатериалов.Разрешение = КлючиПоРазрешениюСНазначением.Разрешение)
	|			И (АналогиМатериалов.Назначение = КлючиПоРазрешениюСНазначением.Назначение)
	|			И (АналогиМатериалов.Обособленно = КлючиПоРазрешениюСНазначением.Обособленно)
	|			И (АналогиМатериалов.Ссылка = КлючиПоРазрешениюСНазначением.Ссылка)
	|ГДЕ
	|	НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	...
	|	ТаблицаМатериаловПоНазначениям.КлючСтроки КАК КлючСтроки,
...
Функция ТекстЗапросаРазрешенияПоПриоритетам(ПараметрыОтборов = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|	ТаблицаМатериалов.Ссылка КАК Ссылка,
	|	ТаблицаМатериалов.КлючСтроки КАК КлючСтроки,
	|	ТаблицаМатериалов.Разрешение КАК Разрешение,
	|	ТаблицаМатериалов.Номенклатура КАК Номенклатура,
	|	ТаблицаМатериалов.Характеристика КАК Характеристика,
	...
	|	ТаблицаМатериалов.Разрешение КАК Разрешение,
	|	ТаблицаМатериалов.КлючСтроки КАК КлючСтроки,
	|	ТаблицаМатериалов.РольНоменклатуры КАК РольНоменклатуры,
	|	ТаблицаМатериалов.Приоритет КАК Приоритет,
	|	ТаблицаМатериалов.Период КАК Период,
	|	ТаблицаМатериалов.РазрешениеМоментВремени КАК РазрешениеМоментВремени
	|ИЗ
	|	Товары КАК Товары
	...
	|			И Товары.Назначение = ТаблицаМатериалов.Назначение
	|			И Товары.Обособленно = ТаблицаМатериалов.Обособленно
	...
	|				ГДЕ
	|					РазрешенияМногиеКоМногим.Разрешение = ТаблицаМатериалов.Разрешение)
	|	И НЕ (Товары.Обособленно
	|			И ТаблицаМатериалов.КлючСтроки = 0)";
	...
	Если УчестьМногиеКоМногим Тогда
		...
		|	ТаблицаМатериалов.Разрешение,
		|	ТаблицаМатериалов.КлючСтроки,
		|	ТаблицаМатериалов.РольНоменклатуры,
		|	ТаблицаМатериалов.Приоритет,
		|	ТаблицаМатериалов.Период,
		|	ТаблицаМатериалов.РазрешениеМоментВремени
		|ИЗ
		|	Товары КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаМатериаловПоНазначениям КАК ТаблицаМатериалов
		|		ПО Товары.Номенклатура = ТаблицаМатериалов.Номенклатура
