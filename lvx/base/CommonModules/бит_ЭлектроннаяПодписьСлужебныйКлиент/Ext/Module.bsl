...
Процедура ОткрытьСертификатПродолжение(Контекст)
	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		ТекстСообщения = НСтр("ru='Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент: Некорректное значение параметра ДанныеСертификата ""%1"".';
							  |en='Error when calling the OtkrytSertifikat common module ElektronnayaPodpisKlient: Incorrect value DannyeSertifikata ""%1"".'");
		ТекстСообщения = СтрШаблон(ТекстСообщения,Строка(Контекст.ДанныеСертификата));
...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения сертификата';
												|en='Select a file to store the certificate'");
...
Процедура СохранитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		Состояние(НСтр("ru='Сертификат сохранен в файл.';en='The certificate is stored in a file.'"));
	КонецЕсли;
...
Процедура СохранитьЗапросНаСертификатПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения запроса на сертификат';
							|en='Select a file to store the certificate request'");
	Диалог.Фильтр    = НСтр("ru='Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*';
							|en='Files certificate (* .p10)|* .p10|All files (*.*)|*.*'");
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		ЕстьПолученныеФайлы = Истина;
		Состояние(НСтр("ru='Запрос на сертификат сохранен в файл.';en='The certificate request stored in the file.'"));
	КонецЕсли;
...
Процедура СохранитьПодпись(АдресПодписи) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*';
			 |en='Files electronic signatures (*.%1)|*.%1|All files (*.*)|*.*'"),
		ЭлектроннаяПодписьКлиент.ПерсональныеНастройки().РасширениеДляФайловПодписи);
	
	ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
	ПараметрыСохранения.Диалог.Фильтр = Фильтр;
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения подписи';en='Select a file to save the signature'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ТекстСообщения = НСтр("ru='Электронная подпись сохранена в файл
				 			  |""%1"".';
						  |en='An electronic signature is stored in a file
				 			  |""%1"".'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, ПолученныеФайлы[0].Имя);
...
Процедура ПолучитьСертификатПоОтпечаткуЗавершение(Сертификат, ПредставлениеОшибки, Контекст)
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = НСтр("ru='Сертификат не найден на компьютере по причине: 
							   |%1';
						   |en='Certificate can not be found on your computer because: 
						       |%1'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ru='Сертификат не найден на компьютере.';en='The certificate is not found on the computer.'");
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеОшибкиПроверкиПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СтандартнаяОбработка = Ложь;
	Результат = Новый Структура("ТекстОшибкиПроверкиПодписи", ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ru='Сертификат не найден в данных подписи.';en='The certificate is not found in the data signature.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ТекстСообщения = НСтр("ru='%1 (на сервере) %2%3 (на клиенте)';en='%1 (on server) %2%3 (on the client)'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, Контекст.ОписаниеОшибкиНаСервере, Символы.ПС, ОписаниеОшибки);
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("ru='Требуется программа электронной подписи и шифрования';
						  |en='Requires software electronic signature and encryption'");
	Операция = Контекст.Операция;
	
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось подписать данные по причине:';en='Unable to sign the data because:'");		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить подпись по причине:';en='Unable to verify the signature because:'");		
	...
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось расшифровать данные по причине:';en='Unable to decrypt the data because:'");		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось проверить сертификат по причине:';en='Unable to verify the certificate because:'");		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ru='Не удалось получить сертификаты по причине:';en='Unable to obtain certificates because:'");		
	ИначеЕсли Операция = Null И Контекст.ПоказатьОшибку <> Истина Тогда
		ЗаголовокОшибки = "";		
	ИначеЕсли Операция <> "" Тогда
		ТекстСообщения = НСтр("ru='Ошибка в функции МенеджерКриптографии. Неверное значение параметра Операция ""%1"".';
				 			  |en='Error function MenedzherKriptografii. Invalid parameter Operation ""%1"".'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, Операция);
		ВызватьИсключение ТекстСообщения;
	Иначе
	...
	Если Не Подключено Тогда
		ТекстОшибки = НСтр("ru = 'В обозреватель интернет требуется установить расширение для работы с электронной подписью и шифрованием.'; 
						   |en = 'The Internet browser is required to install the extension for e-signatures and encryption.'");		
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст) Экспорт
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru='Не предусмотрено использование ни одной программы.';en='Not provided use of a single program.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
		...
		Если Не СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ru = 'Обратитесь к администратору.';en='Contact administrator'");
		КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = НСтр("ru='Программа %1 не доступна или не установлена на компьютере.';
					 		   |en='The program %1 is not available or is not installed on your computer.'");
			ТекстОшибки = СтрШаблон(ТекстОшибки, Контекст.Программа);
		Иначе
			ТекстОшибки = НСтр("ru='Ни одна из программ не доступна или не установлена на компьютере.';
							   |en='None of the programs is not available or is not installed on the computer.'");
		КонецЕсли;
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Выберите файл сертификата (только для шифрования)';
							|en='Select the certificate file (for encryption)'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru='Файл сертификата должен быть в формате DER X.509, операция прервана.';
							   	   |en='The certificate file must be in the format DER X.509, the operation is interrupted.'");
	...
	Если Не Форма.Открыта() Тогда
		...
		ТекстВопроса = НСтр("ru='Сертификат уже добавлен.';en='The certificate has already been added.'");
		ПоказатьВопрос(Новый ОписаниеОповещения("ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПредупрежденияОСуществующем",
					   ЭтотОбъект, Форма.Сертификат), ТекстВопроса, Кнопки);
	КонецЕсли;
...
Процедура ПоказатьОшибкуОбращенияКПрограмме(ЗаголовокФормы, ЗаголовокОшибки, ОшибкаНаКлиенте, ОшибкаНаСервере,
	...
	Если ТипЗнч(ОшибкаНаКлиенте) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru='Для процедуры ""ПоказатьОшибкуОбращенияКПрограмме"" указан некорректный тип параметра ""ОшибкаНаКлиенте"".';
				 |en='For the procedure ""PokazatOshibkuObrascheniyaKProgramme"" set incorrect parameter type ""OshibkaNaKliente"".'");
	КонецЕсли;
	
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru='Для процедуры ""ПоказатьОшибкуОбращенияКПрограмме"" указан некорректный тип параметра ""ОшибкаНаСервере"".';
				 |en='For the procedure ""PokazatOshibkuObrascheniyaKProgramme"" set incorrect parameter type ""OshibkaNaServere"".'");
	КонецЕсли;
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Текст = НСтр("ru='Сертификаты, которые могут быть использованы для этой операции ограничены.';
				 |en='Certificates may be used for this operation is limited.'");
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("ru='Следующие разрешенные сертификаты доступны для выбора:';
						 |en='The following resolutions certificates are available for selection:'");
		...
		Если ЭлектроннаяПодписьКлиент.СоздаватьЭлектронныеПодписиНаСервере() Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';
								 |en='The following resolutions certificates are not found on the computer or on the server:'");
			Иначе
				Заголовок = НСтр("ru='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';
								 |en='None of these certificates are not allowed to match any computer or server:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru='Следующие разрешенные сертификаты не найдены на компьютере:';
								 |en='The following resolutions certificates are not found on the computer:'");
			Иначе
				Заголовок = НСтр("ru='Ни один из следующих разрешенных сертификатов не найден на компьютере:';
								 |en='None of the following resolutions certificates can not be found on your computer:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПодписьИзФайлаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		ТекстСообщения = НСтр("ru='Требуется программа электронной подписи и шифрования';
							  |en='Requires software electronic signature and encryption'");
		ПоказатьОшибкуОбращенияКПрограмме(ТекстСообщения, "", Контекст.Результат, 
										  Контекст.ФормаДобавления.МенеджерКриптографииНаСервереОписаниеОшибки);
	Иначе
...
Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		ТекстСообщения = НСтр("ru='При записи файла возникла ошибка:';en='When writing a file the error occurred:'");
		Ошибка = Новый Структура("ОписаниеОшибки", ТекстСообщения + Символы.ПС + Результат.ОписаниеОшибки);
			
		ТекстСообщения = НСтр("ru='Не удалось сохранить подписи вместе с файлом';en='Could not save signature with the file'");
		ПоказатьОшибкуОбращенияКПрограмме(ТекстСообщения, "", Ошибка, Новый Структура);
		Возврат;
...
Процедура УстановитьРасширениеПослеПроверкиПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(
		...
		НСтр("ru='Перед установкой расширения для работы с электронной подписью и шифрованием необходимо установить расширение для работы с файлами.';
			 |en='Before installing the expansion for e-signatures and encryption you must install the extension for the files.'"),
		Ложь);
		...
		Если Контекст.НаСторонеКлиента Тогда
			...
			Исключение
				...
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
					НСтр("ru='При записи подписи возникла ошибка:';en='When recording a signature error occurred:'")
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи зашифрованных данных возникла ошибка:';en='When writing the encrypted data error occurred:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи расшифрованных данных возникла ошибка:';en='With the decrypted data recording error occurred:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(Получен, Адрес, ВыбранноеИмяФайла, Контекст) Экспорт
	...
	Иначе
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("ru='Передача данных отменена пользователем.';en='Data transfer canceled by the user.'"));
	КонецЕсли;
