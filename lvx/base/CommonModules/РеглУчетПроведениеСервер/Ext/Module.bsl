...
Функция ОтразитьДокумент(РеквизитыДокумента, МенеджерВременныхТаблиц = Неопределено, ПараметрыОтражения = Неопределено) Экспорт
	...
	Транслировать = (НЕ бит_ОбщиеПеременныеСервер.ЗначениеПеременной("бит_ЭтоЧужойПодчиненныйУзел") = Истина);
	Если Транслировать Тогда
		Отказ = Ложь;
		Источник = РеквизитыДокумента.Ссылка.ПолучитьОбъект();
		// вызов механизма контроля закрытого периода		
		бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(Источник, Отказ, РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
	КонецЕсли;
	...
	//Если Транслировать Тогда
		// -- NB 
		МетаданныеИсточника = РеквизитыДокумента.Ссылка.Метаданные();		
		ОтображатьПротокол  = бит_УправлениеПользователямиСервер.ПолучитьЗначениеПоУмолчанию("ОтображатьПротоколТрансляции", 
								Пользователи.ТекущийПользователь());	

		РезультаТрансляции = бит_МеханизмТрансляции.ВыполнитьТрансляцию(Источник, , , Истина, Перечисления.бит_РежимыТрансляции.OnLine, ОтображатьПротокол);
		
		Если ОтображатьПротокол Тогда			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(РезультаТрансляции.Протокол);
		КонецЕсли;
		
		// вызов механизма контроля закрытого периода
	    бит_КонтрольЗакрытогоПериода.КонтрольПриОбработкеПроведения(Источник, Отказ, РежимПроведенияДокумента.Неоперативный, Истина);
	КонецЕсли; 			
	// бит_Финанс изменения кода. Конец.
...
Процедура ДополнительнаяОбработкаПриОтраженииДокумента(ТипДокумента, МенеджерВременныхТаблиц)
	...
	// бит_Финанс изменения кода. Начало.
	бит_ТрансляцияПриОтраженииДокумента(МенеджерВременныхТаблиц);
	// бит_Финанс изменения кода. Конец.
...
// 02.05.2017 # BF-927   
// 
Процедура бит_ТрансляцияПриОтраженииДокумента(ВременныеТаблицы)

	Если НЕ бит_ОбщиеПеременныеСервер.ЗначениеПеременной("бит_ЭтоЧужойПодчиненныйУзел") Тогда
		
		ДокументыКОтражению = ВременныеТаблицы.Таблицы.Найти("ДокументыКОтражению");
		Если НЕ ДокументыКОтражению = Неопределено Тогда
			
			ВыборкаДокументов = ДокументыКОтражению.ПолучитьДанные().Выбрать();
			Пока ВыборкаДокументов.Следующий() Цикл
				
				Отказ = Ложь;
				Источник = ВыборкаДокументов.Ссылка.ПолучитьОбъект();
				// вызов механизма контроля закрытого периода		
				бит_КонтрольЗакрытогоПериода.КонсервироватьДвижения(Источник, Отказ, РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);

				РезДанные = бит_МеханизмТрансляции.ВыполнитьТрансляцию(Источник, , ,Истина, Перечисления.бит_РежимыТрансляции.OnLine, Ложь);
				Протокол  = РезДанные.Протокол;
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Протокол);
