...
Функция ТекстВыпущеннаяПродукция()
	...
	//++ НЕ УТКА
	ТекстЗапроса = 
		// Выпущенная продукция по заказам (выпущенные товары и прослеживаемые работы).
		"ВЫБРАТЬ
		...
		|
		|ВЫБРАТЬ
		|	""ТекстВыпущеннаяПродукция_12""	КАК ЗапросИсточник,
		|	РеквизитыОтчета.Ссылка			КАК РегистраторРасхода,
		|	РеквизитыОтчета.Дата			КАК ДатаПроизводства,
		|	ДД.ПартияПроизводства			КАК ПартияПроизводства,
		|	РеквизитыОтчета.Подразделение	КАК Подразделение,
		...
		|	РеквизитыОтчета.Организация							КАК Организация
		|ИЗ
		|	Документ.ОтчетДавальцуМеждуОрганизациями.ВидыЗапасов КАК ДД
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцуМеждуОрганизациями КАК РеквизитыОтчета
		|		ПО ДД.Ссылка = РеквизитыОтчета.Ссылка
		|		
