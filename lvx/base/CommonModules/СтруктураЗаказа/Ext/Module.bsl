...
Процедура ВыполнитьРазузлованиеВнутриЗамераВремени(ПараметрыДействия)
	...
	|;
	|
	|ВЫБРАТЬ // все партии и изделия
	|
	|	Партии.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	|	Партии.ГруппаИзделий       КАК ГруппаИзделий,
	|	Партии.КлючПартия          КАК КлючПартия,
	|	Партии.КлючНоменклатура    КАК КлючНоменклатура,
	|
	|	ВЫБОР 
	|		КОГДА ИСТИНА В (
	|				ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Иначе
		...
		|		ИНАЧЕ ISNULL(ПартииИзделия.КлючПартия, Изделия.КлючПартияРодитель)
		|	КОНЕЦ КАК КлючПартия
		|ИЗ
		|	Изделия КАК Изделия
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПартииИзделия КАК ПартииИзделия
		|		ПО Изделия.КлючНоменклатура = ПартииИзделия.КлючНоменклатура";
	КонецЕсли;
...
#Область График
	...
	// В нормативном графике задания регистрируются по упрощенной схеме, в боевом графике могут рассчитываться по изменениям.
...
#КонецОбласти
	...
	ПартииИзделия = РезультатЗапроса[КоличествоПакетов-2];
	ПартииДоИзменения = СкопироватьКолонки(ПартииИзделия.Колонки, "ЗаказНаПроизводство,ГруппаИзделий,КлючПартия,БылиОшибки");
	КэшПартий = СкопироватьКолонки(ПартииИзделия.Колонки, "КлючНоменклатура"); 
	КэшПартий.Колонки.Добавить("Партия");
	КлючПартия = Неопределено;
	Выборка = ПартииИзделия.Выбрать();
	Пока Выборка.Следующий() Цикл
		Если КлючПартия <> Выборка.КлючПартия Тогда 
			Партия = ПартииДоИзменения.Добавить();
			ЗаполнитьЗначенияСвойств(Партия, Выборка);
			КлючПартия = Выборка.КлючПартия;
		КонецЕсли;
		СтрокаКэшПартий = КэшПартий.Добавить();
		СтрокаКэшПартий.КлючНоменклатура = Выборка.КлючНоменклатура;
		СтрокаКэшПартий.Партия = Партия;
	КонецЦикла;
	КэшПартий.Индексы.Добавить("КлючНоменклатура");
	...
	ПартииДоИзменения.Колонки.Добавить("КлючИспользован", Новый ОписаниеТипов("Булево"));
	ПартияСтруктура = Новый Структура("КлючПартия");
	КлючНоменклатураСтруктура = Новый Структура("КлючНоменклатура");
	...
	ИзделияГруппы = СкопироватьКолонки(Изделия.Колонки, "Порядок,Требуется,КлючПартия,ДлительностьДоВыпускаРодитель");
	...
	Пока ВсегоИзделий > Индекс Цикл
		...
		Если Не
			...
			// Упрощенная ветка кода
			// Не поддерживается укрупнение партии и распределение запуска на несколько строк, оптимальная партия и расчет размера партии для не основного изделия.
			
			Партия = ПартииГруппы.Добавить();
			ЗаполнитьЗначенияСвойств(Партия, Изделие, "ЗаказНаПроизводство, ГруппаИзделий" + ", ПроизводствоИзДавальческогоСырья");
			...
			Если Не Изделие.КлючПартия = ПустойКлючСвязи Тогда 
				Партия.КлючПартия = НайтиВКэшеКлючПартии(КэшПартий, Изделие.КлючНоменклатура, Изделие.КлючПартия, КлючНоменклатураСтруктура);
			КонецЕсли;
		...
		Иначе
			...
			Пока ВсегоИзделий > Индекс И ГруппаИзделий = Изделия[Индекс].ГруппаИзделий Цикл
				...
				ЗаполнитьЗначенияСвойств(ИзделиеГруппы, Изделие, "Порядок,Требуется,КлючПартия,ДлительностьДоВыпускаРодитель");
			...
			ИзделияГруппы.Сортировать("ДлительностьДоВыпускаРодитель Убыв, ПоляГруппировки Убыв, Порядок, КлючПартия Убыв, Требуется Убыв", Новый СравнениеЗначений());
					...
					Если Партия.КлючПартия <> ПустойКлючСвязи Тогда // партия инициализирована
						...
						Иначе
							...
							Иначе
								...
								ОстатокПартии = ?(ОстатокПартии <> Неопределено, ОстатокПартии,
									Партия.РазмерПартии * ?(Изделие.Порядок > 1, Изделие.КоличествоНаЕдиницуПартииЗапуска / Изделие.КоличествоОсновногоИзделияНаЕдиницуПартииЗапуска, 1));
							КонецЕсли;
						КонецЕсли;
					Иначе
						ЗаполнитьЗначенияСвойств(Партия, Изделие, "ЗаказНаПроизводство,ГруппаИзделий" + ",ПроизводствоИзДавальческогоСырья");
						...
						Если Партия.КлючПартия = ПустойКлючСвязи Тогда
							...
							Партия.КлючПартия = Новый УникальныйИдентификатор();
						КонецЕсли;
						ОстатокПартии = Партия.РазмерПартии * ?(Изделие.Порядок > 1, Изделие.КоличествоНаЕдиницуПартииЗапуска / Изделие.КоличествоОсновногоИзделияНаЕдиницуПартииЗапуска, 1);
				...
				Если ТактРасчетаРазмераПартии Тогда
					...
					Если ГруппировкаРавна Тогда // расчет размера партий
						...
						Если Не ИзделиеГруппы.Изделие.ПобочныйВыпуск Тогда
							...
							Иначе
								РазмерПартии = ?(ТекущееИзделие.ЕдиницаИзмеренияШтучная И КоличествоИзделия > Цел(КоличествоИзделия), Цел(КоличествоИзделия) + 1, КоличествоИзделия) 
									* (?(ТекущееИзделие.Порядок > 1, ТекущееИзделие.КоличествоОсновногоИзделияНаЕдиницуПартииЗапуска / ТекущееИзделие.КоличествоНаЕдиницуПартииЗапуска, 1));
		...
		Для каждого Партия Из ПартииГруппы Цикл
			...
			Если ИспользуетсяГрафикПроизводства Тогда
				Если Не Партия.Действует Тогда
					ЗаполнитьЗначенияСвойств(ЗаданияКОчисткеГрафика.Добавить(), Партия);
				Иначе
					Если Не ЕстьЗаданияКРасчетуГрафикаПолныйРасчет
...
Функция НайтиВКэшеКлючПартии(КлючиПартий, КлючНоменклатура, КлючПартияПриоритет, КлючНоменклатураСтруктура)
	
	ПодходящаяСтрока = Неопределено;
	КлючНоменклатураСтруктура.КлючНоменклатура = КлючНоменклатура; 
	НайденныеСтроки = КлючиПартий.НайтиСтроки(КлючНоменклатураСтруктура);
	
	Для каждого Строка ИЗ НайденныеСтроки Цикл 
		Если Строка.Партия.КлючИспользован Тогда
			Продолжить;
		КонецЕсли;
		ПодходящаяСтрока = Строка;
		Если ПодходящаяСтрока.Партия.КлючПартия = КлючПартияПриоритет Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ПодходящаяСтрока = Неопределено Тогда 
		КлючПартии = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");	
	Иначе
		КлючПартии = ПодходящаяСтрока.Партия.КлючПартия;
 		ПодходящаяСтрока.Партия.КлючИспользован = Истина;
	КонецЕсли;
...
Функция ЗапасДоступен(Выборка, Запас)
	...
	ИначеЕсли Выборка.НомерОчереди = Запас.НомерОчереди Тогда
		...
		Если Выборка.ПорядокНормативногоГрафика >= Запас.ПорядокНормативногоГрафика И НЕ Выборка.Запланировано > 0 Тогда // ограничение для контроля зацикливания расчета
...
Процедура ЗапланироватьПеремещение(ПараметрыРасчета, МенеджерВременныхТаблиц, ИмяВременнойТаблицы = "ПланПотребления")
	...
	|
	|ГДЕ
	|	НЕ &ИспользуетсяГрафикПроизводства
	|;
	|
	|ВЫБРАТЬ
	|
...
#КонецОбласти
	...
	Пока ЕстьЗаписиВВыборке И Выборка.Замещать Цикл // И (Выборка.Действие = "Удалить" ИЛИ Выборка.Действие = "Изменить") 
		...
		Попытка
			...
			Если ИспользуетсяГрафикПроизводства И Выборка.Количество = 0 Тогда
