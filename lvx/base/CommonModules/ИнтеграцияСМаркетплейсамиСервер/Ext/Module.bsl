		...
		Если НовыеКлючи.Свойство("access_token") 
			...
			Иначе
				ТекстОписанияСобытия = НСтр("ru = 'Ошибка сохранения данных в безопасном хранилище';
											|en = 'Ошибка сохранения данных в безопасном хранилище'", 
				ОбщегоНазначения.КодОсновногоЯзыка());
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Некорректное значение свойства expires_in при получении ключа авторизации: %1';
					|en = 'Некорректное значение свойства expires_in при получении ключа авторизации: %1'", 
				ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ЗаписатьЗаказыКлиентов(Результат, ТаблицаЗаказов, Параметры, ПараметрыПрогресса = Неопределено) Экспорт
	...
	Если Не ЗначенияФункциональныхОпций.ИспользоватьЗаказыКлиентов Тогда
		ТекстОшибки = НСтр("ru = 'Отключена функциональная опция ""Использовать заказы клиентов"".';
							|en = 'The ""Use sales orders"" functional option is disabled.'", 
			ОбщегоНазначения.КодОсновногоЯзыка());
						...
						ДанныеЗаполнения.Вставить("Комментарий",        СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																			...
																			+ ?(ВыборкаДанных.ЭтоВозврат, 
																				НСтр("ru = '(невыкуп, возврат товаров)';
																					|en = '(non-purchase, return of goods)'"), 
...
Функция ПеренестиНоменклатуруПартнера(УчетнаяЗапись, ПредыдущийПартнер, НовыйПартнер, Скопировать = Истина) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНоменклатуруПартнеров") Тогда
		ТекстОшибки = НСтр("ru = 'Для обновления данных о партнере для товаров из личного кабинета включите возможность использования номенклатуры партнеров в настройках раздела ""НСИ и администрирование"" - ""Настройка НСИ и разделов"" - ""Номенклатура"" - ""Дополнительная информация"".';
							|en = 'Для обновления данных о партнере для товаров из личного кабинета включите возможность использования номенклатуры партнеров в настройках раздела ""НСИ и администрирование"" - ""Настройка НСИ и разделов"" - ""Номенклатура"" - ""Дополнительная информация"".'", 
			ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если Не ЗначениеЗаполнено(НовыйПартнер) Тогда
		ТекстОшибки = НСтр("ru = 'Не указан партнер, представляющий торговую площадку.';
							|en = 'Не указан партнер, представляющий торговую площадку.'", 
			ОбщегоНазначения.КодОсновногоЯзыка());
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При %1 позиции товарного каталога %2 возникли ошибки.';
							|en = 'Errors occurred when %1 the %2 goods directory item.'",
							ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При %1 позиций товарного каталога возникли ошибки %2.';
				|en = 'Errors occurred when %1 the goods directory items: %2.'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			?(Скопировать, "копировании", "переносе"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
