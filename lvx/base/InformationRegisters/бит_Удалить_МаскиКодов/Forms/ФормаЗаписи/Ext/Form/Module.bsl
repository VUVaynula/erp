...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если МаскаНеКорректна Тогда
		...
		ТекстСообщения = НСтр("ru='Маска может состоять только из значимых символов ""@"" и разделителей ';
							  |en='Mask can only consist of significant symbols ""@"" and separators'") + фСтрокаРазделителей;
...
Процедура МаскаКодаПриИзменении(Элемент)
	...
	Если СтрДлина(СокрЛП(Запись.МаскаКода)) > ДлинаКода Тогда
		ТекстСообщения = НСтр("ru='Длина кода не должна превышать %1';en='Code length should not exceed %1'");
