			...
			Если РезультатОбработки.СоединениеАктивно 
				...
				Иначе	
					ИнформацияОПодключении = 
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Согласие действует: %1 дней (%2)';
																					|en = 'Consent is valid: %1 days (%2)'"), СрокДействияСогласия, ДатаОкончанияСогласия);
				КонецЕсли;
