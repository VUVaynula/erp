...
Процедура ОткрытьФормуПодключенияBankFeeds()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не записан элемент';
				|en = 'Item is not saved'"), , , "Объект", НеДобавлять); 
	КонецЕсли;

	Если Не ЗначениеЗаполнено(Объект.НомерСчета) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не указан номер счета';
				|en = 'Account number is not specified'"), , "НомерСчета", "Объект", НеДобавлять); 
	КонецЕсли;
	...
	Если Объект.РучноеИзменениеРеквизитовБанка
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не выбран банк или не заполнено наименование банка в ручных реквизитах банка';
				|en = 'Bank is not selected or the bank name is not specified in the manual bank details'"), , "Банк", "Объект", НеДобавлять); 
	ИначеЕсли Объект.РучноеИзменениеРеквизитовБанка
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не заполнена страна банка в ручных реквизитах банка';
				|en = 'Bank country is not filled in the manual bank details'"), , "Банк", "Объект", НеДобавлять); 
	КонецЕсли;
