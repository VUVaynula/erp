...
&НаКлиенте
Процедура ВидФормыСбораДанныхПриИзменении(Элемент)
	
	ВидФормыСбораДанныхИзменение();
...
Процедура КомандаЗагрузить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВидФормыСбораДанных) Тогда		
...
Процедура КомандаЗагрузитьЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	КомплектДанных.Вставить("ВидФормыСбораДанных", Объект.Ссылка);
...
Процедура ОбработатьЗагруженныеДанные(Знач КомплектДанных)
	...
	Если НЕ ЗначениеЗаполнено(Объект.КурсДокумента) 
		ИЛИ НЕ ЗначениеЗаполнено(Объект.КратностьДокумента) Тогда
		ВалютаДокументаИзменение();
	КонецЕсли; 
...
Процедура ДобавитьДинамическиеЭлементы()
	...
	Для каждого КиЗ Из ПсевдоМетаданные.ТабличныеЧасти Цикл
		...
		Элементы.Переместить(НоваяСтраница, Элементы.ГруппаСтраницы, Элементы.ГруппаДанныеФормыСбораДанных);
...
Процедура ГенерироватьФорму()
	...
	ПсевдоМетаданные = бит_РаботаСПроизвольнымиФормами.ЭмулироватьМетаданные(Объект.ВидФормыСбораДанных);
	ИмяПроформы      = Объект.ВидФормыСбораДанных.Имя;		
...
// 
&НаСервере
Процедура ВидФормыСбораДанныхИзменение()
...
// 
// Параметры:
//  ТекущийОбъект - ДокументОбъект.бит_ФормаСбораДанных.
// 
&НаСервере
...
Функция ПодготовитьДанныеТабЧастей()
	...
	ТабДанные = Документы.бит_ФормаСбораДанных.КонструкторТаблицыХраненияТабЧасти();
...
Процедура ВосстановитьДанныеТабЧастей()
	...
	Если ЗначениеЗаполнено(ИсточникКопирования) Тогда
		РезультатЗапроса = Документы.бит_ФормаСбораДанных.ПолучитьДанныеТабличныхЧастей(ИсточникКопирования);
	Иначе	
		РезультатЗапроса = Документы.бит_ФормаСбораДанных.ПолучитьДанныеТабличныхЧастей(Объект.Ссылка);
	КонецЕсли; 
