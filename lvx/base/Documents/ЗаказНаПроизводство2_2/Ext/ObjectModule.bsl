...
Процедура ПриЗаписи(Отказ)
	...
	ЗаписатьЗаданияПриИзмененииПриоритета();
...
Процедура ЗаписатьЗаданияПриИзмененииПриоритета()
	...
	НетИзменений = ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(
		ЭтотОбъект,
		ДополнительныеСвойства.ДанныеДокументаПередЗаписью, "Приоритет,Подразделение,Очередь");
	
	Если НетИзменений Или НЕ Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.КПроизводству Тогда
		Возврат;
	КонецЕсли;
	
	ПриоритетИзменен = Приоритет <> ДополнительныеСвойства.ДанныеДокументаПередЗаписью.Приоритет;
	
	Если ПриоритетИзменен Или ДинамическаяСтруктура Тогда
		
		ОтложенныеЗадания.ОбработатьЗаданияОчереди(
							"ЗаданияКРасчетуСтруктурыЗаказаПриИзмененииПриоритета",
							Новый Структура("ЗаказНаПроизводство", Ссылка));
