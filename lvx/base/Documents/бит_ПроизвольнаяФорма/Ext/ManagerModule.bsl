...
// 
Функция ПодготовитьДанныеДляВыгрузки(ПроизвольныеФормы)  Экспорт
	
	ДанныеДляВыгрузки = Новый Массив;
	СисИнфо = бит_ОбщегоНазначения.ПолучитьСведенияОПрограмме();
	
	Для каждого ТекПроформа Из ПроизвольныеФормы Цикл
		
		ДанныеДокумента = ПодготовитьДанныеДокумента(ТекПроформа);		
		
		СтрНабор = Новый Структура;
		СтрНабор.Вставить("ПсевдоМетаданные", бит_РаботаСПроизвольнымиФормами.ЭмулироватьМетаданные(ТекПроформа.ВидПроизвольнойФормы)); 
		...
		НаименованиеПроформы = СтрШаблон(НаименованиеПроформы, ТекПроформа.ВидПроизвольнойФормы, НомерДляВывода, ДанныеДокумента.Дата); 
...
// 
// Параметры:
//  Проформа - ДокументСсылка.бит_ПроизвольнаяФорма.
// 
// Возвращаемое значение:
//  ДанныеДокумента - Структура.
// 
Функция ПодготовитьДанныеДокумента(ПроизвольнаяФорма) Экспорт
	
	ПсевдоМета = бит_РаботаСПроизвольнымиФормами.ЭмулироватьМетаданные(ПроизвольнаяФорма.ВидПроизвольнойФормы);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ПроизвольнаяФорма);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПроизвольнаяФорма.Ссылка КАК Ссылка,
	|	ПроизвольнаяФорма.ПометкаУдаления КАК ПометкаУдаления,
	|	ПроизвольнаяФорма.Номер КАК Номер,
	|	ПроизвольнаяФорма.НомерПроформы КАК НомерПроформы,
	|	ПроизвольнаяФорма.Дата КАК Дата,
	|	ПроизвольнаяФорма.Проведен КАК Проведен,
	|	ПроизвольнаяФорма.ВидПроизвольнойФормы КАК ВидПроизвольнойФормы,
	|	ПроизвольнаяФорма.ВалютаДокумента КАК ВалютаДокумента,
	|	ПроизвольнаяФорма.КурсДокумента КАК КурсДокумента,
	|	ПроизвольнаяФорма.КратностьДокумента КАК КратностьДокумента,
	|	ПроизвольнаяФорма.Ответственный КАК Ответственный,
	|	ПроизвольнаяФорма.СуммаДокумента КАК СуммаДокумента
	|ИЗ
	|	Документ.бит_ПроизвольнаяФорма КАК ПроизвольнаяФорма
	|ГДЕ
	|	ПроизвольнаяФорма.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.бит_ПроизвольнаяФорма.ДанныеШапки КАК ДанныеШапки
	|ГДЕ
	...
	|ИЗ
	|	Документ.бит_ПроизвольнаяФорма.ДанныеТабличныхЧастей КАК ДанныеТабличныхЧастей
	|ГДЕ
	...
	ДанныеДокумента.Вставить("ВидПроизвольнойФормы" , ВыборкаРеквизиты.ВидПроизвольнойФормы);
...
Процедура ЗаполнитьПриемникиПроформы(ПроформаОбъект) Экспорт
   ...
   Для каждого СтрокаТаблицы Из ПроформаОбъект.ВидПроизвольнойФормы.НастройкиДвижений Цикл
...
Процедура РаспознатьДанные(МодельДокумента, ПсевдоМетаданные, СпискиИсточники) Экспорт
	...
	Для каждого МетаРеквизит Из ПсевдоМетаданные.СтандартныеРеквизиты Цикл
		...
		Если МодельДокумента.Свойство(ИмяРеквизита, ЗагруженныйЭлемент) Тогда 
			...
			ЗагруженныйЭлемент.Значение = бит_ОбменДанными.ПреобразоватьЗагруженноеЗначение(
				ЗагруженныйЭлемент.ЗначениеСтр, ТекТип, ЗагруженныйЭлемент.ИмяСписка, СпискиИсточники);
		КонецЕсли;
...
Функция КонструкторТаблицыХраненияТабЧасти() Экспорт
	...
	Таб.Колонки.Добавить("Проформа"          , Новый ОписаниеТипов("ДокументСсылка.бит_ПроизвольнаяФорма"));
...
// 
Функция ПолучитьДанныеТабличныхЧастей(ПроизвольнаяФорма) Экспорт

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПроизвольнаяФорма", ПроизвольнаяФорма);
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	Документ.бит_ПроизвольнаяФорма.ДанныеТабличныхЧастей КАК ДанныеТабличныхЧастей
	|ГДЕ
	|	ДанныеТабличныхЧастей.Ссылка = &ПроизвольнаяФорма
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ДанныеТабличныхЧастей.Проформа = &ПроизвольнаяФорма
	|ИТОГИ ПО
	|	ИмяТабличнойЧасти,
	|	НомерСтрокиТаблицы";
...
Функция ДанныеВидаПроформы(ИмяПроформы) Экспорт
	...
	РезСтр = Новый Структура("ВидПроизвольнойФормы, ПсевдоМетаданные, ИмяТаблицыДанных");
	РезСтр.ВидПроизвольнойФормы = Справочники.бит_ВидыПроизвольныхФорм.НайтиПоРеквизиту("Имя",ИмяПроформы);
	Если НЕ ЗначениеЗаполнено(РезСтр.ВидПроизвольнойФормы) Тогда
		ТекстСообщения = НСтр("ru = 'Для выбранного файла отсутствует шаблон в справочнике ""Виды произвольных форм"".';
								|en = 'There is no template for the selected file in the ""Data Collection Form Types"" reference.'"); 	
		ВызватьИсключение ТекстСообщения;
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(РезСтр.ВидПроизвольнойФормы) Тогда
		РезСтр.ПсевдоМетаданные = бит_РаботаСПроизвольнымиФормами.ЭмулироватьМетаданные(РезСтр.ВидПроизвольнойФормы);
		РезСтр.ИмяТаблицыДанных = Документы.бит_ПроизвольнаяФорма.ИмяТаблицыДляЗагрузкиДанныхБюджета(РезСтр.ПсевдоМетаданные);
	КонецЕсли; 
