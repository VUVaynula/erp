...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОбщиеНастройки.Доступность = (Метаданные.Роли.Найти("бит_сл_Администратор") <> Неопределено И РольДоступна("бит_сл_Администратор")) 
	...
	ФормаОткрытаКодом = Параметры.ФормаОткрытаКодом
							ИЛИ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "АвтоТест", Ложь);
...
Процедура ОткрытьФормуСписка(Команда)
	...
	ОткрытьФорму(ИмяФормыОбработки,
				 ...
				 ,
				 ,
				 Новый ОписаниеОповещения("ОткрытьФормуСпискаЗакрытие", ЭтотОбъект),
				 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);				  
			  	
КонецПроцедуры

&НаКлиенте
Процедура ПереключитьВерсиюСистемыЛицензирования(Команда)
	
	бит_ОбщегоНазначенияКлиент.ОткрытьФормуКонстант("СистемаЛицензирования");
...
// Процедура открывает форму списка.
//
// Параметры:
//  Результат				- Булево.
