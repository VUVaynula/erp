...
Процедура НастроитьФормуПоТипуГрузоперевозки()
	...
	Заголовок = СтрШаблон(НСтр("ru = '%1: Параметры авторизации';
								|en = '%1: Authorization parameters'"), ПредставлениеГрузоперевозчика);
	...
	Если ЭтоКурьерика Или ЭтоСДЭК Тогда
		...
		Элементы.ПодсказкаФормы.Заголовок = СтрШаблон(НСтр(
			"ru = 'Для подключения организации %1 к сервису %2, выполните следующие шаги:';
			|en = 'To connect the %1 company to %2:'"), Организация,
