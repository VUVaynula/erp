...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ВидРасположенияДанных   = Параметры.ВидРасположенияДанных;
	// ПодключениеАктивировано = Параметры.ПодключениеАктивировано;
	...
	Если ХранилищеДеревоОбъектов = Неопределено Тогда
		...
		Иначе	
			...
			ИЛИ ХранилищеДеревоОбъектов = Неопределено Тогда
				...
				СоединениеИБ = бит_ПолучениеДанныхВнешниеИсточникиСервер.ПолучитьСоединениеИБ(СостояниеПодключенияКОМ, 
																					ВидТекущейИБ, 
																					ВидПодключеннойИБ, 
																					"КОМ",
																					АдресКомОбъекта,
...
Процедура СформироватьДеревоОбъектов(ИспользоватьКОМ, СоединениеИБ, ВидТекущейИБ, ВидПодключеннойИБ)
	...
	Для каждого ЭлПеречисление Из ЗначенияПеречисления Цикл
		...
		КартинкаВида = бит_ПолучениеДанныхВнешниеИсточникиСервер.ОпределитьКартинкуВидаМетаданных(ВидОбъекта);
...
// 
&НаСервере
Процедура ДобавитьСтрокуДерева(СтрокаВерх, ИмяОбъекта, ВидОбъекта, Синоним = "", Картинка = Неопределено)
...
Функция ОпределитьВидОбъектаПоИмениКласса(ИмяКлассаАнгл)
	...
	ИмяКласса = бит_ПолучениеДанныхВнешниеИсточникиСервер.ПеревестиИмяКласса(ИмяКлассаАнгл);
...
Процедура СформироватьДеревоОбъектовHTTP(НастройкаПодключения)
	...
	ДобавитьСтрокуДерева(ДеревоВрем, "Справочник", ТекВид,, бит_ПолучениеДанныхВнешниеИсточникиСервер.ОпределитьКартинкуВидаМетаданных(ТекВид));
	
	ТекВид = Перечисления.бит_мпд_ВидыОбъектовМетаданных.Перечисление;
	ДобавитьСтрокуДерева(ДеревоВрем, "Перечисление", ТекВид,, бит_ПолучениеДанныхВнешниеИсточникиСервер.ОпределитьКартинкуВидаМетаданных(ТекВид));
	
	ТекВид = Перечисления.бит_мпд_ВидыОбъектовМетаданных.Документ;
	ДобавитьСтрокуДерева(ДеревоВрем, "Документ", ТекВид,, бит_ПолучениеДанныхВнешниеИсточникиСервер.ОпределитьКартинкуВидаМетаданных(ТекВид));
	
	ТекВид = Перечисления.бит_мпд_ВидыОбъектовМетаданных.ПланСчетов;
	ДобавитьСтрокуДерева(ДеревоВрем, "ПланСчетов", ТекВид,, бит_ПолучениеДанныхВнешниеИсточникиСервер.ОпределитьКартинкуВидаМетаданных(ТекВид));
	
	ТекВид = Перечисления.бит_мпд_ВидыОбъектовМетаданных.ПланВидовХарактеристик;
	ДобавитьСтрокуДерева(ДеревоВрем, "ПланВидовХарактеристик", ТекВид,, бит_ПолучениеДанныхВнешниеИсточникиСервер.ОпределитьКартинкуВидаМетаданных(ТекВид));
		...
		Пока Чтение.Прочитать() Цикл
			...
			Если Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента 
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					ДобавитьСтрокуДерева(СтрокаКоллекция, ИмяОбъектаПолное, ВидОбъекта, ИмяОбъекта);
