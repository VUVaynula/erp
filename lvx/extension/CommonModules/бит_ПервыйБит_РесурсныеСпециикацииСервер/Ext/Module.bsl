...
// {{ПервыйБИТ ЛистТребований [ЛТ6.12]  [VUVaynula@1cbit.ru] [20.04.2026 15:05:23] +        
    Перем РесурснаяСпецификацияОбъект;
	...
	Если Характеристика = Неопределено Тогда
		мХарактеристик = ПолучитьХарактеристикиНоменклатуры(Номенклатура);
	Иначе                
		мХарактеристик = Новый Массив;
		мХарактеристик.Добавить(Характеристика);
	КонецЕсли;
	
	Для каждого ТекХарактеристика из мХарактеристик Цикл
		// выгрузим все доп параметры характеристик
		СоответствиеСвойств = новый Соответствие;   
		Для каждого текСвойство из ТекХарактеристика.ДополнительныеРеквизиты Цикл
			СоответствиеСвойств.Вставить(текСвойство.Свойство, текСвойство.Значение);
		КонецЦикла; 
		Если СоответствиеСвойств.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		мПроверенныхНоменклатур = Новый Массив;  
		мПроверенныхНоменклатур.Добавить(Номенклатура);
		ОбработатьДеревоОбъект(ДеревоСпецификаций, СоответствиеСвойств, мПроверенныхНоменклатур);
	КонецЦикла;
	
// }}ПервыйБИТ ЛистТребований [ЛТ6.12]  [VUVaynula@1cbit.ru] [20.04.2026 15:05:23] -
КонецПроцедуры  

Процедура ОбработатьДеревоОбъект(ДеревоОбъект, Знач СоответствиеСвойств, мПроверенныхНоменклатур)

	Для каждого ТекСтрока Из ДеревоОбъект.Строки Цикл 
		Если ЗначениеЗаполнено(ТекСтрока.Номенклатура) 
			и мПроверенныхНоменклатур.Найти(ТекСтрока.Номенклатура) = Неопределено
			И Не ЗначениеЗаполнено(ТекСтрока.Характеристика) Тогда
			
			ПроверитьНоменклатуруНаНаличиеХарактеристик(ТекСтрока.Номенклатура, СоответствиеСвойств, мПроверенныхНоменклатур);
		КонецЕсли;
		ОбработатьДеревоОбъект(ТекСтрока, СоответствиеСвойств, мПроверенныхНоменклатур);
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьНоменклатуруНаНаличиеХарактеристик(Знач Номенклатура, Знач СоответствиеСвойств, мПроверенныхНоменклатур)
	
	ТаблицаХарактеристик = ПолучитьХарактеристикиНоменклатуры(Номенклатура, Ложь);   
	Если ТаблицаХарактеристик.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;      
	
	СпрХарактеристика = Справочники.ХарактеристикиНоменклатуры.СоздатьЭлемент();
	ЭталонСвойств = СпрХарактеристика.ДополнительныеРеквизиты.Выгрузить(, "Свойство, Значение");
	ЭталонСвойств.Свернуть("Свойство, Значение");
	
	Для Каждого Пара Из СоответствиеСвойств Цикл
		СтрокаТаблицы = ЭталонСвойств.Добавить();
		СтрокаТаблицы.Свойство = Пара.Ключ;
		СтрокаТаблицы.Значение = Пара.Значение;
	КонецЦикла;
	
	ВТХарактеристики = ТаблицаХарактеристик.Скопировать(, "Характеристика");
	ВТХарактеристики.Свернуть("Характеристика");
     // -------------------------------------
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВТ_Характеристики", ВТХарактеристики);
	Запрос.УстановитьПараметр("ВТ_ЭталонСвойств", ЭталонСвойств);
	...
	|ИЗ
	|	&ВТ_Характеристики КАК Хар
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Эталон.Свойство КАК Свойство,
	|	Эталон.Значение КАК Значение
	|ПОМЕСТИТЬ ВТ_ЭталонСвойств
	|ИЗ
	|	&ВТ_ЭталонСвойств КАК Эталон
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДопРекв.Ссылка КАК Характеристика,
	|	ДопРекв.Свойство КАК Свойство,
	|	ДопРекв.Значение КАК Значение
	|ПОМЕСТИТЬ ВТ_ФактическиеСвойства
	|ИЗ
	|	Справочник.ХарактеристикиНоменклатуры.ДополнительныеРеквизиты КАК ДопРекв
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Характеристики КАК Хар
	|		ПО ДопРекв.Ссылка = Хар.Характеристика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Эталон.Свойство КАК Свойство,
	|	Эталон.Значение КАК Значение
	|ПОМЕСТИТЬ ВТ_ОжидаемыеПары
	|ИЗ
	|	ВТ_Характеристики КАК Хар
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ЭталонСвойств КАК Эталон
	|		ПО ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Ожидаемые.Характеристика КАК Характеристика
	|ИЗ
	|	ВТ_ОжидаемыеПары КАК Ожидаемые
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ФактическиеСвойства КАК Факт
	|		ПО Ожидаемые.Характеристика = Факт.Характеристика
	|			И Ожидаемые.Свойство = Факт.Свойство
	|			И Ожидаемые.Значение = Факт.Значение
	|ГДЕ
	|	Факт.Характеристика ЕСТЬ NULL";
	
	Выборка = Запрос.Выполнить().Выгрузить();
	Если Выборка.Количество() Тогда
		Результат = Выборка.ВыгрузитьКолонку("Характеристика");
	КонецЕсли;
	
	СоздатьХарактеристики(Результат, ЭталонСвойств, Номенклатура, мПроверенныхНоменклатур);

КонецПроцедуры

Процедура СоздатьХарактеристики(мХарактеристик, ЭталонСвойств, Номенклатура, мПроверенныхНоменклатур)
	
		СпрОбъект = Справочники.ХарактеристикиНоменклатуры.СоздатьЭлемент();
		СпрОбъект.Владелец = ОпределитьВладельцаХарактеристики(Номенклатура);
		СпрОбъект.ДополнительныеРеквизиты.Загрузить(ЭталонСвойств);
		СпрОбъект.ВидНоменклатуры = Номенклатура.ВладелецХарактеристик;
		...
		СпрОбъект.Наименование = НоменклатураСервер.НаименованиеПоШаблону(РеквизитыВидаНоменклатуры.ШаблонРабочегоНаименованияХарактеристики,
																		СпрОбъект);
		СпрОбъект.НаименованиеПолное = НоменклатураСервер.НаименованиеПоШаблону(РеквизитыВидаНоменклатуры.ШаблонНаименованияДляПечатиХарактеристики,
																		СпрОбъект);
...
Функция ПолучитьХарактеристикиНоменклатуры(Владелец, ВозвратМассивом = Истина)
	...
	Если тзРезультат.Количество() И ВозвратМассивом = Истина Тогда
		тзРезультат = тзРезультат.ВыгрузитьКолонку("Характеристика");
	КонецЕсли;
...
Функция ОпределитьВладельцаХарактеристики(Владелец)
