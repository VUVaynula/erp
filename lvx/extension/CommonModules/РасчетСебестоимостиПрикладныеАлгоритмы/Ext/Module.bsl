
&ИзменениеИКонтроль("ЗаписатьНаборЗаписей")
Процедура бит_ЗаписатьНаборЗаписей(НаборЗаписей, Замещать, ОписаниеОшибокЗаписи, КоличествоДополнительныхПопытокЗаписи, ПараметрыЗаписи)
	
	Регистратор = НаборЗаписей.Отбор.Регистратор.Значение;
	
	КоличествоПопытокЗаписи = ?(ПараметрыЗаписи.Свойство("КоличествоПопытокЗаписи"), ПараметрыЗаписи.КоличествоПопытокЗаписи, 1);

	Для НомерПопытки = 1 По КоличествоПопытокЗаписи Цикл

		КоличествоДополнительныхПопытокЗаписи = КоличествоДополнительныхПопытокЗаписи + ?(НомерПопытки = 1, 0, 1);

		Попытка

			Если ПараметрыЗаписи.КоличествоРегистраторовВТранзакцииЗаписи > 0 // в этом случае выполнена проверка изменения всех документов в транзакции записи 
				ИЛИ НЕ РегистраторИзмененПриРасчете(Регистратор, НаборЗаписей, ОписаниеОшибокЗаписи, ПараметрыЗаписи) Тогда
				ДатыЗапретаИзмененияУТ.ОтключитьПроверкуДатыЗапретаИзменения(НаборЗаписей);
				НаборЗаписей.Записать(Замещать);
			КонецЕсли;
			
			#Вставка // {{ПервыйБИТ ЛистТребований [ЛТ8.1] [ASHolyavko@1cbit.ru] [08.05.2026] +        
			
			Если Строка(НаборЗаписей) = "РегистрНакопленияНаборЗаписей.СебестоимостьТоваров" Тогда
				Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.ПеремещениеТоваров")  Тогда 				
					Документы.ПеремещениеТоваров.бит_СформироватьДвижения(Регистратор, НаборЗаписей.Выгрузить());
				ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.СписаниеНедостачТоваров") Тогда
					Документы.СписаниеНедостачТоваров.бит_СформироватьДвижения(Регистратор, НаборЗаписей.Выгрузить());	
				ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
					Документы.РеализацияТоваровУслуг.бит_СформироватьДвижения(Регистратор, НаборЗаписей.Выгрузить());
				ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.ПередачаТоваровМеждуОрганизациями") Тогда
					Документы.ПередачаТоваровМеждуОрганизациями.бит_СформироватьДвижения(Регистратор, НаборЗаписей.Выгрузить());	
				КонецЕсли;
				
				//МенеджерДокумента = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Регистратор);
				//МенеджерДокумента.бит_СформироватьДвижения(Регистратор, НаборЗаписей.Выгрузить())
				
			КонецЕсли;
			
			#КонецВставки // }}ПервыйБИТ ЛистТребований [ЛТ8.1] [ASHolyavko@1cbit.ru] [08.05.2026] -
			
			Прервать; // больше выполнять запись не требуется

		Исключение

			Если НомерПопытки <> КоличествоПопытокЗаписи Тогда
				Продолжить; // ошибка будет зарегистрирована на последней неудачной попытке
			КонецЕсли;

			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка записи движений документа ""%1"" по регистру ""%2"":
			|%3';
			|en = 'An error occurred while writing the ""%1"" document register records by the ""%2"" register:
			|%3'", ОбщегоНазначения.КодОсновногоЯзыка()),
			СокрЛП(Регистратор),
			НаборЗаписей.Метаданные().Имя,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

			ЗарегистрироватьОшибкуЗаписиДвижений(
			Регистратор,
			ПараметрыЗаписи,
			НСтр("ru = 'При записи движений диагностированы ошибки';
			|en = 'Errors were found when saving records'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстДляПротокола,
			ОписаниеОшибокЗаписи);

		КонецПопытки;

	КонецЦикла;

	НаборЗаписей.Очистить();

КонецПроцедуры
