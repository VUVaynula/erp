...
Процедура ДорисоватьФорму() 
	...
	Для сч = 0 По СоответствиеСвойств.Количество() - 1 Цикл
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Колонка" + сч, Новый ОписаниеТипов("Строка"), "ПравилаНаследования", СоответствиеСвойств[сч].СвойствоМатериалаПредставление));	
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("Изменена" + сч, Новый ОписаниеТипов("Строка"), "ПравилаНаследования", СоответствиеСвойств[сч].СвойствоМатериалаПредставление));	
	КонецЦикла;
	ИзменитьРеквизиты(МассивДобавляемыхРеквизитов);     
	// элементыФормы     
	Для сч = 0 По СоответствиеСвойств.Количество() - 1 Цикл
		НоваяКолонка = Элементы.Вставить("ПравилаНаследования" + "Колонка" + сч, Тип("ПолеФормы"), Элементы.ПравилаНаследования, Элементы.ПравилаНаследованияКоличество);
		НоваяКолонка.Заголовок = СоответствиеСвойств[0].СвойствоМатериалаПредставление;
		НоваяКолонка.ПутьКДанным = "ПравилаНаследования.Колонка" + сч;
		НоваяКолонка.Вид = ВидПоляФормы.ПолеВвода; 
		НоваяКолонка.УстановитьДействие("НачалоВыбора", "НачалоВыбораЭлементаПравилаНаследования");
...
Процедура НастроитьВидимостьВсеПараметры()
	...
	Для сч = 0 По СоответствиеСвойств.Количество() - 1 Цикл
		Элементы["ПравилаНаследованияИзменена" + сч].Видимость = ПравилаНаследования.Итог("Изменена" + сч); 	
	КонецЦикла;
...
Процедура Удалить(Команда)      
	...
	ТекущаяСтрока = Элементы.ПравилаНаследования.ТекущаяСтрока;
	Если ТекущаяСтрока <> Неопределено Тогда
        ПравилаНаследования.Удалить(ПравилаНаследования.НайтиПоИдентификатору(ТекущаяСтрока));
	КонецЕсли;  
	ПересчитатьНумерациюСтрок();
...
Процедура СоответствиеСвойствПриИзменении(Элемент)
	...
	Индекс = ТД.ПолучитьИдентификатор();
	СтрокаИсходныхДанных = СоответствиеСвойствИсходныеЗначения.НайтиПоИдентификатору(Индекс);
...
&НаСервере
Процедура ПеречитатьЗначенияНаСервере()
...
Процедура Добавить(Команда)              
	...
	нов.Количество = 1;
	Для сч = 0 По СоответствиеСвойств.Количество() - 1 Цикл
		нов["Колонка" + сч] = СоответствиеСвойств[сч].ЗначениеСвойства;
		нов["Изменена" + сч] = СоответствиеСвойств[сч].ЗначениеСвойства <> СоответствиеСвойствИсходныеЗначения[сч].ЗначениеСвойства;
	КонецЦикла;
