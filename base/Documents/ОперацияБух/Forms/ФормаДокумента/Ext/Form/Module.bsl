...
Процедура СоздатьЭлементыФормы()
	...
	Для каждого СтрокаРегистра Из Регистры Цикл
		...
		Если СтрокаРегистра.Имя = "Хозрасчетный"
			ИЛИ СтрНайти(СтрокаРегистра.Имя, "бит_") = 1 Тогда
			Продолжить;
		КонецЕсли;
        // бит_Финанс изменения кода. Конец.
...
Процедура бит_УстановитьДействияФормы()
	...
	бит_РаботаСДиалогамиСервер.ВыполнитьПерехватСобытий(ЭтотОбъект, События, "бит_Обработчики");
...
Процедура бит_ЗаполнитьДвиженияПоВыбраннымДанным(ВыбранноеЗначение, ОчищатьДанные)
	...
	Обработки.бит_ПолучениеДанныхРегистровБухгалтерии.ЗаполнитьДвиженияХоз(ВыбранноеЗначение.ХранилищеДанные, 
		ХозрасчетныйНаборЗаписей, КоличествоСубконто);
...
Функция бит_ПолучитьКодВыполненияТиповогоОбработчика()
	...
	СтруктураТиповыхОбработчиков = ЭтотОбъект.бит_Обработчики.Форма;
...
//
&НаКлиенте
Процедура Подключаемый_бит_ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
