...
Процедура ПриПолученииДанных(ТекущийОбъект)
	...
	ДоступноОтклонениеЗаявки = Не (Объект.ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.Опоздание
		Или Объект.ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.ЛичныеДела);
...
Функция УстановитьТекстПоясненияПорядкаВыполнения()
	...
	Иначе
		...
		Строка2 = НСтр("ru = 'после этого заявку можно отметить как выполненную.';
						|en = 'после этого заявку можно отметить как выполненную.'");
		ТекстПояснения = СтрШаблон("%1 %2", Строка1, Строка2);
	КонецЕсли;
