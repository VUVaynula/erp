...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.ГруппаЗаменаОдинКОдному.Видимость = Ложь;
	...
	ЗаполнитьДанныеРеквизитовПриВариантеОдинКОдному();
		...
		Если Не ЗначениеВыбора.МножественноеУказание Тогда
			...
			Если Объект.ОбластиДействия.Количество() > 0 Тогда
				...
				Если СпецификацияДоВыбора <> ЗначениеВыбора.Спецификация Или ЭтапДоВыбора <> ЗначениеВыбора.Этап Тогда
					...
					Материал = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
	
		Если НЕ ХарактеристикиИспользуются Тогда
			МассивНепроверяемыхРеквизитов.Добавить("Характеристика");
		КонецЕсли;
		
		Если НЕ ХарактеристикиИспользуютсяАналог Тогда
			МассивНепроверяемыхРеквизитов.Добавить("ХарактеристикаАналог");
		КонецЕсли;
		
	Иначе
		
		МассивНепроверяемыхРеквизитов.Добавить("Материал");
		МассивНепроверяемыхРеквизитов.Добавить("Характеристика");
		МассивНепроверяемыхРеквизитов.Добавить("КоличествоУпаковок");
		МассивНепроверяемыхРеквизитов.Добавить("Аналог");
		МассивНепроверяемыхРеквизитов.Добавить("ХарактеристикаАналог");
		МассивНепроверяемыхРеквизитов.Добавить("КоличествоУпаковокАналог");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ПеренестиДанныеВТабличныеЧасти(Отказ, ПараметрыЗаписи);
...
Асинх Процедура ВариантЗаменыПриИзменении(Элемент)
	...
	ОчиститьСообщения();
	ЗадатьВопрос = Ложь;
	ЕстьДанныеОбластиДействия = Ложь;
	ИзменениеДоступно = Истина;
	ВариантСписок = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.СписокАналогов");
	ВариантОдинНаОдин = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин"); 
	
	Если ВариантЗаменыДоИзменения <> ВариантОдинНаОдин
	   И Объект.ВариантЗамены = ВариантОдинНаОдин Тогда
	
		ЕстьЛишниеСтрокиМатериалов = Объект.Материалы.Количество() > 1;
		ЕстьЛишниеСтрокиАналогов = Объект.Аналоги.Количество() > 1;
		
		Если ЕстьЛишниеСтрокиМатериалов И ЕстьЛишниеСтрокиАналогов Тогда
			ТекстПредупреждения = НСтр("ru = 'Для переключения в режим ""Один на один"" должна быть только одна строка материалов и аналогов.';
										|en = 'To switch to ""One to one"" mode, leave only one row of substitutes and materials.'");
		ИначеЕсли ЕстьЛишниеСтрокиМатериалов И Не ЕстьЛишниеСтрокиАналогов Тогда	
			ТекстПредупреждения = НСтр("ru = 'Для переключения в режим ""Один на один"" должна быть только одна строка материалов.';
										|en = 'To switch to ""One to one"" mode, leave only one row of materials.'");
		ИначеЕсли Не ЕстьЛишниеСтрокиМатериалов И ЕстьЛишниеСтрокиАналогов Тогда	
			ТекстПредупреждения = НСтр("ru = 'Для переключения в режим ""Один на один"" должна быть только одна строка аналогов.';
										|en = 'To switch to ""One to one"" mode, leave only one row of substitutes.'");
		КонецЕсли;
		
		Если ЕстьЛишниеСтрокиМатериалов Или ЕстьЛишниеСтрокиАналогов Тогда
	...
	ИначеЕсли ВариантЗаменыДоИзменения <> ВариантСписок
	   И Объект.ВариантЗамены = ВариантСписок Тогда
		...
		Для каждого СтрокаОбластиДействия Из Объект.ОбластиДействия Цикл
			...
			Если ЗначениеЗаполнено(СтрокаОбластиДействия.НаправлениеДеятельности)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.Подразделение)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.Изделие)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.ХарактеристикаИзделия)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.ЗаказНаПроизводство)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.Спецификация)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.Этап)
			 ИЛИ ЗначениеЗаполнено(СтрокаОбластиДействия.ЗаказКлиента) Тогда
			
				ЕстьДанныеОбластиДействия = Истина;
				Прервать;
		...
		Если ЕстьСтрокиАналогов И ЕстьДанныеОбластиДействия Тогда
		...
		ИначеЕсли ЕстьСтрокиАналогов И НЕ ЕстьДанныеОбластиДействия Тогда
			ТекстВопроса = НСтр("ru = 'Изменение варианта замены приведет к потере данных набора аналогов. Продолжить?';
								|en = 'If you change the replacement option, you will lose the substitute set data. Continue?'");
		ИначеЕсли Не ЕстьСтрокиАналогов И ЕстьДанныеОбластиДействия Тогда
			ТекстВопроса = НСтр("ru = 'Изменение варианта замены приведет к невозможности ввода области действия. Продолжить?';
								|en = 'If you change the replacement option, you will not be able to enter the scope. Continue?'");
		Иначе
			ЗадатьВопрос = Ложь;
		КонецЕсли;
	...
	Если ЗадатьВопрос Тогда
		...
		Если Результат = КодВозвратаДиалога.Да Тогда
			ВыполнитьДействияПриИзмененииВариантаЗамены(ЕстьДанныеОбластиДействия);
		Иначе
			Объект.ВариантЗамены = ВариантЗаменыДоИзменения;
		КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовСтраницыЗаменаОдинКОдному

&НаКлиенте
Процедура НоменклатураПриИзменении(Элемент)
	
	МатериалАналогПриИзмененииНаСервере(Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура АналогПриИзменении(Элемент)
	МатериалАналогПриИзмененииНаСервере(Ложь);
КонецПроцедуры
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("Статус")
		...
		ИменаЭлементов.Добавить("ГруппаЗаменаОдинКОдному");
...
Процедура ВыполнитьДействияПриИзмененииВариантаЗамены(ОчиститьОбластиДействия = Ложь)
	...
	ПриИзмененииДанныхПриВариантеОдинКОдному();
	НастроитьВидимостьСтраницыЗамены(ЭтотОбъект);
	НастроитьВидимостьОбластиДействия(ЭтотОбъект);
	...
	Если Объект.ВариантЗамены <> ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин")
...
Процедура НастроитьВидимостьСтраницыЗамены(Форма)
	...
	Если ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
		
		Элементы.ГруппаЗаменаОдинКОдному.Видимость = Истина;
		Элементы.ГруппаЗаменаНабор.Видимость = Ложь;
		Элементы.ГруппаЗаменаСписок.Видимость = Ложь;
		
	ИначеЕсли ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.НаборНаНабор") Тогда
...
Процедура ОбработатьДанныеТабличныхЧастейПриИзмененииВариантаЗамены()
	...
	Если Объект.ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
		...
		Аналог = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
		ХарактеристикаАналог = ПредопределенноеЗначение("Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка");
		УпаковкаАналог = ПредопределенноеЗначение("Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка");
		КоличествоУпаковокАналог = 0;
		
		Если Объект.Материалы.Количество() > 0 Тогда
		
			СтрокаМатериалов = Объект.Материалы[0];
			Материал = СтрокаМатериалов.Номенклатура;
			Характеристика = СтрокаМатериалов.Характеристика;
			КоличествоУпаковок = СтрокаМатериалов.КоличествоУпаковок;
			Упаковка = СтрокаМатериалов.Упаковка;
		
		КонецЕсли;
		
		Если Объект.Аналоги.Количество() > 0 Тогда
		
			СтрокаАналогов = Объект.Аналоги[0];
			Аналог = СтрокаАналогов.Номенклатура;
			ХарактеристикаАналог = СтрокаАналогов.Характеристика;
			КоличествоУпаковокАналог = СтрокаАналогов.КоличествоУпаковок;
			УпаковкаАналог = СтрокаАналогов.Упаковка;
		
		КонецЕсли;
		
	ИначеЕсли Объект.ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.НаборНаНабор")
		    И ВариантЗаменыДоИзменения = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
	
		Если Объект.Материалы.Количество() > 0 Тогда
		
			СтрокаМатериалов = Объект.Материалы[0];
			СтрокаМатериалов.Номенклатура = Материал;
			СтрокаМатериалов.Характеристика = Характеристика;
			СтрокаМатериалов.КоличествоУпаковок = КоличествоУпаковок;
			СтрокаМатериалов.Упаковка = Упаковка;
		...
		Если Объект.Аналоги.Количество() > 0 Тогда
			...
			СтрокаАналогов = Объект.Аналоги[0];
			СтрокаАналогов.Номенклатура = Аналог;
			СтрокаАналогов.Характеристика = ХарактеристикаАналог;
			СтрокаАналогов.КоличествоУпаковок = КоличествоУпаковокАналог;
			СтрокаАналогов.Упаковка = УпаковкаАналог;
				
		КонецЕсли;
		
	ИначеЕсли Объект.ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.СписокАналогов") Тогда
...
&НаСервере
Функция ДанныеНоменклатурыСтруктурой(ЭтоМатериал)
	...
	Если ЭтоМатериал Тогда
		...
		СписокРеквизитов.Вставить("Номенклатура",               "Материал");
		СписокРеквизитов.Вставить("ХарактеристикиИспользуются", "ХарактеристикиИспользуются");
		СписокРеквизитов.Вставить("Характеристика",             "Характеристика");
		СписокРеквизитов.Вставить("Упаковка",                   "Упаковка");
		СписокРеквизитов.Вставить("КоличествоУпаковок",         "КоличествоУпаковок");
		
	Иначе
		
		СписокРеквизитов.Вставить("Номенклатура",               "Аналог");
		СписокРеквизитов.Вставить("ХарактеристикиИспользуются", "ХарактеристикиИспользуютсяАналог");
		СписокРеквизитов.Вставить("Характеристика",             "ХарактеристикаАналог");
		СписокРеквизитов.Вставить("Упаковка",                   "УпаковкаАналог");
		СписокРеквизитов.Вставить("КоличествоУпаковок",         "КоличествоУпаковокАналог");
	
	КонецЕсли;
	
	Для каждого КлючЗначение Из СписокРеквизитов Цикл
		СтруктураСтроки.Вставить(КлючЗначение.Ключ,  ЭтотОбъект[КлючЗначение.Значение]);
	КонецЦикла;
...
&НаСервере
Процедура МатериалАналогПриИзмененииНаСервере(ЭтоМатериал)

	СтруктураСтроки = ДанныеНоменклатурыСтруктурой(ЭтоМатериал);
	...
	Если ЭтоМатериал Тогда
		ЭлементХарактеристики 	= Элементы.Характеристика;
		ЭлементУпаковки 		= Элементы.Упаковка;
	Иначе
		ЭлементХарактеристики 	= Элементы.ХарактеристикаАналог;
		ЭлементУпаковки 		= Элементы.УпаковкаАналог;
	КонецЕсли;
	
	Если СтруктураСтроки.ХарактеристикиИспользуются Тогда
		
		ЭлементХарактеристики.Доступность					= Истина;
		ЭлементХарактеристики.АвтоОтметкаНезаполненного 	= Истина;
		ЭлементХарактеристики.ПодсказкаВвода  				= "";
	Иначе
		
		ЭлементХарактеристики.Доступность		= Ложь;
		ЭлементХарактеристики.ПодсказкаВвода	= НСтр("ru = '<характеристики не используются>';
	...
	Иначе
		ХарактеристикиИспользуютсяАналог	= СтруктураСтроки.ХарактеристикиИспользуются;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СтруктураСтроки.Упаковка) Тогда
		ОтобразитьИнформациюОЕдиницеХранения(СтруктураСтроки.Номенклатура, ЭлементУпаковки);
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ОтобразитьИнформациюОЕдиницеХранения(Номенклатура, ПолеВводаУпаковки)

	Справочники.УпаковкиЕдиницыИзмерения.ОтобразитьИнформациюОЕдиницеХранения(Номенклатура, ПолеВводаУпаковки);

КонецПроцедуры

&НаСервере
Процедура ПеренестиДанныеВТабличныеЧасти(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
		...
		СтрокаМатериалы = Объект.Материалы.Добавить();
		СтрокаМатериалы.Номенклатура = Материал;
		СтрокаМатериалы.Характеристика = Характеристика;
		СтрокаМатериалы.КоличествоУпаковок = КоличествоУпаковок;
		СтрокаМатериалы.Упаковка = Упаковка;
		СтрокаМатериалы.ИдентификаторСтроки = Новый УникальныйИдентификатор;
		
		Объект.Аналоги.Очистить();
		
		СтрокаАналоги = Объект.Аналоги.Добавить();
		СтрокаАналоги.Номенклатура = Аналог;
		СтрокаАналоги.Характеристика = ХарактеристикаАналог;
		СтрокаАналоги.КоличествоУпаковок = КоличествоУпаковокАналог;
		СтрокаАналоги.Упаковка = УпаковкаАналог;
		СтрокаАналоги.ИдентификаторСтроки = Новый УникальныйИдентификатор;
...
&НаКлиенте
Процедура ПриИзмененииДанныхПриВариантеОдинКОдному()
	
	Если Объект.ВариантЗамены <> ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьРеквизитыПриВариантеОдинКОдномуНаСервере();

КонецПроцедуры

&НаСервере
Процедура ЗаполнитьРеквизитыПриВариантеОдинКОдномуНаСервере()
...
&НаСервере
Процедура ЗаполнитьДанныеРеквизитовПриВариантеОдинКОдному()

	Если Объект.ВариантЗамены = ПредопределенноеЗначение("Перечисление.ВариантЗаменыАналогов.ОдинНаОдин") Тогда
	
		Если Объект.Материалы.Количество() > 0 Тогда
		
			СтрокаМатериалы = Объект.Материалы[0];
			
			Материал = СтрокаМатериалы.Номенклатура;
			Характеристика = СтрокаМатериалы.Характеристика;
			КоличествоУпаковок = СтрокаМатериалы.КоличествоУпаковок;
			Упаковка = СтрокаМатериалы.Упаковка;
		
		КонецЕсли;
		
		Если Объект.Аналоги.Количество() > 0 Тогда
		
			СтрокаАналоги = Объект.Аналоги[0];
		
			Аналог = СтрокаАналоги.Номенклатура;
			ХарактеристикаАналог = СтрокаАналоги.Характеристика;
			КоличествоУпаковокАналог = СтрокаАналоги.КоличествоУпаковок;
			УпаковкаАналог = СтрокаАналоги.Упаковка;
