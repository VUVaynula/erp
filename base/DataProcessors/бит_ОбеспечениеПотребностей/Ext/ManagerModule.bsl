...
Процедура ТаблицаСпособовОбеспечения(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.СпособОбеспечения = Неопределено Тогда
		СхемаКомпоновкиДанных = Обработки.бит_ОбеспечениеПотребностей.ПолучитьМакет("МакетКомпоновкиДляСерверныхОтборов");
...
Процедура СформироватьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:18:42] +        
	ТекстыЗапроса.Добавить(ТаблицаЗаказыНаПеремещениеПоОднойНоменклатуреГПКоформлению(), "ТаблицаЗаказыНаПеремещениеГП");
	Запрос.УстановитьПараметр("бит_ГФУ", Справочники.бит_ЗначенияПоУмолчанию.ПолучитьМассивЗначений(Справочники.бит_ЗначенияПоУмолчанию.ГФУНоменклатурыГП, , Истина));
	// }}ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:18:42] -
	...
	КонецЦикла;
	// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:19:38] +        
	ПоляКлючаЗаказа = "СкладОтправитель, СкладПолучатель, Организация, Подразделение, НаправлениеДеятельности, Номенклатура, Характеристика";
	ПоляКлючаУдаляемые = "СкладОтправитель, СкладПолучатель, Организация, Подразделение, НаправлениеДеятельности";
	МассивТаблиц = ОбеспечениеСервер.РазбитьТаблицуПоЗначениюКлюча(Таблицы.ТаблицаЗаказыНаПеремещениеГП, ПоляКлючаЗаказа, ПоляКлючаУдаляемые);
	Приоритет = Справочники.бит_ЗначенияПоУмолчанию.ПолучитьМассивЗначений(Справочники.бит_ЗначенияПоУмолчанию.ПриоритетЗаказаНаПеремещение, , Истина); 
	Для Каждого Элемент Из МассивТаблиц Цикл
		
		ДанныеЗаполнения = Элемент.Ключ;
		ДанныеЗаполнения.Вставить("Товары", Элемент.Таблица);
		ДанныеЗаполнения.Вставить("Комментарий", "Сформирован автоматически обработкой `Формирование заказов на перемещение готовой продукции`");
		Документ = ПолучитьДокумент(Документы.ЗаказНаПеремещение, ДанныеЗаполнения, Перечисления.СтатусыВнутреннихЗаказов.Закрыт, Приоритет);
...
Функция ТаблицаЗаказыНаПеремещениеПоОднойНоменклатуреГПКоформлению()
	// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:30:35] +        
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВЫБОР КОГДА Т.ИсточникОбеспечения = НЕОПРЕДЕЛЕНО ТОГДА
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [12.01.2026 21:37:56] +        
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:33:12] +        
		|	И Т.Номенклатура.ГруппаФинансовогоУчета = &бит_ГФУ
		// }}ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:33:12] -
...
Функция ТаблицаЗаказыНаПеремещениеКОформлению()
	...
	ТекстЗапроса =
		...
		// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:31:47] +        
		|	И НЕ Т.Номенклатура.ГруппаФинансовогоУчета = &бит_ГФУ
		// }}ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:31:47] -
...
Функция ПолучитьДокумент(МенеджерДокументов, ДанныеЗаполнения, Статус, Приоритет = Неопределено)
	...
	// {{ПервыйБИТ ЛистТребований [ЛТ5.1]  [VUVaynula@1cbit.ru] [09.01.2026 21:22:26] +        
