...
Процедура ЗакончитьОтгрузку(Команда)
	...
	Если ТоварыОтгружено.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru = 'Не возможно закончить отгрузку, пока не отгружено ни одного товара.';
							|en = 'It is not possible to finish the shipment until at least one item has been shipped.'");
	...
	Если ТоварыКОтгрузке.Количество() > 0 Или ТоварыОтобрано.Количество() > 0 Тогда
		Если ИспользоватьАдресноеХранение Тогда
			ПоказатьВопрос(Оповещение, НСтр("ru = 'Завершить отгрузку? Товары, оставшиеся на вкладках «Отобрать» и «Отобрано» не будут отгружены.';
											|en = 'Finish the shipment? Items remaining on the ""Select"" and ""Selected"" tabs will not be shipped.'"), СписокЗначений, 0);
		Иначе
	...
	Для Каждого Строка Из Ссылка.ТоварыПоРаспоряжениям Цикл
		...
		Если Количество > Строка.Количество Тогда
			...
			ТекстОшибки = НСтр("ru = 'Отгружаемое количество товара %Товар% не соответствует количеству в распоряжении';
								|en = 'The quantity of item %Товар% being shipped does not match the quantity on hand'");
...
Функция ОтгрузкаРазрешена(СвойстваТовара, Количество)
	...
	Если Количество > ДоступноеКоличество Тогда
		ТекстОшибки = НСтр("ru = 'Отгружаемое количество превышает остатки на складе.';
							|en = 'The quantity being shipped exceeds the warehouse balance.     '");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
