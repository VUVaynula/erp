	...
	Пока Поз <> СтрДлина(СтрокаКода) + 1 Цикл
		...
		Если ТекущийСимвол = "/" Тогда
			...
			Если Не ВСтроке Тогда 
				Если ПолучитьСимвол(СтрокаКода, Поз + 1) = "/" Тогда
					...
					СтрокаКода = Лев(СтрокаКода, Поз - 1) + ОткрывающиеТеги["Комментарий"]
						+ Прав(СтрокаКода, СтрДлина(СтрокаКода) - Поз + 1) + "</font>";
					Возврат СтрокаКода;
				Иначе
					// Это символ деления
		...
		ИначеЕсли ТекущийСимвол = """" Тогда
			...
			Если ВСтроке Тогда
				...
				СтрокаКода = Лев(СтрокаКода, НачалоСтроки - 1) + ОткрывающиеТеги["Строка"]
					+ Сред(СтрокаКода, НачалоСтроки, Поз - НачалоСтроки + 1) + "</font>"
					+ Прав(СтрокаКода, СтрДлина(СтрокаКода) - Поз);
