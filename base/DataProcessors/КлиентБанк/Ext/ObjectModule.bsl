...
Процедура ДобавитьИнформациюОПлатеже(МанифестБанка, ДокументыКВыгрузкеПоБанку)
	...
	Для Каждого Документ Из ДокументыКВыгрузкеПоБанку Цикл
		...
		бит_ЗаявкаНаРДС = бит_Казначейство.ЗаявкаПоДаннымОтраженияФакта(ДокументСсылка);	
		
		Если Не (РеквизитыПлатежа.ОплатаПоЗаявкам ИЛИ ЗначениеЗаполнено(бит_ЗаявкаНаРДС)) Тогда
			Продолжить;
		КонецЕсли;
		...
		// бит_Финанс изменения кода. Начало.
		Если ЗначениеЗаполнено(ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств) Тогда
		// бит_Финанс изменения кода. Конец.
		...
		// бит_Финанс изменения кода. Начало.
		бит_ЗаявкаНаРДС = Неопределено;
		
		ИначеЕсли ЗначениеЗаполнено(бит_ЗаявкаНаРДС) Тогда	
			Контекст = Новый Структура("ЗаявкаНаРасходованиеДенежныхСредств", бит_ЗаявкаНаРДС);
			ЗаполнитьЗначенияСвойств(МанифестДокумента, 
				ИдентификаторИПредставлениеТипаПлатежа(Контекст));
		КонецЕсли;
		// бит_Финанс изменения кода. Конец.
		...
		// бит_Финанс изменения кода. Начало.
		ДобавитьИнформациюОПодтверждающихДокументах(МанифестДокумента, ДокументСсылка, бит_ЗаявкаНаРДС);
		// бит_Финанс изменения кода. Конец.
	КонецЦикла;
...
// бит_Финанс изменения кода. Начало.
Процедура ДобавитьИнформациюОПодтверждающихДокументах(МанифестДокумента, ДокументСсылка, бит_ЗаявкаНаРДС)
// бит_Финанс изменения кода. Конец.
	...
	// бит_Финанс изменения кода. Начало.
	Если ЗначениеЗаполнено(бит_ЗаявкаНаРДС) Тогда
		Запрос.УстановитьПараметр("бит_ЗаявкаНаРДС", бит_ЗаявкаНаРДС);
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПодтверждающиеДокументы.Номер КАК Номер,
		|	ПодтверждающиеДокументы.Дата КАК Дата,
		...
		|ИЗ
		|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК РасшифровкаПлатежа
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.бит_кзн_ЗаявкаНаРасходованиеСредств.ПодтверждающиеДокументы КАК ПодтверждающиеДокументы
		|		ПО (ПодтверждающиеДокументы.Ссылка = &бит_ЗаявкаНаРДС)
		|ГДЕ
		|	РасшифровкаПлатежа.Ссылка = &ДокументСсылка
		|	И НЕ ПодтверждающиеДокументы.Файл ЕСТЬ NULL";
...
Функция МанифестПоУмолчанию()
	...
	СвойстваКорневогоЭлемента.Вставить("Author_Attribute",
		Метаданные.Синоним + ", " + НСтр("ru = 'версия';
										|en = 'version'") + " " + бит_ОбщегоНазначения.МетаданныеВерсия());
