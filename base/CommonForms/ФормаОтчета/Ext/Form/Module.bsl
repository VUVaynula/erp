...
#Область бит_ИзмененияБИТФИНАНС

// бит_Финанс изменения кода. Начало.
&НаКлиенте
Процедура бит_Подключаемый_ПолучитьПараметрыФормированияОтчета(Команда)
	
	АдресПараметровФормирования = бит_ПараметрыФормированияОтчета(ЭтотОбъект.ВладелецФормы.УникальныйИдентификатор);
	Оповестить("ПолученыПараметрыФормированияОтчета", АдресПараметровФормирования);
	Закрыть();
...
Процедура бит_ПриСозданииНаСервереЗавершение()
	...
	Перем КнопкаПолучитьПараметры, КомандаПолучитьПараметры, ПараметрыИсполняемогоОтчета;
	
	Если Параметры.Свойство("бит_ПолучениеПараметров") Тогда
		КомандаПолучитьПараметры = Команды.Добавить("бит_ПолучитьПараметры");
		КомандаПолучитьПараметры.Действие  = "бит_Подключаемый_ПолучитьПараметрыФормированияОтчета";
		КомандаПолучитьПараметры.Заголовок = НСтр("ru='Получить параметры';
												  |en='Get parameters'");		
		КнопкаПолучитьПараметры = Элементы.Добавить("бит_ПолучитьПараметры",Тип("КнопкаФормы"), Элементы.ГруппаГлавное);
		КнопкаПолучитьПараметры.ИмяКоманды        = "бит_ПолучитьПараметры";
		КнопкаПолучитьПараметры.КнопкаПоУмолчанию = Истина;
		
		Если Параметры.Свойство("бит_ПараметрыИсполняемогоОтчета") Тогда
			ПараметрыИсполняемогоОтчета = ПолучитьИзВременногоХранилища(Параметры.бит_ПараметрыИсполняемогоОтчета);			
			Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(ПараметрыИсполняемогоОтчета.НастройкиКД);
			Отчет.КомпоновщикНастроек.ЗагрузитьФиксированныеНастройки(ПараметрыИсполняемогоОтчета.ФиксированныеНастройкиКД);
			Отчет.КомпоновщикНастроек.ЗагрузитьПользовательскиеНастройки(ПараметрыИсполняемогоОтчета.ПользовательскиеНастройкиКД);
		КонецЕсли;
	КонецЕсли;
	
	Если ПустаяСтрока(Заголовок)
		И Параметры.Свойство("Заголовок") Тогда
...
// бит_Финанс изменения кода. Начало.
&НаСервере
Функция бит_ПараметрыФормированияОтчета(ИдентификаторВладельца)
	
	ИмяОтчета = СтрРазделить(НастройкиОтчета.ПолноеИмя, ".")[1];
	ПараметрыФормирования = ПараметрыФормированияОтчета(ИмяОтчета, НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный);
	
	Возврат ПоместитьВоВременноеХранилище(ПараметрыФормирования, ИдентификаторВладельца);
