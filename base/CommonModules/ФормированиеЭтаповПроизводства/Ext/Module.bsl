...
Процедура СформироватьЭтапыКЗапуску(ГруппыИзделий, Изделия, ДатыОбеспеченияПоГрафику, Параметры, АдресХранилища) Экспорт
	...
	Если ГруппыИзделий.Количество() <= РазмерПорции Тогда
		СформироватьПорциюЭтапов(ГруппыИзделий, Изделия, ДатыОбеспеченияПоГрафику, Параметры, Результат, АдресХранилища);
	Иначе
		Счетчик = 1;
		...
		ДатыОбеспеченияКопия = ДатыОбеспеченияПоГрафику.СкопироватьКолонки();
	
		Изделия.Индексы.Добавить("НомерГруппы");
		ДатыОбеспеченияПоГрафику.Индексы.Добавить("НомерГруппы");
		...
		Для Индекс = 0 По МаксимальныйИндекс Цикл
			...
			КонецЦикла;
			Для каждого СтрокаОбеспечение Из ДатыОбеспеченияПоГрафику.НайтиСтроки(СтруктураПоиска) Цикл
				ЗаполнитьЗначенияСвойств(ДатыОбеспеченияКопия.Добавить(), СтрокаОбеспечение);
			КонецЦикла;
			
			Если Счетчик = РазмерПорции ИЛИ Индекс = МаксимальныйИндекс Тогда
				СформироватьПорциюЭтапов(ГруппыИзделийКопия, ИзделияКопия, ДатыОбеспеченияКопия, Параметры, Результат, АдресХранилища);
				
				ГруппыИзделийКопия.Очистить();
				ИзделияКопия.Очистить();
				ДатыОбеспеченияКопия.Очистить();
				Счетчик = 1;
			Иначе
				Счетчик = Счетчик+1;
			КонецЕсли;
...
//  ДатыОбеспеченияПоГрафику - ТаблицаЗначений
//  Параметры - Структура
//  Результат - Структура
//  АдресХранилища - Строка
//
Процедура СформироватьПорциюЭтапов(ГруппыИзделий, Изделия, ДатыОбеспеченияПоГрафику, Параметры, Результат, АдресХранилища)
