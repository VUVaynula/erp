	...
	Попытка
		...
		Если Отказ Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось заблокировать сведения об объектах учета ЭДО';
								|en = 'Не удалось заблокировать сведения об объектах учета ЭДО'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Процедура ЗаписатьОшибкуФормированияОтветногоТитулаВЖурналРегистрации(СтруктурированнаяИнформацияОбОшибке)
	...
	Шаблон = НСтр(
		...
		|%1';
		|en = 'Не удалось сформировать ответный титул, по причине:
		|%1'");
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Не удалось сформировать ответный титул. Подробности см. в журнале регистрации.';
					|en = 'Не удалось сформировать ответный титул. Подробности см. в журнале регистрации.'"));
		КонецЕсли;
...
Функция КлючевыеРеквизитыУчастниковЭДО(УчастникиЭДО, ТипУчастниковЭДО)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный тип участников ЭДО';
								|en = 'Некорректный тип участников ЭДО'");
	КонецЕсли;
...
Процедура ЗапуститьПроверкуАктуальностиВнутреннегоЭД(ОбъектУчета, Отказ)
	...
	Если Отказ Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не удалось закончить проверку актуальности электронного документа: %Ошибка%';
								|en = 'Не удалось закончить проверку актуальности электронного документа: %Ошибка%'");
...
Процедура ЗаблокироватьДанныеОбъектовУчетаДокументовЭДО(ОбъектУчета, ЭлектронныйДокумент, БлокироватьТолькоАктуальные, Отказ)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось заблокировать данные объектов учета ЭД: %ИнформацияОбОшибке%';
								|en = 'Не удалось заблокировать данные объектов учета ЭД: %ИнформацияОбОшибке%'");
