...
Процедура МеханизмРегистрацииОбъектовПередЗаписьюРегистра(ИмяПланаОбмена, Источник, Отказ, Замещение) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.НаборыЗначенийДоступа") 
		И СтрСравнить(ИмяПланаОбмена, "Полный") <> 0
		И СтрСравнить(ИмяПланаОбмена, "бит_Полный") <> 0 Тогда
		
		Объект = Источник.Отбор.Объект.Значение;
		Если бит_ОбщегоНазначения.ОбъектБИТ(Объект) Тогда
			Возврат;
		КонецЕсли; 
		...
		Если ПустаяСтрока(СтрокаТаблицы.ИмяТабличнойЧасти) Тогда // реквизиты шапки
			...
			Для Каждого Элемент Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Ссылка = Объект[Элемент.Ключ];
					...
					// бит_Финанс изменения кода. Начало.
					// Таблица фильтров узла плана обмена Аудиторская выгрузка может иметь строковые значения.
