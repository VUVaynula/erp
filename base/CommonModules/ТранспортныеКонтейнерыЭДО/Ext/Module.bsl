...
Функция РаспаковатьФайлКонтейнера(Файл, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если РезультатРаспаковки = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при распаковке архива транспортного контейнера ЭДО';
								|en = 'Ошибка при распаковке архива транспортного контейнера ЭДО'");
	КонецЕсли;
	...
	Если ОписаниеКонтейнера.Карточка = Неопределено Или ОписаниеКонтейнера.Метаданные = Неопределено Тогда
		...
		|en = 'Ошибка при чтении транспортного контейнера ЭДО.
		|Отсутствует один из служебных файлов card.xml или meta.xml'");
	КонецЕсли;
...
Функция ПрочитатьКарточкуЭлектронногоДокумента(ИмяФайла)
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru = 'Возникла ошибка при чтении данных файла card.xml: %2.';
								|en = 'Возникла ошибка при чтении данных файла card.xml: %2.'");
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИмяФайла,
					...
					Иначе
						...
						Иначе
							ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка чтения файла подписи %1 из архива';
														|en = 'Ошибка чтения файла подписи %1 из архива'"), 
								Подпись.Path);
							...
							Иначе
								ТекстОшибки = СтрШаблон( 
									НСтр("ru = 'Ошибка чтения файла подписи %1 дополнительных данных из архива';
										|en = 'Ошибка чтения файла подписи %1 дополнительных данных из архива'"),
									Подпись.Path);
								ВызватьИсключение ТекстОшибки;
							КонецЕсли;
						КонецЕсли;
		...
		Для Каждого Документ Из Документооборот.Documents.Document Цикл
			...
			Если Не ЗначениеЗаполнено(ДвоичныеДанныеФайлаЭД) И ЗначениеЗаполнено(ИмяОсновногоФайла) Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка при разборе файла meta.xml транспортного контейнера ЭДО.
					|Файл %1 не найден в транпортном контейнере.';
...
Функция ДанныеДоверенностиПоХэшамПодписиИзРаспакованныхФайлов(РаспакованныйФайл, РаспакованныеФайлы)
	...
	Результат = Новый Соответствие();
	ВидОперации = НСтр("ru = 'Чтение доверенностей электронного документа.';
						|en = 'Чтение доверенностей электронного документа.'");
	ПодсистемаСобытия = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
	...
	Для Каждого ДанныеМЧД Из МЧД Цикл
		...
		Если ФайлыМЧД <> Неопределено Тогда
			...
			Если ДвоичныеДанныеМЧД = Неопределено Тогда
				ШаблонПредупреждения = НСтр("ru = 'В контейнере отсутствуют данные файла доверенности: %1';
											|en = 'В контейнере отсутствуют данные файла доверенности: %1'");
				ТекстПредупреждения = СтрШаблон(ШаблонПредупреждения, ИмяФайлаМЧД);
				ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстПредупреждения, ПодсистемаСобытия,
					УровеньЖурналаРегистрации.Предупреждение);
				Возврат Результат;
			...
			Для Каждого Подпись Из Подписи Цикл
				...
				Если ДвоичныеДанныеПодписиМЧД = Неопределено Тогда
					ШаблонПредупреждения = НСтр("ru = 'В контейнере отсутствуют данные подписи доверенности: %1';
												|en = 'В контейнере отсутствуют данные подписи доверенности: %1'");
					ТекстПредупреждения = СтрШаблон(ШаблонПредупреждения, ИмяФайлаПодписи);
					ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстПредупреждения, ПодсистемаСобытия,
						УровеньЖурналаРегистрации.Предупреждение);
					Возврат Результат;
		...
		Для Каждого Подпись Из Подписи Цикл
			...
			Если ДвоичныеДанныеПодписи = Неопределено Тогда
				ШаблонПредупреждения = НСтр("ru = 'В контейнере отсутствует файл удостоверяемой подписи: %1';
											|en = 'В контейнере отсутствует файл удостоверяемой подписи: %1'");
				ТекстПредупреждения = СтрШаблон(ШаблонПредупреждения, ИмяФайлаПодписи);
				ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстПредупреждения, ПодсистемаСобытия,
					УровеньЖурналаРегистрации.Предупреждение);
				Возврат Результат;
