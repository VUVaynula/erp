...
Процедура УстановитьЗаголовокЭлектроннаяПодписьНаСервере(Форма)
	...
	Если ИнформационнаяБазаФайловая Тогда
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на компьютер, где работает веб-сервер, подключенный к файловой информационной базе, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.';
									|en = 'Important: a <a href=%1>digital signature application</a> and a <a href=%2>certificate</a> with a private key must be installed on the computer running the web server connected to the file infobase.'");
	Иначе
		ПодсказкаНаСервере = НСтр("ru = 'Важно: на каждый компьютер, где работает сервер 1С:Предприятия, должно быть установлено <a href=%1>приложение электронной подписи</a> и <a href=%2>сертификат</a> с закрытым ключом.';
									|en = 'Important: a <a href=%1>digital signature application</a> and a <a href=%2>certificate</a> with a private key must be installed on each computer running 1C:Enterprise server.'"); 
	КонецЕсли;
