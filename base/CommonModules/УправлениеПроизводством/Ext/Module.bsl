...
Процедура СоздатьВТСвязиЭтапов(МенеджерВременныхТаблиц, ТаблицаЭтапы, ПолеЭтап, ПолеКлючПартия, ИсключатьЗавершенные = Ложь, ТолькоВнутренниеСвязи = Ложь) Экспорт
	...
	ТекстЗапроса =
		...
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Таблица.Этап.ПартияПроизводства)          КАК КлючПартия,
		|	Таблица.СледующийЭтап                                             КАК СледующийЭтап,
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Таблица.СледующийЭтап.ПартияПроизводства) КАК СледующийЭтапКлючПартия
		|ИЗ
...
Процедура ПланироватьГрафикЭтапов(Параметры, АдресХранилища) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	Этапы.Распоряжение.ДинамическаяСтруктура КАК ДинамическаяСтруктура,
		...
		|		ИЛИ Этапы.ДинамическаяСтруктура
	...
	Для каждого СтрокаРаспоряжение Из Дерево.Строки Цикл
		...
		Если СтрокаРаспоряжение.ДинамическаяСтруктура Тогда
			РезультатыПланирования = ГрафикПроизводстваСтруктурыЗаказа.РассчитатьГрафикВыбранныхЭтапов(
...
Процедура ПроверитьПорядокСледованияЭтапов(Этапы) Экспорт
	...
	КонецЕсли;
	Документы.ЭтапПроизводства2_2.СоздатьВТСвязиЭтапов(МенеджерВременныхТаблиц, МассивЭтапов);
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ВТСвязиЭтапов.Этап КАК Документ.ЭтапПроизводства2_2) КАК Этап,
	|	ВЫРАЗИТЬ(ВТСвязиЭтапов.СледующийЭтап КАК Документ.ЭтапПроизводства2_2) КАК СледующийЭтап
	...
	|	ВТСвязиСНарушением.СледующийЭтап.Распоряжение КАК Распоряжение,
	|	ВТСвязиСНарушением.СледующийЭтап.ДинамическаяСтруктура КАК ДинамическаяСтруктура,
	|	ВТСвязиСНарушением.СледующийЭтап.Статус КАК Статус
	|ПОМЕСТИТЬ ВТСсылки
	|ИЗ
	|	ВТСвязиСНарушением КАК ВТСвязиСНарушением
	|ГДЕ
	|	ВТСвязиСНарушением.Этап В(&Этапы)
	|	И ВТСвязиСНарушением.Этап.Распоряжение.РазмещениеВыпуска = &КНачалу
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТСвязиСНарушением.Этап КАК Этап,
	|	ВТСвязиСНарушением.Этап.Распоряжение КАК Распоряжение,
	|	ВТСвязиСНарушением.Этап.ДинамическаяСтруктура КАК ДинамическаяСтруктура,
	|	ВТСвязиСНарушением.Этап.Статус КАК Статус
	|ИЗ
	|	ВТСвязиСНарушением КАК ВТСвязиСНарушением
	|ГДЕ
	|	ВТСвязиСНарушением.СледующийЭтап В(&Этапы)
	|	И ВТСвязиСНарушением.СледующийЭтап.Распоряжение.РазмещениеВыпуска = &КОкончанию
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Ссылки.Этап КАК Этап
	|ИЗ
	|	ВТСсылки КАК Ссылки
	|ГДЕ
	|	НЕ Ссылки.ДинамическаяСтруктура
	|	И Ссылки.Статус В (
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Формируется),
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Сформирован),
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.КВыполнению))";
	...
	Если НеактуальныеЭтапы.Количество() > 0 Тогда
		// Задача не актуальна для динамической структуры
...
Процедура ПроверитьСрокиОбеспеченияЭтапов(Этапы) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	График.Начало < Обеспечение.ДатаОбеспечения");
...
Процедура СоздатьВТОграниченияГрафикаПроизводстваПоМатериаламДинамическаяСтруктура(МенеджерВременныхТаблиц, УсловиеПолучения, ВРазрезеНоменклатуры, ИсключитьДанныеГрафикаПроизводства, СтатусГрафика = 0) Экспорт
	...
	ТекстЗапроса = 
		...
		|	ВЫБОР
		|		КОГДА МАКСИМУМ(ВложенныйЗапрос.ДатаПоступления) < &НачалоТекущегоДня
		|			ТОГДА
		|				&НачалоТекущегоДня
		|			ИНАЧЕ
		|				МАКСИМУМ(ВложенныйЗапрос.ДатаПоступления)
		|	КОНЕЦ КАК ДатаОбеспечения
		|ПОМЕСТИТЬ ВТОграниченияГрафикаПроизводстваПоМатериалам
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|					ТОГДА СтруктураЗаказа.КлючПартия
		|			КОНЕЦ) КАК КлючПартия,
		|		МАКСИМУМ(ВЫБОР
		|				КОГДА СтруктураЗаказа.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Потребление)
		...
		|			КОНЕЦ) КАК Этап,
		|		ВЫБОР
		|			КОГДА СУММА(СтруктураЗаказа.Требуется
		|						- СтруктураЗаказа.Запланировано - СтруктураЗаказа.РаспределеноИзПартий - СтруктураЗаказа.РаспределеноИзПартийОбособленно
		|						- ВЫБОР
		|							КОГДА СтруктураЗаказа.ЗаказНаПоступление = НЕОПРЕДЕЛЕНО ИЛИ СтруктураЗаказа.ДатаПоступления <> ДАТАВРЕМЯ(1,1,1)
		|								ТОГДА СтруктураЗаказа.РаспределеноИзЗапасов + СтруктураЗаказа.РаспределеноИзЗапасовОбособленно
		|							ИНАЧЕ 0
		|						 КОНЕЦ) = 0
		|			ТОГДА
		|				МАКСИМУМ(ISNULL(График.Окончание, СтруктураЗаказа.ДатаПоступления))
		|			ИНАЧЕ
		|				МАКСИМУМ(РасчетДатыПоставкиПереопределяемый.ПлановаяДатаПоставки)
		...
		|						ТОГДА СтруктураЗаказа.Запланировано + СтруктураЗаказа.РаспределеноИзПартий + СтруктураЗаказа.РаспределеноИзПартийОбособленно
		|					ИНАЧЕ 0
		|				 КОНЕЦ
		|				- ВЫБОР
...
Процедура ПроверитьСрокиОбеспеченияЭтаповДинамическаяСтруктура(МенеджерВременныхТаблиц, ТекстОтбора) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|
		|ГДЕ
		|	График.Начало < Ограничения.ДатаОбеспечения");
...
Процедура ОбновитьДвиженияДокументовПослеЗаписиГрафикаПроизводства(Заказ, Этапы, ДинамическаяСтруктура = Ложь) Экспорт
	...
	Задания = РегистрыСведений.УниверсальнаяОчередьОперативныхЗаданий.СоздатьНаборЗаписей().Выгрузить();
	
	Для каждого Этап Из Этапы Цикл
		НовоеЗадание = Задания.Добавить();
		НовоеЗадание.Объект = Этап;
		НовоеЗадание.Задание = Перечисления.ЗаданияУниверсальнойОчереди.ОбновлениеГрафикаДвиженияТоваровПослеРасчетаГрафикаПроизводства;
	КонецЦикла;
	
	Если ДинамическаяСтруктура Тогда
		НовоеЗадание = Задания.Добавить();
		НовоеЗадание.Объект = Заказ;
		НовоеЗадание.Задание = Перечисления.ЗаданияУниверсальнойОчереди.ОбновлениеГрафикаДвиженияТоваровПослеРасчетаГрафикаПроизводства;
	Иначе
	...
	УниверсальнаяОчередьОперативныхЗаданий.ОбработатьЗадания(Задания);
