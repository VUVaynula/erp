		...
		Для Каждого Исправление Из Исправления Цикл
			...
			Если УдалитьИсправление Тогда
				...
				Если СписокСборок <> Неопределено Тогда
					...
					Иначе
						ВерсияБиблиотеки = бит_ОбщегоНазначения.МетаданныеВерсия();
						ТекстСообщения = НСтр("ru = 'Исправление ""%1"" устарело и будет удалено.
...
Функция ДляТекущейВерсииНеобходимоУспешноеЗавершениеОбработчиков() Экспорт
	...
	Возврат ТекущийСтатус.ВерсииТребующиеУспешногоОбновления.Найти(бит_ОбщегоНазначения.МетаданныеВерсия()) <> Неопределено;
