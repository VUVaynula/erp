...
Функция ОписаниеКлиентскогоПриложения() Экспорт
	...
	Если Не ПустаяСтрока(ВерсияПодсистемы) Тогда
		ТекущееПриложение = СтрШаблон("%1/%2 HRLib/%3", ОбщегоНазначения.ИдентификаторИнтернетПоддержкиКонфигурации(), бит_ОбщегоНазначения.МетаданныеВерсия(), ВерсияПодсистемы);
	Иначе
		ТекущееПриложение = СтрШаблон("%1/%2", ОбщегоНазначения.ИдентификаторИнтернетПоддержкиКонфигурации(), бит_ОбщегоНазначения.МетаданныеВерсия());
	КонецЕсли;
