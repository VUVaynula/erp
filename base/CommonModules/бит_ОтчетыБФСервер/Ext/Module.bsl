...
Процедура УстановитьМакетОформленияВРежимеТакси(Форма, НовыеНастройкиКД) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЭтоМобильныйКлиент")
		И Форма.ЭтоМобильныйКлиент Тогда
		ПараметрМакетОформленияЗначение = ИмяМакетаОформленияПоУмолчаниюМобильныйКлиент();
	Иначе
		ПараметрМакетОформленияЗначение = ИмяМакетаОформленияПоУмолчанию();
	КонецЕсли;
...
Функция ПараметрыДиаграмм(КомпоновщикНастроек) Экспорт
	...
	ПараметрМаксимумСерий = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "МаксимумСерийКоличество");
	...
	ПараметрТипДиаграммы = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ТипДиаграммы");
	...
	ПараметрВыделениеСерийДиаграмм = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ВыделениеСерийДиаграмм");
	...
	ПараметрГрадиентСерийДиаграмм = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ГрадиентСерийДиаграмм");
	...
	ПараметрОтображениеМаркеровТочекДиаграмм = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ОтображениеМаркеровТочекДиаграмм");
	...
	ПараметрОтслеживаемыеАналитики = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ОтслеживаемыеАналитики");
...
Процедура СкрытьВспомогательныеПараметрыОтчета(СхемаКомпоновкиДанных, КомпоновщикНастроек, ТабличныйДокумент, ПараметрыОтчета) Экспорт
	...
	Для Каждого СкрываемыйПараметрОтчета Из СкрываемыеПараметрыОтчета Цикл 
		...
		Если НайденнаяОбласть <> Неопределено 
			...
			// АПК:1391-выкл. Строковый параметр.
...
Процедура ДобавитьПредставленияПараметраВМассив(СхемаКомпоновкиДанных, КомпоновщикНастроек, Параметры, ИмяПараметра) Экспорт
	...
	Если Не Параметр = Неопределено Тогда
		...
		ПараметрКомпоновки = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, ИмяПараметра);
...
Функция СтруктураЗаголовковВалют(КомпоновщикНастроек, Скобки = Истина) Экспорт
	...
	ПараметрДанныеОтчета = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ДанныеОтчета");
...
Процедура УстановитьПараметрыВалютыОтчета(КомпоновщикНастроек, ПользовательскиеНастройкиМодифицированы) Экспорт
	...
	Если ПараметрДанныеОтчета <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ПараметрДанныеОтчета.Значение) Тогда
			бит_ОтчетыБФКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ДанныеОтчета", 1);
			
			ПользовательскиеНастройкиМодифицированы = Истина;
		КонецЕсли;
		
		ПараметрВалюта = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "Валюта");
		Если ПараметрВалюта <> Неопределено Тогда
			Если ПараметрДанныеОтчета.Значение = 1
				ИЛИ ПараметрДанныеОтчета.Значение = 2 Тогда
				бит_ОтчетыБФКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "Валюта", ВалютаУправленческогоУчета);
			ИначеЕсли ПараметрДанныеОтчета.Значение = 3 Тогда
				бит_ОтчетыБФКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "Валюта", ВалютаРегламентированногоУчета);
			КонецЕсли;
	...
	ПараметрДанныеОтчета = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ДанныеРасчета");
	Если ПараметрДанныеОтчета <> Неопределено Тогда
		ВалютаУправленческогоУчета = Константы.бит_ВалютаУправленческогоУчета.Получить();
		ВалютаРегламентированногоУчета = Константы.ВалютаРегламентированногоУчета.Получить();
	
		ПараметрВалюта = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ВалютаРасчета");
		Если ПараметрВалюта <> Неопределено Тогда
			Если ПараметрДанныеОтчета.Значение = 1
				ИЛИ ПараметрДанныеОтчета.Значение = 2 Тогда
				бит_ОтчетыБФКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВалютаРасчета", ВалютаУправленческогоУчета);
			ИначеЕсли ПараметрДанныеОтчета.Значение = 3 Тогда
				бит_ОтчетыБФКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВалютаРасчета", ВалютаРегламентированногоУчета);
			ИначеЕсли ПараметрДанныеОтчета.Значение = 0 Тогда
				бит_ОтчетыБФКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВалютаРасчета", Справочники.Валюты.ПустаяСсылка());
			КонецЕсли;
...
Процедура НастроитьДинамическийПериод(СхемаКомпоновкиДанных, КомпоновщикНастроек, ДополнятьПериод = Ложь, ДополнениеПути = "") Экспорт 
	...
	ПараметрПериодичность = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "Периодичность");
	
	Если ПараметрПериодичность <> Неопределено 
		И бит_ОтчетыБФКлиентСервер.ПолеИспользуется(КомпоновщикНастроек, Новый ПолеКомпоновкиДанных(ДополнениеПути + "ДинамическийПериод")) Тогда
		...
		Если ИскомоеПоле <> Неопределено Тогда
			...
			// -- NB Если ДополнятьПериод Тогда
				
				ДополнениеПериода = ТипДополненияПериодаКомпоновкиДанных[СтрокаДлительностьПериода];
				ПараметрПериод = бит_ОтчетыБФКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "Период");
				
				Если ПараметрПериод <> Неопределено Тогда
					
					ПолеДинамическийПериод = Новый ПолеКомпоновкиДанных(ДополнениеПути + "ДинамическийПериод");
					Группировки = бит_ОтчетыБФКлиентСервер.ПолучитьГруппировки(КомпоновщикНастроек);
					...
					Для Каждого Группировка Из Группировки Цикл
						...
						Если Группировка.Значение.ПоляГруппировки.Элементы.Количество() = 1
							...
							ГруппировкаДинамическийПериод.ТипДополнения = ?(ДополнятьПериод, ДополнениеПериода, 
							...
							Иначе	
								ГруппировкаДинамическийПериод.НачалоПериода = '0001-01-01';
								ГруппировкаДинамическийПериод.КонецПериода = '0001-01-01';
							КонецЕсли;
...
Процедура ОптимизацияИспользованияОборотнойАналитики(МакетКомпоновкиДанных, СхемаКомпоновкиДанных, НастройкиОтчета, Знач ДанныеРасшифровки) Экспорт
	...
	бит_ОтчетыБФКлиентСервер.УстановитьПараметр(НастройкиОтчета, "ИспользуетсяОборотнаяАналитика", ИспользуетсяОборотнаяАналитика);
...
// Вычисляет относительное отклонение.
//
// Параметры:
// 	Дельта - Число, абсолютное отклонение.
//	Знаменатель - Число, значение план.
//
// Возвращаемое значение:
// Число - Относительное отклонение.
Функция ОтносительноеОтклонение(Знач Дельта, Знач Знаменатель) Экспорт

	// Заменяет выражение компоновки данных:
	//ВЫБОР 
	//    КОГДА СУММА(Знаменатель) > 0 ТОГДА СУММА(Дельта) * 100 / СУММА(Знаменатель) 
	//    КОГДА СУММА(Знаменатель) < 0 ТОГДА СУММА(Дельта) * 100 / - СУММА(Знаменатель) 
	//    КОГДА СУММА(Дельта) <> 0 ТОГДА 100
	//    ИНАЧЕ 0
	//КОНЕЦ

	Перем Отклонение;
	
	Дельта		= ?(Дельта = NULL, 0, Дельта);
	Знаменатель = ?(Знаменатель = NULL, 0, Знаменатель);
	
	Если Знаменатель > 0 Тогда
		Отклонение = Дельта * 100 / Знаменатель;
...
// Вычисляет абсолютное отклонение.
//
// Параметры:
// 	План - Число, значение плановых данных.
//	Факт - Число, значение фактических данных.
//
// Возвращаемое значение:
// Число - Относительное отклонение.
Функция АбсолютноеОтклонение(Знач План, Знач Факт) Экспорт

	План = ?(План = NULL, 0, План);
	Факт = ?(Факт = NULL, 0, Факт);
...
Функция СкопироватьНаборыДанных(ПриемникНаборовДанных, ИсточникНаборовДанных, ОсновноеИмяНабораДанных = "") Экспорт
	...
	Для Каждого НаборИсточник Из Источник Цикл 
		...
		НовыйНабор = ДобавитьПустойНаборДанных(Приемник, ТипНабора, ИмяНабора);
...
Процедура ДобавитьОтборПоКолонкеТаблицыЗначений(ЭлементНастроекКомпоновки, ТаблицаДанных, ИмяКолонки) Экспорт
	...
	Если Не (МассивЗначенийОтбора.Количество() = 1 
		...
		бит_ОтчетыБФКлиентСервер.ДобавитьОтбор(ЭлементНастроекКомпоновки, ИмяКолонки, СписокЗначений, ВидСравненияКомпоновкиДанных.ВСписке);
	КонецЕсли;
...
// Параметры функции бит_ОтчетыБФСервер.ВыгрузитьРезультатСКД
// 
// Возвращаемое значение:
//  Структура - поля 
...
//
// Параметры:
//  Параметры	 - структура - см. бит_ОтчетыБФСервер.ПараметрыВыгрузитьРезультатСКД
...
Процедура НастроитьПоляВыбора(ПоказателиОтчета, ЭлементОтчета) Экспорт
		...
		Для Каждого ПолеВыбора Из ЭлементОтчета Цикл	
			...
			Иначе
				Для Каждого Показатель Из ПоказателиОтчета Цикл
					Если ВРЕГ(ИмяПоля) = ВРЕГ(Показатель.Ключ) Тогда
						Использование = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Показатель.Значение, "Использование", ПолеВыбора.Использование);
						Заголовок	  = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Показатель.Значение, "Заголовок", ПолеВыбора.Заголовок);
						ПолеВыбора.Использование = Использование;
...
//  Строка - имя макета
//
Функция ИмяМакетаОформленияПоУмолчанию()
	
	// ++ ЕРП
	ЗначениеПоУмолчанию = "ОформлениеОтчетовБежевый";
	// -- ЕРП	
...
//  Строка - имя макета
//
Функция ИмяМакетаОформленияПоУмолчаниюМобильныйКлиент()
	
	// ++ ЕРП
	ЗначениеПоУмолчанию = "ОформлениеОтчетовБежевыйМобильныйКлиент";
	// -- ЕРП	
...
Процедура УстановитьОформлениеПоУмолчаниюДиаграммы(Диаграмма, ПараметрыДиаграммы = Неопределено)
	...
	ОбластьПостроения.ЦветШкалы = Метаданные.ЭлементыСтиля.бит_ЦветНедоступногоПоля.Значение;
...
Процедура УстановитьОформлениеПоУмолчаниюСводнойСерии(Серия)
	...
	Серия.Цвет = Метаданные.ЭлементыСтиля.бит_ЦветНедоступногоПоля.Значение;
КонецПроцедуры
...
Функция СхемаКомпоновкиПоПараметру(Знач СхемаКомпоновки) Экспорт
	...
	Если ТипЗнч(СхемаКомпоновки) = Тип("Строка") Тогда
		...
		Иначе
			Возврат МакетПоИмени(СхемаКомпоновки);
		КонецЕсли;
...
//
Функция НовоеПолеНабора(НаборДанных, Поле, ПутьКДанным = "", Заголовок = "", ТипЗначения = Неопределено)
