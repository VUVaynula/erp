...
Процедура ЗагрузитьМодельУчета(Параметры) Экспорт
	...
	ОписаниеВерсии = Метаданные.Имя + " " + бит_ОбщегоНазначения.МетаданныеВерсия();
...
Процедура ВыгрузитьМодельУчета(Параметры, ИмяФайлаНаСервере = Неопределено) Экспорт
	...
	ЗаписьXML.ЗаписатьАтрибут("Version", Метаданные.Имя + " " + бит_ОбщегоНазначения.МетаданныеВерсия());
