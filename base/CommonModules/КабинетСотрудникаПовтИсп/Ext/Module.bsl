...
Функция ОписаниеКлиентскогоПриложения() Экспорт
	...
	Если Не ПустаяСтрока(ВерсияПодсистемы) Тогда
		Приложение = СтрШаблон("%1/%2 HRLib/%3", ОбщегоНазначения.ИдентификаторИнтернетПоддержкиКонфигурации(), бит_ОбщегоНазначения.МетаданныеВерсия(), ВерсияПодсистемы);
	Иначе
		Приложение = СтрШаблон("%1/%2", ОбщегоНазначения.ИдентификаторИнтернетПоддержкиКонфигурации(), бит_ОбщегоНазначения.МетаданныеВерсия());
	КонецЕсли;
