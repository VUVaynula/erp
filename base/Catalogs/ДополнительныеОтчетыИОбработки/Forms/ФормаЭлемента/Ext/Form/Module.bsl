...
// Функция определяет, какая команда используется в зависимости от вида дополнительной обработки.
//
// Параметры:
...
КонецФункции // бит_ОпределитьКомандуПоВидуОбработки()

// Дополняет фильтр-список объектов метаданных типами объектов команд БИТ. 
//
&НаСервереБезКонтекста
...
// Процедура-действие события Нажатие поля НазначениеКоманд. Назначается программно. 
// Отличается от типового действия дополнением фильтра по объектам метаданных.
//
&НаКлиенте
Процедура бит_НазначениеКомандЗаполнениеФормыНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	...
	Иначе
		// Выбор объектов метаданных
		ПараметрыФормы = ПодготовитьПараметрыФормыВыборОбъектовМетаданных();
		// начало добавления 26.02.2013
		бит_ДополнитьФильтрПоОбъектамМетаданных(Объект.Вид, ПараметрыФормы.ФильтрПоОбъектамМетаданных);
		// конец добавления 26.02.2013
...
КонецПроцедуры // бит_ПриСозданииНаСервере()

// Процедура выполняет переопределение типового действия УказатьНазначениеОбработки.
//
&НаСервере
Процедура бит_ВыполнитьПерехватДействий()

	ТекКоманда = ЭтотОбъект.Элементы.Найти("НазначениеКоманд");
	
	Если НЕ ТекКоманда = Неопределено  
		И НЕ ТекКоманда.ПолучитьДействие("Нажатие") = "бит_НазначениеКомандЗаполнениеФормыНажатие" Тогда
	
		 ТекКоманда.УстановитьДействие("Нажатие", "бит_НазначениеКомандЗаполнениеФормыНажатие");
		 ЭтотОбъект.УстановитьДействие("ПриСозданииНаСервере","бит_ПриСозданииНаСервере");
		 
	КонецЕсли; 

КонецПроцедуры // бит_ВыполнитьПерехватДействий()

бит_ВыполнитьПерехватДействий();
