...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Справочники.бит_ВидыФормСбораДанных.СформироватьСписокЗапрещенныхИмен(СписокЗапрещенные);
	...
	Для каждого МетаРеквизит Из Метаданные.Справочники.бит_ВидыФормСбораДанных.ТабличныеЧасти.НастройкиРеквизитов.Реквизиты Цикл
...
Процедура Свойства_ОтборПорядокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.бит_ВидыФормСбораДанных.Форма.НастройкиВыгрузки", ПараметрыФормы, ЭтотОбъект,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура КомандаВыгрузитьШаблонExcel(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстСообщения = НСтр("ru='Для выполнения действия необходимо записать элемент. Продолжить?';
							  |en='To perform the action you want to record element. Proceed?'");
...
Процедура КомандаКонструкторБюджетирования(Команда)
	...
	ОткрытьФорму("Справочник.бит_ВидыФормСбораДанных.Форма.ФормаКонструкторПроформы",ПараметрыФормы, ЭтотОбъект,,,,Оповещение, РежимОкна);
...
// 
// Параметры:
//  ВидПроформы - СправочникСсылка.бит_ВидыФормСбораДанных.
// 
// Возвращаемое значение:
//  ТаблицаИспользования - ТаблицаЗначений.
// 
&НаСервере
Функция ДокументыТекущейПроформы(ВидФормыСбораДанных)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидФормыСбораДанных", ВидФормыСбораДанных);
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               |	ФормаСбораДанных.Ссылка
	               |ИЗ
	               |	Документ.бит_ФормаСбораДанных КАК ФормаСбораДанных
	               |ГДЕ
	               |	ФормаСбораДанных.ВидФормыСбораДанных = &ВидФормыСбораДанных";
...
Функция ПодготовитьДанныеДляВыгрузкиШаблона()
	...
	СтрПар = Справочники.бит_ВидыФормСбораДанных.ПодготовитьДанныеДляВыгрузкиШаблона(Объект.Ссылка);
...
Процедура ВыполнитьКопированиеШаблонов()
	...
	Если Выполнено Тогда
		...
		Для каждого СтрокаТаблицы Из ВидПроформыИсточник.НастройкиДвижений Цикл
			 ...
			 Записан = бит_ОбщегоНазначения.ЗаписатьСправочник(ШаблонОбъект,,"Ошибки");
...
// 
// Параметры:
//  ТекущийОбъект - СправочникОбъект.бит_ВидыФормСбораДанных.
// 
&НаСервере
...
Процедура ОжиданиеАктивизацииСтрокиДерева()
	...
	Если НЕ ТекущаяСтрока = Неопределено Тогда
		...
		Для каждого Элемент Из СписокЭлементыПанели Цикл
			...
			Элементы[ИмяРеквФормы].Доступность = ДоступностьЭлемента;
...
// 
// Возвращаемое значение:
//  ДопустимоеИмя - Булево.
// 
&НаКлиенте
Функция ЭтоДопустимоеИмя(ИмяЭлемента, ВидУзла)

	ДопустимоеИмя = Истина;
	
	Если ВидУзла = "Реквизит" И НЕ СписокЗапрещенные.НайтиПоЗначению(ИмяЭлемента) = Неопределено Тогда
		ДопустимоеИмя = Ложь;
	КонецЕсли; 
	
	Попытка
		СтрТест = Новый Структура;
		СтрТест.Вставить(ИмяЭлемента);
	Исключение
		ДопустимоеИмя = Ложь;		
	КонецПопытки;
	
	Возврат ДопустимоеИмя;
