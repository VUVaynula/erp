...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Объект.Ссылка.Пустая()
		...
		Объект.бит_ОтправленНаСогласование = Ложь;
		Объект.Статус                      = Перечисления.СтатусыДоговоровКонтрагентов.НеСогласован;
	КонецЕсли;
	бит_ОбновитьКомандуУправленияСтатусом();
	// бит_Финанс изменения кода. Конец.
...
Процедура УстановитьТекстПодсказкиПорядокЗачетаДокументов()
	...
	Если Объект.ПорядокЗачетаОплатНакладных = ПредопределенноеЗначение("Перечисление.ПорядкиЗачетаОплатНакладных.ПоДатеВидуДокумента") Тогда
		...
		|en = 'By document date and kind: postpayments are first allocated to invoices with the earliest document posting date. Invoices offset prepayments with the earliest document date.
		|Documents within one day are offset by their kind: first, prepayment and debt offsets, invoices, returns of goods and invoice adjustments, payments, refunds, and so on.'");
...
#Область ОбработчикиБИТ

&НаСервере
Процедура бит_ОбновитьКомандуУправленияСтатусом()
	
	НастройкиВизирования = бит_ВизированиеСервер.НастройкиОбъектаВизирования(Объект.Ссылка);
	Если НЕ НастройкиВизирования.ИспользоватьВизирование Тогда
		Возврат;
	КонецЕсли;
	Элементы.Статус.ТолькоПросмотр = Истина;
	
	ИмяКоманды               = "бит_КомандаУправленияСтатусом";
	КнопкаУправлениеСтатусом = Элементы.Найти(ИмяКоманды);
	
	Если КнопкаУправлениеСтатусом = Неопределено Тогда
		КомандаУправленияСтатусом = ЭтотОбъект.Команды.Добавить(ИмяКоманды);
		КомандаУправленияСтатусом.Действие = "бит_Подключаемый_КомандаУправленияСтатусомНажатие";
		
		ГруппаСтатуса = Элементы.Статус.Родитель;
		НоваяГруппа   = Элементы.Добавить("бит_ГруппаСтатус", Тип("ГруппаФормы"), ГруппаСтатуса);
		НоваяГруппа.Вид                 = ВидГруппыФормы.ОбычнаяГруппа;
		НоваяГруппа.ОтображатьЗаголовок = Ложь;
		НоваяГруппа.Отображение         = ОтображениеОбычнойГруппы.Нет;
		Элементы.Переместить(Элементы.Статус, НоваяГруппа);
		
		КнопкаУправлениеСтатусом = Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), НоваяГруппа);
		КнопкаУправлениеСтатусом.Вид         = ВидКнопкиФормы.ОбычнаяКнопка;
		КнопкаУправлениеСтатусом.ИмяКоманды  = ИмяКоманды;
		КнопкаУправлениеСтатусом.Отображение = ОтображениеКнопки.КартинкаИТекст;
	КонецЕсли;
	
	ДоступнаяКоманда = бит_ДоступнаяКомандаУправленияСтатусом();
	Если ПустаяСтрока(ДоступнаяКоманда.ИмяКоманды) Тогда
		КнопкаУправлениеСтатусом.Видимость = Ложь;
	Иначе
		КнопкаУправлениеСтатусом.Видимость = Истина;
		КнопкаУправлениеСтатусом.Заголовок = ДоступнаяКоманда.Заголовок; 
		КнопкаУправлениеСтатусом.Картинка  = ДоступнаяКоманда.Картинка;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция бит_ДоступнаяКомандаУправленияСтатусом()
	
	ДоступнаяКоманда = Новый Структура;
	ДоступнаяКоманда.Вставить("ИмяКоманды", "");
	ДоступнаяКоманда.Вставить("Заголовок",  "");
	ДоступнаяКоманда.Вставить("Картинка",   Новый Картинка);
	
	Если Объект.Статус = Перечисления.СтатусыДоговоровКонтрагентов.НеСогласован Тогда
		Если Объект.бит_ОтправленНаСогласование Тогда
			ДоступнаяКоманда.ИмяКоманды = "СнятьССогласования";
			ДоступнаяКоманда.Заголовок  = НСтр("ru='Снять с согласования'");
			ДоступнаяКоманда.Картинка   = БиблиотекаКартинок.Обсуждения;
		Иначе
			ДоступнаяКоманда.ИмяКоманды = "ОтправитьНаСогласование";
			ДоступнаяКоманда.Заголовок  = НСтр("ru='Отправить на согласование'");
			ДоступнаяКоманда.Картинка   = БиблиотекаКартинок.НовоеОбсуждение;
		КонецЕсли;
	ИначеЕсли бит_ПраваДоступа.ПолучитьЗначениеДопПраваПользователя(Пользователи.ТекущийПользователь(), 
				ПланыВидовХарактеристик.бит_ДополнительныеПраваПользователей.РазрешеноЗакрыватьДоговор) Тогда
		Если Объект.Статус = Перечисления.СтатусыДоговоровКонтрагентов.Закрыт Тогда
			ДоступнаяКоманда.ИмяКоманды = "ОткрытьДоговор";
			ДоступнаяКоманда.Заголовок  = НСтр("ru = 'Открыть договор'; en = 'Open contract'");
		Иначе
			ДоступнаяКоманда.ИмяКоманды = "ЗакрытьДоговор";
			ДоступнаяКоманда.Заголовок  = НСтр("ru = 'Закрыть договор'; en = 'Close contract'");
		КонецЕсли;
	КонецЕсли;
	
	Возврат ДоступнаяКоманда;
	
КонецФункции

&НаКлиенте
Процедура бит_Подключаемый_КомандаУправленияСтатусомНажатие(Команда)
	
	ДоступнаяКоманда = бит_ДоступнаяКомандаУправленияСтатусом();
	ДействиеСтатуса  = ДоступнаяКоманда.ИмяКоманды;
	Если ПустаяСтрока(ДействиеСтатуса) Тогда
		Возврат;
	КонецЕсли;
	
	СтатусДоИзменения   = Объект.Статус;
	ОтправкаДоИзменения = Объект.бит_ОтправленНаСогласование;
	
	Если ДействиеСтатуса = "СнятьССогласования" Тогда
		Объект.бит_ОтправленНаСогласование = Ложь;
	ИначеЕсли ДействиеСтатуса = "ОтправитьНаСогласование" Тогда
		Объект.бит_ОтправленНаСогласование = Истина;
	ИначеЕсли ДействиеСтатуса = "ЗакрытьДоговор" Тогда
		Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДоговоровКонтрагентов.Закрыт");
	ИначеЕсли ДействиеСтатуса = "ОткрытьДоговор" Тогда
	...
	Попытка
		...
		бит_ОбновитьКомандуУправленияСтатусом();
	Исключение
		Объект.бит_ОтправленНаСогласование = ОтправкаДоИзменения;
